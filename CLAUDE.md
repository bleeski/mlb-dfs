...
## Autonomy (Ben, 2026-08-16; R157, R272, R288)
...
Ben's, not the session's: an exposure or stack change as a STRATEGY preference with no dead player behind it; anything needing `--force` or leaving a gate failing; any reduction of the legal pool; a name-crosswalk failure (under 5 of 9 hitters matched; R133 refuses it by name); truthful labels. `tools/autobuild.py` is this policy as code and records every decision in `outputs/<date>/autobuild_decisions.json`; reach for it first, and a stop it reports is a real question.
...
- Roles. BUILD, one slate: `runs/`, `outputs/<date>/`, `data/slates/<date>/`, its bank cache. ARCHIVE: `ledger/`, `data/archive/`, `data/standings/`, `data/reference/`, outside live build windows. DEV: `mlb_engine/`, `tools/`, `tests/`, `docs/`, `skills/`, `.claude/`, `CHANGELOG.md`, `MLB_Classic.md`, `MANIFEST.md`, `.gitignore`, `.gitattributes`, `requirements*`, this file. Building lineups is always permitted; ledger, inbox, engine, and contract surfaces need their role.
- Claims are live state in `claims/` (gitignored): `python tools/claim.py take <resource> --role <ROLE> --scope "..."`, `check`, `release <resource>`, `dirt --role <ROLE>`. Engine, ledger, and inbox are mutexes: a held claim stops you (read `owner.json`; never delete or take over another session's). The mutex is NOMINAL: a scoped `engine_<slug>` blocks nobody, so glob `claims/engine*` before writing and commit early (R31(d)). Slate claims are beacons (`--beacon`); never edit engine paths while one is lit, because builds re-import modules between steps.
- Git: `git add` by explicit path, never `-A` or `.`, and pass the same paths to `git commit`, because a pathless commit sweeps every session's staged files. While any claim you do not own is live: no checkout, reset, stash, restore, or clean. Foreign dirt inside your write set blocks and names the owner; elsewhere it is reported and left alone. Sessions COMMIT and PUSH; a force-push is refused, because it discards commits that may not be yours (Ben, 2026-09-16; R350). Commit subjects are at most 100 characters and carry the R-numbers; the body carries the rest (R301).
- A DEV change is not shipped until CHANGELOG.md carries its entry, in the same commit, and "change" means the whole write set (docs, skills, this file): one entry per shipped change keyed to its R-number, newest first. A completed backlog item's entry MIGRATES from `docs/backlog.md` into the changelog in the completing commit. `audit.changelog_debt` warns at the next session start; the discipline is yours. An entry claiming a rule now lives in one place carries the grep that enumerates the class, with its hit list (R233).
- One writer per contended file: only ARCHIVE edits the ledger; only DEV edits the backlog and this file. Everyone else drops a fragment in `ledger/inbox/` or `docs/backlog_inbox/` (`<date>_<ROLE>_<slug>.md`, create-only); the owner merges and retires consumed fragments.
...
3. `python tools/solver_probe.py --date <date>` before any build. Exit 3 means the bank does not fit the 130s budget: pass `time_budget_s` and accept a partial bank, or slice with `mlb_engine.optimize.bank_cache`.
...
## Hosts
- **Claude Code on Ben's Windows machine runs DEV** (Ben, 2026-09-15). The full gate runs in one call (about 9 minutes on the pinned `.venv`; `.claude/settings.json` raises the Bash timeout for it), `rm` works, an M item fits one session. `.claude/` carries the settings, the command-guard and session-start hooks, path-scoped rules, and the `/dev-session` and `/land` skills. Host facts go in `CLAUDE.local.md` (gitignored), never here. The shell is PowerShell: prefer `python` entry points over bash idioms.
- **Cowork runs BUILD and ARCHIVE.** Its device VM mounts Ben's disk with no `rm` (`mv` into `_to_delete/`), a lock CLASS to sweep before every git write (R109), and the bash budget below. Read `docs/cowork_sandbox.md` before the first bash call there.

## Sandbox
The Cowork inner bash budget is 130s; the real per-call ceiling is about 180s with an explicit timeout. One number, this one (R271); do not re-derive it per call. The rest of the sandbox procedure is `docs/cowork_sandbox.md`.
