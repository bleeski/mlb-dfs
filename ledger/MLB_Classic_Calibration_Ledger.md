...
1. Macro: from the repo root, `python tools/audit.py --run-tests --terse` -> `PASS  v2.26.0  27 modules  1386 tests`. (Corrected 2026-08-28 by DEV on a `ledger` claim, pin line only, THIRD correction this date: R37(2)(a) + R37(2)(b) + R263 moved test_core 911 -> 934 and test_showdown 79 -> 91, so the total is 1386 and the current per-suite pins are 934 / 91 / 254 / 9 / 98. The module count is unchanged at 27; both bands live in `execution_pipeline` and the Showdown shadow in `showdown_theses`, so nothing new was added to the filesystem. **This is the first STRATEGY change in this line's history rather than a defect batch, and it is Ben's dated decision of 2026-08-28, not a session's inference** -- CHANGELOG.md carries the `Decided` entry above the shipped one. Three readings matter for post-slate work, and the first two change what an archived brief means. FIRST, R37(2)(a): `primary_stack_min_size` is no longer 4 everywhere. It is 5 on a portfolio whose every contest resolves `payout_breadth <= 0.02` (WTA, one-seat satellites, solo shots, mme_gpp/mini-MAX, single_entry_gpp) on a slate of 3+ games, and 4 otherwise -- including on any MIXED entered set, because R34's floor merge takes the least demanding declared value and one broad-payout contest retires the floor for the whole portfolio. So two builds on the same night can legitimately ship different floors, and the brief's `shape_bands` block is what says which and why; `floor_5_binds_portfolio` is the one-line answer. 1-2g slates are EXEMPT and that is the tranche's own counter-case, not caution: 3.21 puts the cohort on 5-3 (+16.5pp) and 4-4 (+12.0) there with 5-2-1 FLAT (+1.4). SECOND, R37(2)(b): `min_five_stack_share_pct` no longer ships 0.0 in practice. On a portfolio whose every contest sits in breadth 0.10-0.22 it is derived as 0.75x the MEASURED five-stack field share for that slate-size bucket, clamped to Ben's 15-25% band -- 19.3% at 3.21's measured 25.7% on 5g+. Read `five_stack_field_share.source` in any brief before treating that number as current: `ledger_measured` means it came from THIS section's dated values and `as_of` says when, `archive_rollup` means a refreshed file answered, and `unavailable` means the slate size could not be resolved and the quota was OFF rather than defaulted. Today every build reads `ledger_measured`, because nothing aggregates `stack_pattern` out of `data/archive/*/mined_*.json` yet -- that producer is R37(2)(c)'s open remainder and it is ARCHIVE's. The same item retired R34's hard fail: a quota this bank cannot carry now relaxes on a ladder and COUNTS, so `five_stack_quota.relaxations` joins the set of numbers that must be zero before a portfolio is clean, and a NEW flag `reuse_dependent` names the case where the quota is satisfiable only through candidate reuse -- which R34 shipped as a bare infeasibility with no control attached to it. THIRD, and this one is a bound rather than a change: R263's Showdown shadow prints `pitcher_cpt_ceiling_pct` beside the pitcher-CPT share, and on a two-arm slate that ceiling is `2 * floor(cpt_cap * n) / n` -- 42.1% at n=19, 36.4% at n=11, 50.0% at n=20, averaging 45.0% over n=9..24. Read every archived Showdown pitcher-CPT share against that ceiling and not against a 50% target: the target is unreachable at most entry counts under the 0.25 per-player captain cap, and the root doc's proposed 48-52% hard band is above the maximum at most n. The coarse weight lever Ben asked for was built and DECLINED on that arithmetic, measured over 96 apportion-and-solve checks: mean share 43.66% -> 44.91% for 19 additional captain-cap relaxations. R156's 0.45 on `pitchers_duel` stands untouched. Everything here is deterministic construction control, observed cohort shares, and counted relaxations; nothing in it is a win rate, a cash rate, or a probability claim.) (Corrected 2026-08-28 by DEV on a `ledger` claim, pin line only, SECOND correction this date: R205 + R236 moved test_core 902 -> 911 and test_paste_lineups 87 -> 98, so the total is 1351 and the current per-suite pins are 911 / 79 / 254 / 9 / 98. **The MODULE count moved for the first time this line has ever carried, 26 -> 27**: `mlb_engine/intake/paste_odds.py` is a new engine module, and the count comes off the filesystem, so a session reading `26 modules` after this date is reading a stale line and not a missing module. Two readings matter for post-slate work, and the first one changes how an archived F1 should be read. FIRST, R205: `parse_the_odds_api_totals` averaged moneylines ACROSS BOOKS in American-odds space, per side independently, and American odds are discontinuous at +/-100 -- -104 and +100 are adjacent prices, both about half, and their mean is -2.0, which reads as a 98% favorite. On the 2026-08-19 `1235_4g` ATL@MIN game (run `20260819T153118Z_49f8ec18`) DK posted -104/-104 and FD posted -108/+100, the parser emitted `{'ATL': -106.0, 'MIN': -2.0}`, and the 8.5 total split ATL 5.36 / MIN 3.14 against the 4.26 / 4.24 the corrected arithmetic gives. Read any archived multi-book F1 on a NEAR-PICK'EM game as suspect in exactly that direction: the blast radius is INVERTED, since the closer a game is to a coin flip the more likely two books straddle the boundary and the larger the fabricated split, and F1 clips at 0.85/1.15, so a pick'em could take the maximum boost on one side and the maximum penalty on the other. A one-book fetch was never affected -- one book cannot straddle itself -- and `source` in each packet entry names the books, so an archived entry listing a single book needs no re-reading. The packet now de-vigs EACH COMPLETE BOOK first, averages the normalized probabilities, and carries the vig-free price implying that average, with `moneyline_basis`, `moneyline_probabilities` and every book's raw posted pair in `moneyline_books` beside it; the moneyline in a post-R205 packet is therefore DERIVED and is not a posted price, and `moneyline_books` is what to quote when the question is what the market said. The game TOTAL keeps its cross-book median deliberately (a total is linear, with no discontinuity anywhere in its range) and now says so in `total_basis`. SECOND, R236, and it bounds what can be graded: `api.the-odds-api.com` is proxy-gated in cloud sessions exactly as `statsapi.mlb.com` is, so on 2026-08-24's 1940_7g a certified file shipped with `f1_games_priced: 0` while `enrichment.signal_applied` read true, and the F1-live rebuild moved the apex mean 144.0 -> 149.48 and cut CIN from three stacks to one. Read `counts.f1_games_priced` directly on any archived cloud-session build before treating its F1 as live. `tools/odds_from_paste.py` is the paste path for that case, per named book; the 2026-08-24 packet itself carries a single synthetic `consensus` book that was hand-averaged, so its prices are not auditable to any book and it is a shape reference only, never a price fixture. Everything here is parsing arithmetic and provenance; nothing in it is an outcome claim.) (Corrected 2026-08-28 by DEV on a `ledger` claim, pin line only: R212 + R213 + R214 + R215 moved test_core 884 -> 902, so the total is 1331 and the current per-suite pins are 902 / 79 / 254 / 9 / 87. Note the line was TWO moves stale before this one, and by its own rule the dict won: R194/R235 took test_core 863 -> 876 and R219 + R220 took it 876 -> 884, neither carried here, so it read 1292 against a dict summing to 1313. This is the ENUMERATED-SIBLINGS batch -- four P1 defects, each the sibling a previous fix did not enumerate -- and three readings matter for post-slate work. FIRST, R214, and this is the one that changes how an archived supervised run should be read: `autobuild`'s `--controls-override` was appended AFTER `--passthrough`, and because the supervisor's dict starts EMPTY and gains only structural floors, argparse's last-wins did not make the supervisor outrank the operator key by key -- it DROPPED the operator's entire dict the moment any floor was applied. So on any pre-2026-08-28 supervised run where an operator passed controls through `--passthrough` and the log shows an `apply_structural_floor` record, attempt 1 ran with the operator's values and every later attempt ran WITHOUT them, on posture caps, and `autobuild_decisions.json` says nothing about what left because its single `controls=` field printed the floors and called that the state. That is precisely the shape of CLAUDE.md's R157 rescue, which is performed by hand through this passthrough. The log now carries `user_controls` / `derived_controls` / `effective_controls` on every record that touches controls and once at the top level, and exactly one merged `--controls-override` reaches the build. SECOND, R212: the supervised wall-clock stop returned 5 without writing anything, so a MISSING `autobuild_decisions.json` for a slate does not mean the supervisor never ran -- it may mean the run spent its whole twelve-minute window, which is exactly the run whose post-mortem is worth having. Reachable by construction on defaults. All ten exits from `main()` now flush, including the classification-drift refusal that returned before the log object existed. THIRD, R215, closing R167's units family: `max_game_exposure_pct_by_game` was in no gate at all while all three `_game_cap_count` sites clamped, so read a per-game cap above 1.0 in any archived brief as that cap switched OFF and not loosened -- the same reading R167 gave `showdown.exposure_cap_count`, arriving through the one control `late_swap.py` steers operators at. A NaN and a bool did the same thing more quietly: every comparison against NaN is False, so a NaN cleared the `> 1.0` test at every boundary and died later inside `math.floor` after the bank had spent, and `bool` is a subclass of `int`, so `True` returned 1.0 in silence. Both are rejected now, and both boundaries store the COERCED float rather than validating a copy and forwarding the raw value. Also in this batch, R213: a mistyped `--lineups` path and a torn `lineups_feed.json` were raw tracebacks with no brief, which `autobuild` recorded as `refused with no remedy this supervisor may take, errors=[]` -- a crash wearing a refusal's label. A named feed that cannot be read is now exit 4 with the path and the parse error; a torn staged cache is ABSENT, falls through to the guarded fetch leg beside it, and is named in `feed_note.staged_feed_unreadable`. Everything here is exit contracts, units and record-keeping; nothing in it is an outcome claim.) (Corrected 2026-08-25 by DEV on a `ledger` claim, pin line only: R234 moved test_upload_integrity 238 -> 254, so the total is 1292 and the current per-suite pins are 863 / 79 / 254 / 9 / 87. R234 lands R191 + R192, the two defects in `tools/preflight_upload.py` -- the command CLAUDE.md puts between a build and Ben's manual upload -- and both matter for post-slate reading. FIRST, the `--salary` auto-resolve was a read of `runs/latest_valid_run.json`, last writer wins, checked against nothing: on a Showdown delivery `build_slate.py` promotes no run, so the pointer NECESSARILY named some other build, and three field hits (2026-08-20 `1835_1g_sd` and `2010_1g_sd`, 2026-08-24 `texcws_sd`) each hard-FAILED a clean delivered file on `rostered player ID(s) absent from the salary file`. Read any archived preflight output from before this date that shows `[promoted run snapshot]` beside an absent-ID or low-overlap failure as a WRONG-INPUT result and not a finding about the file; the resolver now checks contest geometry and the rostered ids and REFUSES at exit 3 naming the flag. SECOND, and this is the one that touches the archive: the showdown `top exposure` line counted the DRAFTABLE ID, and a Showdown export prices every player twice, so one human split across a CPT key and a UTIL key and neither half reached the printed five. On the 2026-08-19 LAD@COL delivery Sasaki sat at 9 of 19 -- exactly the solver's `floor(0.5*19)=9` player cap -- and did not appear on the line at all, while Ohtani captained 4 and rode 8. Read EVERY pre-2026-08-25 Showdown preflight `top exposure` list as a UTIL-only ranking that under-reports every captained player and can therefore show a breached `max_player_exposure_pct` as clean; the brief's `player_exposure.by_player` was always the correct count and the two disagreed in silence. The line now counts the PERSON and prints the captain distribution separately, since the captain cap (0.25) and the player cap (0.50) are different controls. Classic was never affected -- no multiplier role, so the two counts coincide. Everything here is file-level checking and deterministic counting; nothing in it is an outcome claim.) (Corrected 2026-08-24 by DEV on a `ledger` claim, pin line only, SECOND correction this date: R167 + R168 + R169 moved test_core 841 -> 863 and test_showdown 77 -> 79, so the total is 1276 and the current per-suite pins are 863 / 79 / 238 / 9 / 87. This is the LOST-WINDOW batch -- three failures that arrived after the build window was already spent, or in a form no documented consumer could read -- and two readings matter for post-slate work. FIRST, R167: a units slip in `--controls-override` (`max_pitcher_exposure_pct=45`, 45 typed for 0.45) used to CLEAR the checkpoint, which printed `45 -> cap 10` and reported the capacity check PASSED, then raise uncaught inside `execute_portfolio` after the bank had spent minutes, leaving the run at status `building` with no diagnostics. R71(a) said it fixed 'both `_cap_count` copies'; there were THREE, and the third -- the closure inside `_feasibility_report`, the one the operator reads -- still clamped `min(1.0, value)`. A fourth was found while landing this: `showdown.exposure_cap_count` validated nothing at all, so a slipped Showdown cap returned a count of 25n, capped nobody, and left every R153 relaxation counter reading CLEAN because nothing was ever relaxed. Read any archived Showdown portfolio whose brief shows a slipped `max_cpt_exposure_pct` or `max_player_exposure_pct` as having had that control switched OFF, not loosened. The units rule now lives in one function (`contest_allocator.assert_fraction_cap`) that the solve, the checkpoint and both Showdown caps call, and `build_slate` refuses at exit 4 before staging anything. 1.0 stays legal, because R157's rescue opens all three caps to 1.0 as a sanity check; 0 stays legal, because every posture ships `min_five_stack_share_pct: 0.0`. SECOND, R169(b): `autobuild`'s decision log fell back to `datetime.now()` -- the container's UTC calendar -- because build_slate's `pool_blocked` and exit-10 payloads carried no `date`. After 8pm ET the one artifact explaining why a build never happened filed under TOMORROW's `outputs/`, the R65/R95 class. If a supervised run's decisions are missing from a slate's directory, look one day forward. Those payloads now carry `date` and the log's fallback is the salary file, then `today_et`. Also in this batch: `main()`'s supplied-feed-rejected path returned a TUPLE, so a legitimate refusal exited 1 with a stray tuple on stderr and every documented consumer read it as a crash (R168(a)); the first lineups fetch of the day was unguarded, so the ordinary pre-lock morning state of no staged feed plus no route to statsapi was a raw traceback with no brief (R168(b)); a timeout lost the ENTIRE decision log and exited off the 0/3/4/5/10 contract (R169(a)); the structural-floor applier validated the CHECK name and then moved whatever control the remedy SENTENCE named, so a sentence under a structural check could have moved an exposure cap (R169(c)); and `--passthrough` was naively split and appended AFTER the supervisor's own flags, so an operator override silently outranked the floors the log claimed had landed (R169(d)). `autobuild` had ZERO tests before this batch and now has seven. Everything here is exit contracts, units and record-keeping; nothing in it is an outcome claim.) (Corrected 2026-08-24 by DEV on a `ledger` claim, pin line only: R159 + R160 + R189(2) moved test_core 825 -> 841, so the total is 1252 and the current per-suite pins are 841 / 77 / 238 / 9 / 87. This batch is intake truth on the no-fetch path, and one reading matters for post-slate work on any slate DK had fully posted. R159(a): `dk_order_coverage` read the RAW salary rows while the pool's merge read the status-FILTERED map, so a single `Status=IL` flip inside a DK-posted 1-9 left the team reading COVERED -- fetch skipped, empty feed -- while the merge saw eight slots, refused the side outright, and dropped the whole posted nine to `fallback_top9_appg`, taking the team's `Starting=SP` arm with it because the only code that attached a probable ran inside the accepted-side loop. Read any archived pre-2026-08-24 build reporting `fallback_top9_appg` on a team DK had posted as a side whose real batting order was ON DISK and unused: the pool ranked that team's whole roster by AvgPointsPerGame, which is R60's defect arriving through a different door, and a bench bat with a better season could take a posted starter's seat. The surviving eight are now seeded as a PARTIAL through R60's path, `dk_side_readings` is the one reading of a posted side, and a degraded side counts as NOT covered so the fetch that fills the ninth slot still happens. R160: a game whose DK `Game Info` parses as a matchup but not a time -- real on doubleheader game 2s, which ship `TBD` -- crashed the front door with `ValueError: Invalid isoformat string: ''` instead of blocking; that game is no longer synthesized without a start time, the status map's `_parse_utc` is guarded, and the blocker names the CAUSE rather than the consequence it used to surface (`no probable or declared starter` on both sides). R189(2) is the F4 half and is the one to carry: a probable with a NAME but no joinable Savant row scored a silent 1.0 and appeared in NO bucket, because `teams_without_opposing_probable` counts only missing NAMES. So F4's SP-QUALITY term was dead on every plain-text mlb.com paste slate (Ben's browser copies carry no MLBAM ids) with `sp_quality_available: True` in the report, and R190's platoon finding is the OTHER half of the same factor -- read an archived paste-sourced build as having carried neither F4 term unless its report says otherwise. `sp_quality_applied`, `sp_quality_unavailable` (with a reason per team) and `sp_quality_name_joined` now separate 'the table loaded' from 'the term applied', and an id-less probable is recovered by the Savant `last_name, first_name` join before anything is scored neutral. All of it is deterministic intake and labeled priors; nothing here is an outcome claim.) (Corrected 2026-08-23 by DEV on a `ledger` claim, pin line only: R190 moved test_core 821 -> 825 and test_showdown 66 -> 77, so the total is 1236 and the current per-suite pins are 825 / 77 / 238 / 9 / 87. R190 is F4's platoon hand, dead at three boundaries, and one reading matters for post-slate work on every slate whose feed came from `tools/fetch_slate_bundle.py`: the bundle fetched `batSide` and never `pitchHand`, so every probable arrived `hand: None` -- 30 of 30 measured on 08-18 and again on 08-19 -- and F4's platoon component was INERT for every hitter while `bat_side` sat populated on all 270. The only place it showed was `f4_platoon_applied: 0` in the brief's counts block, beside `signal_applied: true`, `degraded: false` and `factors_inert: []`. Read any archived build from a bundle-sourced feed knowing its hitters carried the opposing-SP QUALITY term and none of the platoon term. The Showdown half is narrower and one-sided: `showdown_handedness` keyed the hand by the feed's raw team abbrev against DK's `Opponent`, so `AZ` never matched `ARI` and on the 08-19 1610_1g_sd ARI@BOS build every BOS hitter took a flat 1.00 against RHP Pfaadt instead of 0.94/1.04 while the brief read `teams_with_hand: 1` -- half-clean, because the side needing no remap resolved. Three BUILD sessions filed the bundle half before it was fixed, each working around it by hand, and the reference implementation SKILL.md tells a sandbox session to copy was the one path that never did the backfill. Also in R190: the gate's own call ceiling was a hardcoded 39s measured against a `device_bash` call, which capped `--gate-budget` underneath itself, so a unit slower than 39s could never land and the gate could not print this line at all in a container whose cap is ~178s. `--gate-ceiling` / `MLB_GATE_CEILING_S` is now the host's to state; the device default is unchanged.) (Corrected 2026-08-21 by DEV on a `ledger` claim, pin line only: R156 moved test_showdown 62 -> 66, so the total is 1221 and the current per-suite pins are 821 / 66 / 238 / 9 / 87. R156 fixes a live delivery: the 2026-08-21 ATL@MIL Showdown build shipped a "Pitchers duel - both starters rostered" lineup carrying Misiorowski alone, no Sale, because `build_thesis_ladder`'s generic lock filter drops whichever starter is assigned captain (`!= cpt`), so once the overlap bound made that captain infeasible and `solve_ladder`'s relaxation ladder substituted him out, nothing else required his presence. Both starters are now unconditional `hard_locks`, merged into a thesis's `locks` regardless of who captains, and `cpt_ladder` is restricted to the two arms so a bat is never captained under this thesis's name. The original design also locked a top-band hitter per side on top of both arms; measured infeasible outright on this slate's own salary structure (two arms plus two top-band bats ran to $47,500-48,100 of the $50,000 cap before the remaining two roster spots), so those hitter locks are gone and the four non-arm spots are a free salary/points choice. Re-verified across 380 apportion+solve checks (n=3-40, six moneyline scenarios on the tracked fixture, four on the real ATL@MIL data): 0 infeasible. Weight bumped 0.30 -> 0.45 so a slate with room schedules a second pitchers_duel slot (one variant per captain) more often, per Ben's read that a 25% captain cap leaves room for exactly that hedge -- this is a bias, not a guarantee, since a representative n=19 case hit allocation=2 in only 3 of 6 tested moneylines on the fixture and 2 of 4 on the real data. Read the invariant as "both arms always roster together" and the second slot as "more likely, not certain.") (Corrected 2026-08-19 by DEV on a `ledger` claim, pin line only, SECOND correction this date: R154 moved test_core 809 -> 821, so the total is 1217 and the current per-suite pins are 821 / 62 / 238 / 9 / 87. R154 is the first time the ownership prior reaches the optimizer, and the reason it could is the FIRST GRADED PREDICTION: contest 194022265 (2026-08-19 Relay Throw, 7,833 entries) is the first slate holding both a pre-lock `ownership_pred_<tag>.json` and an archived standings export, which is the pairing R135 built the emit step for. Graded against realized %Drafted the prior scores Spearman +0.581 on `large_field_gpp` and +0.66 on `cash`, with the LEVEL under-predicted by about a third and NOT fixable by rescaling (MAE 5.71 raw, 5.92 normalised to the known 1000% slot budget). Read those two columns as opposite findings: the ORDERING is usable and the MAGNITUDE is not, so R154 spends the ordering on constraints and spends none of the magnitude on an objective coefficient. Three numbers a post-slate reader should carry. FIRST, the realized relationship on this contest is monotone across every finish band -- 1st at 167.0% cumulative ownership with 4.00 sub-10% bats, top 10 at 160.1% / 4.50, cashed at 177.9% / 3.62, missed at 190.3% / 3.25, field at 187.5% / 3.33 -- and the delivered entry sat at 254.7% with 2, the 95.6th percentile of chalkiness, and finished 5287th. SECOND, that was KNOWABLE BEFORE LOCK: scored on the prediction alone the same entry was the 97.6th percentile chalkiest, and `qa_portfolio.py` printed it as +37.6 pp chalk-positive, so the number was computed, written to disk and displayed while nothing in the objective consumed it. THIRD, and the caveat that bounds all of it: the entry also scored 56.35 against a field mean of 64.74, so it lost on POINTS as well as on leverage and one result cannot separate them, and on this slate the chalk BUSTED (Skenes 8.95 at 53.2% owned), which is what makes the band table look so clean. On a chalk-hits slate the ordering inside the cashing bands inverts. The structural case for leverage rests on duplication splitting a top-heavy prize, never on chalk busting, and the MAGNITUDES above are ONE contest and are not a calibration. What shipped is two LINEAR constraints on `build_single_lineup`, `max_cumulative_ownership_pct` and `min_low_owned_hitters` (threshold default 10.0, the same bar 3.17 already uses so the floor and the archived observation share one definition), both OFF unless a caller passes a number, plus `attach_projected_ownership` writing the single `Projected_Ownership_Pct` column and never `Ownership_Tier`. Nothing calls attach on the production path yet, so no build has changed. Also worth carrying: the hitters-only filter on the floor was first written `slot != 'P'`, and DK's slot vocabulary is P1/P2, so it was true for both pitcher slots and a cheap arm satisfied a leverage-bat floor -- 4 asked, 2 shipped, every counter clean. Caught in bring-up, same shape as R153's two leaked caps one week earlier.) (Corrected 2026-08-19 by DEV on a `ledger` claim, pin line only: R153 moved test_showdown 56 -> 62, so the total is 1205 and the current per-suite pins are 809 / 62 / 238 / 9 / 87. R153 is Ben's tightening of the Showdown portfolio controls -- captain exposure 0.33 -> 0.25, and a THIRD control, a player-exposure cap at 0.50 that did not previously exist. Two findings a post-slate reader should carry. FIRST, the gap the new cap closes is real and was invisible: on the 2026-08-19 1610_1g_sd ARI@BOS build the overlap bound was clean at 0 relaxations, the captain cap was clean, all 19 rosters were unique, and Nick Sogard appeared in 12 of 19 entries (63.2%) because he was the cheapest posted leadoff bat and every BOS-leaning thesis reached for the same salary relief. Read any archived Showdown portfolio from before this date knowing that NOTHING measured single-player concentration; 'clean' meant two controls held. SECOND, and this is the part worth generalizing beyond Showdown: a cap enforced anywhere other than where the roster spots are actually SPENT is not a cap. Both caps leaked on the first cut and both leaks reported themselves clean. `solve_ladder` enforced no captain cap at all -- it trusted `build_thesis_ladder`'s apportionment, and the lock-relaxation rung then substituted a captain with no cap awareness, so Payton Tolle finished at 5 of 19 (26.3%) under a cap count of 4 while every relaxation counter read clean, because a lock substitution is not an exposure event. And the player cap carved out a thesis's own `cpt` and `locks` on the reasoning that a lock is the more specific instruction -- correct reasoning, wrong consequence, since the ladder assigns captains BY NAME and a captain is a roster spot: Wilyer Abreu reached 11 of 19 (57.9%) under a 50% cap with `player_relaxed: 0`. Both now bind against the REALIZED set, a capped player comes off the thesis's own cpt and locks before the solve, and that removal is NAMED in `player_exposure.cap_reassignments` / `.locks_dropped` rather than counted as a relaxation, because nothing gave way -- the cap held and the thesis label moved. Delivered under the corrected caps: captain max 21.1%, player max 47.4%, 0 relaxations on all three controls.) (Corrected 2026-08-18 by DEV on a `ledger` claim, pin line only, FOURTH correction this date: R148(a) + R152 moved test_core 792 -> 809, so the total is 1199 and the current per-suite pins are 809 / 56 / 238 / 9 / 87. The sandbox caveat further down this line is REPLACED rather than re-measured, and that is the finding: its class boundaries dated from the 594-test tree, and re-measuring them would only have restarted the clock on the same staleness. The macro still does not fit a 45s `device_bash` call -- measured this date, the five suites are 257.5s of test time (test_core 809 tests / 133.0s, showdown 56 / 12.7s, upload_integrity 238 / 52.0s, golden_replay 9 / 44.9s, paste_lineups 87 / 14.9s) -- and the supported answer is now `python tools/audit.py --gate-run` repeated until it prints GATE COMPLETE, then `--gate-report --terse`, which prints the same pinned line when every unit ran and GATE INCOMPLETE when it did not. The whole gate ran that way this date, green, in about twenty calls. Two facts a post-slate reader should carry. FIRST, no fixed chunk plan can be right here: the same test class measured 24.5s at 19:50 and did not finish in 42s at 20:15, same VM and same code, so the runner learns each unit's time and resumes from what it recorded instead of following boundaries someone measured once. SECOND, a test CLASS is not always a small enough unit -- a cut-off class is re-run as its individual test METHODS on the next call, and a method that still cannot finish is REFUSED by name rather than skipped, because an environment that cannot run a test is a thing to report and not a thing to drop from the gate. R148(a) rode with it: the audit's default-branch check reads GitHub through `ls-remote --symref` instead of this clone's cached `origin/HEAD`, so a null there means "asked and agrees" only when `default_branch_source` says `remote`, and carries R147's classified reason when it could not ask.) (Corrected 2026-08-18 by DEV on a `ledger` claim, pin line only, THIRD correction this date: R133 moved test_upload_integrity 218 -> 238 and test_paste_lineups 82 -> 87, so the total is 1182 and the current per-suite pins are 792 / 56 / 238 / 9 / 87. R133 closes the partial-side remainder R60 did not, and two of its findings change how a post-slate reader should treat an archived build. FIRST, a team merely SHORT OF NINE hitters no longer blocks: three separate readers answered that question and two disagreed, with the strictest winning silently, so any pre-2026-08-18 slate where an operator forced past a thin-team blocker was working around a contradiction rather than a real structural limit. The bar is now MAX_HITTERS_PER_TEAM (5), the count that fills a maximum DK stack; 5 through 8 warns and certifies, under 5 blocks, and `pool_report.thin_teams` is the one definition. The contract's own 5-of-9 sentence was always the CROSSWALK bar and is unchanged. SECOND, the two documented overrides did not work and now do: `--assume-gates lineup_gate_passed` promoted a gate only where the derivation was None, so an assertion against a derived False was discarded WHILE STILL BEING RECORDED in `assumed_gates` -- a recorded assumption that changed nothing. Read any archived `assumed_gates` list from before this date as a REQUEST and not an effect. The assertion now lands in `overridden_gates` with the evidence string it contradicts, and a request that is neither an assumption nor a legal override is named in `gates_assumption_refused`. What did NOT change is the seeded ninth's SELECTION weight: a fill standing in for an unknown is still selectable at parity with an observed posted starter, that is R133(2), it is filed to MLB_Classic.md section 6 awaiting Ben, and the live case is the 2026-08-12 1840_3g build that carried a non-playing bench catcher in 6 of 18 certified entries.) (Corrected 2026-08-18 by DEV on a `ledger` claim, pin line only, SECOND correction this date: R117 moved test_paste_lineups 75 -> 82 and test_core 781 -> 792, so the total is 1157 and the current per-suite pins are 792 / 56 / 218 / 9 / 82. R117 closes a paste-intake hole worth carrying forward for post-slate reading: mlb.com renders the probable's hand line TWO ways, alone on its own line and with the record trailing it on one line, and only the first was ever read. The second render attached ZERO probables with no warning anywhere, DK's `Starting` fallback then supplied a name with a null MLBAM id and an empty hand, and that kills BOTH F4 terms -- the Savant join and the platoon prior -- so `f4_non_neutral: 0` of 180 hitters certified clean on 1910_10g. Read this before trusting an archived slate's F4 distribution: any build sourced from a joined-render paste before 2026-08-18 ran with the hitter side's entire opposing-pitcher signal at neutral, and repairing 1910_10g's twenty probables by hand moved primary stacks off Oracle Park, so the effect on SELECTION is material rather than cosmetic. Two surfaces now answer it without re-reading a log: `pool_report.opposing_probables_incomplete` names the sides whose opposing probable carries no id or no hand, and the brief's `factors_inert` names any factor that scored rows and moved none of them, beside the gates rather than inside them.) (Corrected 2026-08-18 by DEV on a `ledger` claim, pin line only: R136 moved test_core 762 -> 781 and the total to 1139; current per-suite pins are 781 / 56 / 218 / 9 / 75. R136 gives `tools/qa_portfolio.py` a fourth section, the FIELD-FACING one: sections 2 and 3 measure a portfolio against the slate and none of them says whether the entered set looks like everybody else's. Four columns, per contest and conditioned on that contest's archetype, never pooled -- cumulative chalk against the field's own mean, bottom-tier hitters carried, Showdown captain own-tier, and salary left against 3.17/3.18's archived medians. Three readings this Quick Card should carry forward. FIRST, the field mean is an ACCOUNTING IDENTITY and not a simulation: over a field whose player shares are own_p, mean cumulative ownership is exactly sum(own_p^2), assuming nothing about how the field builds. SECOND, 3.17's sub-10% carry threshold DOES NOT TRANSFER to the v0.1 prior and the panel does not pretend it does: measured on the 2026-08-17 1905_7g slate the prior spreads its 800% hitter budget over 284 rows, the top hitter reaches 8.9%, and every hitter is therefore 'sub-10%', so the literal column returned 8-of-8 on every entry -- a constant, not a column. It ships as the prior's own within-pool TIER instead, and the absolute count returns with the fitted model. THIRD, the same slate is the first portfolio ever read on this axis and it reads chalk-POSITIVE in all six contests (+11.5 to +28.0 pp against the prior's own field mean) while carrying a bottom-tier hitter in 2 of 9 entries and leaving $0 of salary in 8 of 9 -- against archived medians of $250 for winners and $200 for the field. Read the SIGN and the cross-contest ORDER, never the magnitude: the prior under-concentrates by a measured 10.44 points (R135) and the ledger deltas are on the actual %Drafted scale. Everything in the panel is a deterministic review proxy over a LABELED PRIOR; the panel gates nothing.) (Corrected 2026-08-17 by DEV on a `ledger` claim, pin line only, EIGHTH correction this date: R135 + R151 moved test_core 743 -> 762 and the total to 1120; current per-suite pins are 762 / 56 / 218 / 9 / 75. R135 wires the predict-then-grade ownership loop that this Quick Card and three docs have named since July while nothing called it: `tools/ownership_pred.py emit` writes `outputs/<date>/ownership_pred_<tag>.json` before lock from the salary file, feed and odds packet, and `grade` scores it against an archived standings export at mine time. THE ARCHIVE-SIDE STEP IS NOW IN THE RUNBOOK (Job 1, step 9) and it is ARCHIVE's to run: one grade per contest, `--archetype` taken from that contest's own JSON and never the prediction's default, because ownership is conditioned on archetype and field size and never pooled. The block it prints files beside the miner's under the slate's A-NNN entry. Two numbers in it are the point: the per-feature buckets (which structural signal missed) and TWO baselines rather than one, because flat-12 is R10's named bar but a constant 12% spends 2160% of a 1000% roster budget, so beating it is nearly free -- flat-budget (800/200 spread evenly) is the null worth arguing with. Measured on the archived 06-03 grid the session it landed: prior MAE 14.35 pts against flat-12's 14.53 and flat-budget's 15.64, mean signed -10.44, Spearman 0.266, and the arms bucket at MAE 46.95 because that old salary file has no `Starting` column, so `probable_sp` was INERT and every arm took the non-probable discount. Everything here is an UNCALIBRATED STRUCTURAL PRIOR or a deterministic error measurement over ONE contest; one contest never moves a prior. R151 rode with it, on the DK/feed merge R143 built: an accented feed name never matched DK's plain-ASCII spelling, so on the 2026-08-16 1335_8g slate 7 of 15 posted sides reported a false DISAGREEMENT and ten hitters silently lost the feed's MLBAM id and bat side -- both F4 terms dead for those bats, with `f4_handedness_unavailable` unable to see it because it only fires when a side loses all nine.) (Corrected 2026-08-17 by DEV on a `ledger` claim, pin line only, SEVENTH correction this date: R126 moved test_core 716 -> 743 and the total to 1101; current per-suite pins are 743 / 56 / 218 / 9 / 75. Note this line was TWO moves stale, not one: R127 moved test_core 698 -> 716 and the total to 1074 earlier the same date without carrying it here, and `skills/generate-lineups/SKILL.md` was stale by the same pair. R126 makes Ben's stated objective a measurement for the first time: apex (portfolio ceiling total, mean, best single entry off the run's own Ceiling column) and washout (per game, zero that game's HITTERS, keep the arms, report the percent of portfolio ceiling retained, entries left fully intact, and a histogram over entries of bats drawn from that game) now ride `diagnostics.json` and the brief's exposure block, with one review line at build time. Read for post-slate work: the RETAINED PERCENT is a slate-size artifact and is NOT comparable across slates, because it is bounded below by the arms plus the other games' bats; ENTRIES FULLY INTACT is comparable and is the number that separated the two 2026-08-15 2138_2g builds, which posted the same apex and the same retained percent. All of it is a deterministic review proxy off a projection input, never an outcome.) (Corrected 2026-08-17 by DEV on a `ledger` claim, pin line only, FIFTH correction this date: R147 moved test_core 698 and the total to 1056; current per-suite pins are 698 / 56 / 218 / 9 / 75. R147 came out of a review of R142-R146 rather than a build: a failed `git fetch` now says WHICH of three things stopped it -- no network, a rejected credential, or unknown -- because both audit.py and sync_check.py hard-coded "check the token scope or expiry" for every failure and sent a session at a working PAT while the real cause was a sandbox with no outbound network. Measured live the same day: a cloud Cowork session's DEVICE VM cannot reach GitHub at all (its proxy 403s CONNECT even for a public repo), while the container reaches it normally, so R146's fetch succeeds in one and never in the other. Underneath it, a FAILED fetch was truncating .git/FETCH_HEAD to zero bytes and resetting fetch_age_hours to 0.0, which reported contact that never happened and kept R145's stale-contact warning from ever firing.) (Corrected 2026-08-17 by DEV on a `ledger` claim, pin line only, fourth correction this date: R146 moved test_core 683 -> 688, so the total is 1046 and the current per-suite pins are 688 / 56 / 218 / 9 / 75. R146 found a valid fine-grained PAT already in `REPO/.env` under `GH_PAT` that `find_token` never looked for, so the audit now FETCHES before measuring and `behind` is a measurement rather than a hedge.) (Corrected 2026-08-17 by DEV on a `ledger` claim, pin line only, third correction this date: R145 moved test_core 676 -> 683, so the total was 1041 and the per-suite pins were 683 / 56 / 218 / 9 / 75. R145 makes the audit report how stale the clone is, since `git log` cannot: ahead, behind, hours since the last remote contact, and the still-live trap that `origin/HEAD` points at `master` while the work is on `main`.) (Corrected 2026-08-17 by DEV on a `ledger` claim, pin line only, second correction this date: R143 moved test_core 667 -> 676, so the total was 1034 and the per-suite pins were 676 / 56 / 218 / 9 / 75. R143 is Ben's lineup-source ranking: the DKSalaries `Starting` column is now the FIRST source for batting order, a paste second, an API pull third, per side. Measured on the 2026-08-16 file, 15 of 16 sides carried a complete 1-9 that the build had been fetching over the network instead.) (Corrected 2026-08-17 by DEV on a `ledger` claim, pin line only: R142 moved test_core 657 -> 667, so the total was 1025 and the per-suite pins were 667 / 56 / 218 / 9 / 75. Same session, and the reason R142 exists: the Cowork-installed copy of `generate-lineups` was found 85 commits behind the repo, and the audit now warns when an installed skill snapshot drifts from `skills/<name>/SKILL.md` -- WARNING only, and silent rather than clean when it cannot see the cache, which is the case from Ben's own PowerShell.) (Corrected 2026-08-16 by DEV on a `ledger` claim, pin line only, second correction this date: R128 moved test_core 653 -> 657 and test_upload_integrity 207 -> 218, so the total was 1015 and the per-suite pins at that date were 657 / 56 / 218 / 9 / 75.) (Corrected 2026-08-16 by DEV on a `ledger` claim, pin line only: R129 + R36 F6m(1) moved test_upload_integrity 182 -> 207 and the total 975 -> 1000, so the per-suite pins at that date were 653 / 56 / 207 / 9 / 75. Measured the same session: `python -m unittest` run by hand does NOT put `.pylibs` on `sys.path` the way `audit.py` and `env_probe.py` do, so the per-suite fallback in this line fails with `scipy MILP backend unavailable: No module named 'scipy'` -- 21 errors in test_showdown and 8 of 9 golden-replay tests missing -- until you `export PYTHONPATH=$PWD/.pylibs`. That is an environment gap, not a red gate, and it is the first thing to check when the fallback goes red while `audit.py --terse` is clean.) (Corrected 2026-08-15 by DEV on a `ledger` claim, pin line only, third correction of the R114/false-signal/R116 sequence: R114+R67 moved test_upload_integrity 168 -> 182 and the total 928 -> 942, the false-signal batch moved test_core 621 -> 640 and test_showdown 55 -> 56 for 962, and R116 moved test_core 640 -> 653 for 975. The per-suite numbers quoted further down this line are stale by the same three moves; the pins at that date were 653 / 56 / 182 / 9 / 75 and the dict in `tools/audit.py` is the source of truth for all of them.) **A KILLED MACRO IS NOT A RED GATE (merged 2026-08-22 by ARCHIVE from a 2026-08-15 DEV fragment; this half of that fragment survives and the other half does not).** A macro kill exits with NO VERDICT. At session start the instruction is to block on a FAILING suite; a killed macro means "run the gate per-unit", never "the gate is red", and nothing else in this line says so. The fragment's other half — that the macro fits and the "does not fit" note was stale — is SUPERSEDED and is not merged: it measured a 942-test tree at 76-84s and was itself falsified within the same session by a fourth run killed at 178s, with per-suite timings 2.4x slower on an unchanged tree forty minutes later. R152 then re-measured the current tree at 257.5s of test time and replaced the caveat outright. Both readings are kept because together they are the finding: the suite total is close enough to the ceiling that MACHINE LOAD, not a fixed boundary, decides whether any one-call attempt survives, which is why the supported path is `--gate-run` / `--gate-report` and not a chunk plan. If the audit fails on pins or inventory only while the suites pass in full, proceed and flag; never repair infrastructure mid-slate. The source of truth for the count is `EXPECTED_SUITE_COUNTS` in `tools/audit.py`, a PER-SUITE dict since R62 that the total is summed from (`EXPECTED_TEST_COUNT` still exists but is now only that sum); when this line and that dict disagree the dict wins and this line is stale. Read the state word the audit prints before touching any pin: only `grew` is a stale pin, while `shortfall`, `skipped_in_place` and `absent` are LOST COVERAGE and name the precondition to stage. (Corrected 2026-08-12 by DEV on a `ledger` claim, pin line only: this line read `782` while the dict summed to 871 — 570 core, 55 showdown, 162 upload_integrity, 9 golden_replay, 75 paste_lineups — so by its own rule it had been stale across four moves: 782 -> 792 -> 829 (R104/R45/R105/R54, the Showdown batch) -> 850 (R62/R85) -> 871 (R96, upload_integrity 141 -> 162). Earlier correction, 2026-08-08: the line read `25 modules  602 tests` against a constant of 742, then 745, 758, 773. The chunk boundaries below were measured against the 602-test tree and are staler still; re-measure before trusting them.) **R62, landed 2026-08-10: the paste suite has no staging precondition any more.** Anywhere this ledger tells a session to stage `data/slates/2026-07-29/` or `data/slates/2026-07-30/` to run the suite, that instruction is retired: both salary files are vendored and tracked at `tests/fixtures/slates/`, eight of the suite's nine `skipUnless` guards are deleted (the ninth is a Showdown guard already satisfied by a tracked fixture), the suite's `SUITE_PRECONDITIONS` entry is gone, and a shortfall in `tests.test_paste_lineups` is now genuinely unexplained. Measured while landing it: a tracked-files-only checkout previously ran `Ran 75 ... OK (skipped=62)`, the pin matching exactly while 62 of 75 tests asserted nothing. **Sandbox caveat (R152, 2026-08-18, replacing the 2026-08-03 measurement):** the suites exceed a 45s tool call and `tests.test_core` alone needs ~89s, so the gate runs across calls: `python tools/audit.py --gate-run` until GATE COMPLETE, then `--gate-report --terse`. CLAUDE.md's session-start step 2 carries the procedure and the exit codes. The hand-measured class boundaries that stood here ("the first 33 classes run 205 tests in ~20s") are RETIRED, not updated: they were taken against the 594-test tree, they had already gone stale twice, and the environment they assume is not stable within an hour on this VM. Do not background the macro -- the log comes back empty, which reads like a silent pass, and the killed process strands the next commit on a zero-byte `.git/index.lock` (R109). (Corrected 2026-08-13 by DEV on a `ledger` claim, pin line only: R69 moved test_core 582 -> 594 and the total 889 -> 901. Measured the same session: the single-line `--run-tests` macro does NOT fit this sandbox's per-call ceiling — it was killed twice at ~178s and once at ~935s in the background with no output — so the per-suite fallback in this very line is the path that produced the evidence, suite by suite, each at its pin. A background `nohup` run is not a workaround: it survives the call but never lands its output.) (Corrected 2026-08-12 by DEV on a `ledger` claim, pin line only, third correction this date: R46 round 2 moved test_upload_integrity 162 -> 168 and the total 883 -> 889.) (Corrected 2026-08-12 by DEV on a `ledger` claim, pin line only, second correction this date: R60 then R110 moved test_core 570 -> 582 and the total 871 -> 883. The 2026-08-12 correction earlier in this line describes the 782 -> 871 history and is left as written.) **R42(a), landed 2026-08-03: `env_probe.py` and `audit.py` now check `.pylibs` before anything else runs.** The repo vendors a working scipy 1.15.3 in `.pylibs/`; both tools put it on `sys.path` and report `env warm (vendored at ...)` / a clean dependency PASS with zero installs, zero network -- this replaces the prior workaround note (manually export `PYTHONPATH=$PWD/.pylibs` when `--terse` reported missing scipy) with the tools doing it themselves. `python tools/env_probe.py --install` still runs the pinned install for a genuinely empty `.pylibs` and empty site-packages. (Corrected 2026-07-24: this line still carried the pre-restructure claude.ai macro, naming a `/mnt/project` mount, a `/home/claude/work` copy, and a `project_audit.py` that do not exist in the v3.0.0-pre layout, plus stale counts. It is the mandated session-start read, so every session began by running a command that could not work.)
...
4a. Platoon reference (F17, 2026-07-27): `data/reference/fangraphs_platoon_lineups.json` is now aged against the SLATE, not against its own `collected_date`. Past 7 days it raises a pool blocker when a TBD team is being filled from it; `build_slate.py` tiers that blocker SOFT, so it prints and the build ships. It was 27 days old on 2026-07-27. Refresh it before leaning on projected orders. Refreshed 2026-08-05 by browser-session fetch (FanGraphs 403s scripted pulls; `tools/fetch_fangraphs_platoon.py --from-dir` parses, `--fetch` is refused); the refresh bought the batting ORDER, top-4 slots especially, not the roster (A-036). **STILL STALE as of 2026-08-22, and the refresh path is now gated (merged by ARCHIVE from a BUILD fragment, slate 2026-08-22 `1335_3g`).** The canonical file is untouched since 2026-08-05, 17.7 days at that build, for all 30 teams. That build refreshed WASHINGTON ONLY and did it BUILD-SCOPED: it copied `data/reference/` to `data/slates/2026-08-22/reference/`, patched the copy, and pointed `build_slate.py --reference-dir` at it, because `data/reference/` is ARCHIVE's write set. No later slate inherits that refresh. Two things bound any canonical refresh from here. FanGraphs RosterResource has put the **`Projected` stat set behind membership**, which is the set this file's global `stat_set` claims and the set `tools/fetch_fangraphs_platoon.py` assumes from a bare URL; a logged-out browser session reaches only the free `Current Year` (Go-To Starting Lineup) table, same vs_RHP/vs_LHP shape, different meaning. The WSH entry in that build-scoped copy is labelled `Current Year` in its own `stat_set` and `collected_via` rather than overwriting the file's global label, which is still accurate for the other 29 teams. So a genuine canonical refresh needs a FanGraphs session logged into a member account, and until one exists every `--fetch`/`--from-dir` refresh silently downgrades the stat set. Open for Ben.
...
### 3.20 The 2026-08-22 mined-data review: 108 contests, the shape series regraded, and five facts the repo already holds and never reads (2026-08-22)

Written by ARCHIVE after mining A-038..A-043 (108 contests across six slate dates, 2026-08-08 to 2026-08-13, pulled by Ben on
2026-08-20; 67 Classic, 41 Showdown; 94 full coverage, 14 `standings_only`). The archive goes from 271 to 379 mined records. Working
is `tools/_scratch_archive0822/` for the pass and is swept with it; every number below is reproducible from `data/archive/`.
Everything here is an observed outcome or a deterministic descriptive statistic, conditioned on contest type and field size per
Section 2. None of it is ROI, a win rate, a cash rate, or a probability claim, and none of it auto-applies.

Shape families below are read off `entries[].stack_pattern`, which is the RAW pattern (`5-2-1`, `5-3`, `4-1-1-1-1`). Earlier reviews
collapsed some of these; where a number is not comparable to 3.16-3.18 the text says so.

**The 5-2-1 lift is positive again, and 3.18's "lift moves opposite to field share" reading does not survive.** Tranche: +6.2pp
[+5.0, +7.4] top-decile lift on 30.3% field share, 46 Classic contests at 40+ entries, 69,113 entries. The four tranches now read
+3.1 on 25.1%, +0.2 on 29.7%, +13.6 on 21.6%, +6.2 on 30.3%. The first three were monotone inverse and 3.18 recorded that; the
fourth is the HIGHEST field share in the series carrying the SECOND-HIGHEST lift, which breaks it. Read the inverse relation as a
three-point coincidence, now falsified, and read the shape itself as durably positive: combined archive +4.6pp [+3.9, +5.3] on
27.7% share, 172 contests, 181,616 entries. On the win line 5-2-1 took 9 of 38 tranche wins (23.7%) on a 31.6% share — at-share
where the top decile is positive, which is the distinction the decile measure exists to make.

**5-3 is a distinct positive shape that no prior review separated out.** +3.1pp [+2.1, +4.1] tranche, +1.6pp [+1.1, +2.2] combined
archive, at 16-18% field share. It is the field's second-largest family and it is the family we build MOST. Earlier reviews folded
it into a generic five-primary bucket; entered provisional as its own family.

**`<=3-primary` is negative a FOURTH consecutive tranche and firm stands:** -9.3pp [-10.1, -8.6] tranche, -7.2pp [-7.8, -6.7]
combined archive. **4-2-x provisional also stands**, small and negative: -1.0pp [-1.6, -0.4] tranche, -0.9pp [-1.3, -0.5] combined.

**Our own mix reversed the drift 3.17 and 3.18 recorded, and opened one specific gap.** 112 own Classic entries: 5-3 29.5%, 4-4
22.3%, 4-1-1-1-1 19.6%, 4-2-1-1 13.4%, 4-3-1 7.1%, 5-1-1-1 1.8%, `<=3-primary` 1.8%. The `<=3-primary` series now reads 7.0% ->
21.5% -> 26.3% -> **1.8%**, and five-primary 5% -> **31.3%**. The three-tranche drift into the confirmed-negative family stopped
between 2026-08-06 and 2026-08-08, before any of the R135-R141 leverage work landed. What replaced it is the gap: **we built 5-2-1
ZERO times in 112 Classic entries**, against a 30.3% field share and the strongest measured lift on the board. Our concentration
went to 4-4 (22.3% of ours against a 3.2% field share, +0.5pp [+0.0, +1.0]) and 4-1-1-1-1 (19.6% of ours against 1.9%, -0.5pp
[-0.8, -0.2]) — two shapes we build at six to ten times the field's rate for no measured edge. This is the R37 input; the direction
of the ask has changed from "stop drifting into `<=3-primary`" to "the drift is fixed, now explain the zero."

**Duplication, and the Classic side is no longer clean.** Share of entries sitting in a duplicated lineup, median by bucket:
Classic <=150 0.0% (n=48), 151-500 4.5% (n=11), >500 17.6% (n=8); Showdown <=150 9.1% (n=26), 151-500 23.1% (n=13), >500 47.1%
(n=2). Own entries duplicated by the field: Showdown 13/102, **Classic 15/112** — 3.17 and 3.18 both measured the Classic side at
or near zero (4/93, then 0/29), so this is the first tranche where our Classic lineups are duplicated at the Showdown rate.

**Showdown captain anatomy, on the captain-share denominator.** Winner's captain was captained by a median 23.9% of the field
against a captain-table median of 8.0%; the top-owned captain won 15 of 41 (37%). Our own captains ran a 16.1% median (85 own
Showdown entries). 3.18 measured this on `%Drafted` (winner 14.8 vs field 13.0) and this measures `captain_share_pct`; the two are
different denominators over the same table and this is NOT a regrade of that line. On this denominator winners captain more
concentrated than the table's middle, not less.

**Winner chalk sits at the field.** Winner cumulative-ownership delta against the field mean, median: Classic -0.6 pts (46
contests at 40+), Showdown -0.9 pts (20). Neither 3.17's +4.9 Classic-satellite figure nor 3.18's -23.9 counter-tranche; this
tranche is flat and the conditioning that produced both earlier readings (family and slate size) is what the pooled number here
hides. Recorded, not acted on.

**Money is still unmeasured, and the cause is now specific.** Every one of the 89 contests with own results carries
`entry_fee`, `winnings_total` and `paid_places` NULL. `data/reference/dk_contest_paid_places.json`, the parked 2026-07-29
entry-history export, covers 0 of these 108 contests. Nineteen contests carry no own results at all (six on 08-09, six on 08-12,
seven on 08-13) because their deliveries have no `upload_manifest.json` row, which is the same unmanifested-delivery population
`awaiting_standings.py` reports at 31 files across 13 dates. 214 own entries matched across the 89, median finish percentile
Classic 47.2 / Showdown 35.1, two rank-1 finishes (193564659 on 08-11, 193663596 on 08-13), ten top-3.

---

The rest of this section is the greenfield pass: five facts the repo already holds in a file it trusts, found by enumerating every
column of a raw standings export and every key `field_miner` emits rather than by auditing any algorithm. Where a finding lands on
an open board item it is EVIDENCE toward that item's number and says so; nothing here is filed as a new discovery that already has
one.

**(1) `player_table[].fpts` is already retained, and R118's step (1) is already done.** R118 says the archive step strips
`fpts_by_norm` and budgets an idempotent re-mine of the whole archive to recover it. The strip is real (`field_miner.py:1982`
drops `own_by_norm` and `fpts_by_norm` from the written JSON) and the recovery work is not needed: both stripped maps are pure
functions of `player_table`, which IS retained, and the miner builds them from exactly that list at `field_miner.py:703-710`.
Measured across the archive: 379 mined records, 29,921 player-contest rows, **100% carrying a non-null `fpts`**, 5,926 distinct
(slate_date, player) realized-scoring observations over 26 slate dates. Verified rather than asserted: on twelve contests chosen
for having internal position-row disagreements, the `{player -> FPTS}` map rebuilt from the archived `player_table` is IDENTICAL to
the map rebuilt by parsing the raw standings CSV, 12 of 12, zero mismatches. Two caveats a replay tool must carry. 1,076 of 29,921
rows (3.6%) are one player holding two different `fpts` values across roster-position rows inside a single contest, and the
engine's collapse is FIRST-WINS over the ordered list, so a reconstruction has to be too or it will silently disagree. And 125
(slate_date, player) pairs disagree across contests on the same date, so `slate_date` alone is not the key — the draftgroup is.
**Evidence toward R118. It removes step (1) and the backfill; step (2), `tools/replay_portfolio.py`, is the whole of the remaining
item.**

**(2) The false-signal family grew a sibling four days after the batch closed.** The 2026-08-15 batch's stated theme is that the
underlying computation was correct while the line reporting it pointed at a key that did not exist. `_low_owned_hitter_count`
(`optimizer_v3.py:3035`) counts hitters with `Ownership_Tier == 'Low'`. **No writer anywhere in the tree ever sets
`Ownership_Tier` to anything but `"Mid"`** — `slate_intake_manager.py:1353`, `execution_pipeline.py:3070`, `:3384`, `:3661` are
the complete set. R154 (landed 2026-08-19) added the real low-owned test, on `Projected_Ownership_Pct` against
`DEFAULT_LOW_OWNED_THRESHOLD_PCT = 10.0`, and left the old diagnostic standing beside it. Direct check, five hitters carrying a
4.0% prior: the R154 constraint counts 5, `_low_owned_hitter_count` reports 0. So a build that satisfies a `min_low_owned_hitters`
floor reports zero low-owned hitters in the same record. The constraint is correct; the counter beside it is the batch's shape,
arriving after the batch.

**(3) R10 is sharper than "the field-pressure term was inert", and the sharper form is provable.** `compute_field_pressure_score`
is `0.70*meta_overlap + 0.035*ownership_total + 0.75*salary_band_penalty + 0.40*high_owned_one_offs + 0.35*(primary>=5)`. With
`Ownership_Tier` pinned to `"Mid"` and no production writer of `Projected_Ownership_Pct`, `ownership_total` is exactly 120.0 for
every 10-man Classic lineup, so that term contributes a constant **4.20 to every candidate** — rank-neutral by construction, which
is a stronger statement than inert, and one that no amount of reading the score's output would reveal. `high_owned_one_offs`
requires `Ownership_Tier == 'High'` and is therefore identically 0. **Two of the five terms are dead**; the score's stated job
(penalising duplicate-prone constructions) is carried entirely by meta-overlap, the salary band, and the primary-stack size. The
downstream consequence is the same shape as (2): `has_duplication_signal = bool(meta_lineup_ids) or ownership_sum_pct > 0`, and
120.0 > 0 always, so the flag is permanently True and `duplication_risk_proxy` is never None. **Evidence toward R10**, confirming
in the current tree that R10's premise still holds and naming which two terms carry it.

**(4) The salary resolver's team-coverage floor is a false NEGATIVE on small slates, and it fires on the tier the code calls
authoritative.** `MIN_AUTO_TEAM_COVERAGE = 0.80`, computed as `drafted_teams / salary_teams` — a property of the CONTEST's field
breadth, not of the salary file's fitness. On the 1507_3g slate (A-038, six teams), six contests share one manifest row, one
`run_id`, and one `DKSalaries.csv`. Four resolved `full` at a 100% player join. Two — 193662864 (23 entries) and 193707894 (**142
entries**, so this is not a tiny-field artifact) — were declined with the message `best candidate DKSalaries.csv joined 100% with
67% team coverage` and archived `standings_only`, losing their salary, stack and cheap-count tables. Their fields touched 4 of the
6 teams. On a six-team file the metric can only take the values 4/6 = 0.667 and 5/6 = 0.833; there is nothing between fail and
pass. The gate runs on the MANIFEST tier, which `manifest_salary_candidates`' own docstring describes as "the authoritative tier
and it exists because scoring cannot find it" — the manifest knows the answer by construction and the answer is then scored and
discarded. 3.19's tier table reported 25 tier-None contests across 267 without separating this cause from a genuinely absent file;
the other 12 `standings_only` contests in THIS tranche are the honest case (join 0%, no salary file for that slate exists).

**(5) Two runbook mechanisms have never run once, and a third stopped after its first slate.** (a) **Zero `contest_<id>.json`
files exist anywhere under `data/archive/`** — Job 1 step 2's contest-page trio has never been captured, for any contest, in the
history of the archive. That is why `paid_places` is null on all 89 own-results records in this tranche and why
`posture_allocator.classify_tier` returns UNRESOLVED for every one of them. There are now TWO mechanisms for that one fact — the
per-contest JSON the runbook mandates and the `--paid-places-from` export the miner reads — and neither has data for a contest
mined since 2026-07-28. (b) `ownership_rows_<slate_date>.csv`, which Section 5 step 1 describes as accumulating per slate, exists
exactly once, `data/archive/2026-06-29/ownership_rows_2026-06-29.csv`, from the A-001 seed. (c) `extract_inbox_zips.py` never
moves a zip out of the inbox — there is no `shutil.move`, `os.replace` or `rename` in it, and `data/standings/processed_zips/`
holds 127 files that someone moved by hand. That manual step IS the mechanism of R44: skip it once and the next extraction
refills the inbox with contests already archived. **Evidence toward R44**, naming the specific missing call rather than the
symptom. Sixty zips were moved by hand again this pass.

**Registry reconciliation, and a permanent three-contest gap with a named cause.** After the pass: 375 `contests_mined`, 17,769
users, 3,074 users at 5+ contests (3.18's "regular" threshold), 6.8 MB. The archive holds 378 distinct contest ids across 379
mined files — 192464820 is still double-foldered under 07-18 and 07-19 exactly as 3.16 recorded. Zero contests in the registry are
absent from the archive. **Three are in the archive and absent from the registry: 191787184, 191787186, 191823035**, all
2026-06-29, all schema `0.3-review`, all carrying both a contest id and a winning entry id, so R74(a)'s legitimate decline does
not explain them. The cause is structural: `tools/rebuild_registry.py` globs `contest-standings-*.csv`, and
`data/archive/2026-06-29/` holds three mined records and NO standings CSV — the A-001 seed tranche's raw exports were never
retained under that name. The export is 54 days old and unrecoverable, so 3.19's exact-reconciliation invariant now reads 375 =
378 - 3 and will keep reading that unless the rebuild also reads mined records.

**Mining cost is O(n^2) in the archive, and it is now the binding constraint on a large tranche.** Profiled one mine on the mount:
39.4s wall, of which **27.6s is `default_salary_candidates()`** re-globbing and `stat`-ing ~1,000 paths — an identical list on
every mine, and computed unconditionally even when the manifest tier is going to answer without it — and **10.4s is a full
rewrite of the 6.8 MB registry**, which grows with every mine. Memoizing the candidate list for the duration of one process, a
behaviour-preserving change verified against an uncached call, took the 108-contest pass from ~39s to ~12-22s per contest.
Recorded as a measurement; the fix is DEV's and is filed.

**The predict-then-grade loop has 16 ungraded predictions and cannot run.** `outputs/2026-08-17` through `outputs/2026-08-22` hold
16 `ownership_pred_*.json` files. `data/archive/` holds **zero** standings for any slate date from 2026-08-14 onward, so runbook
Job 1 step 9 has nothing to grade against for any of them. Recorded as an absence rather than skipped, per that step. The pull
list stands at 142 contests across 8 settled slate dates (2026-08-14 through 2026-08-21) plus 14 unsettled on 2026-08-22;
2026-08-14's 35 contests are the oldest and, on the age evidence in this tranche — five 08-08 contests came back zero-byte on a
twelve-day-old pull while eleven-day-old exports came back populated in the 2026-07-28 backfill — they are the ones closest to
going empty.
...
## A-038 — 2026-08-13 — 26 contests, 4 slates (1310_6g, 1507_3g, 2207_2g Classic; NYY @ SEA Showdown)

Mined 2026-08-22 (ARCHIVE), part of the 108-contest 08-08..08-13 tranche pulled on 2026-08-20. Field sizes 22 to 5,460. Full
coverage except two 1507_3g contests, which the auto-resolver declined on TEAM COVERAGE and not on join rate: the manifest's own
`DKSalaries.csv` joined 100% of their player references but their fields touched only 4 of the slate's 6 teams (67%, under the 0.80
floor), so the same salary file that gave the other four contests full coverage was thrown away for these two. That is a false
negative in the resolver rather than a missing file; see 3.20. The seven Showdown contests (NYY @ SEA, Fried/Gilbert) carry no
manifest row, so no own entries could be harvested for them. 43 own entries matched across 19 contests, median finish percentile
48.7, one rank-1 (193663596). Fees, winnings and paid_places null throughout: the DK entry-history export still ends 2026-07-28.
Two BUILD fragments from this slate date are merged below the decompositions.
...
#### BUILD note — 2026-08-13 1507_3g — SP-pair repetition was the binding floor, not exposure
...
#### BUILD note — 2026-08-13 1507_3g — operator override of the same-game SP-pair exclusion
...
## A-039 — 2026-08-12 — 28 contests, 4 slates (2210_2g Classic; 1610_1g_sd, 2210_1g_sd Showdown; one unmanifested Classic slate)

Mined 2026-08-22 (ARCHIVE). Field sizes 20 to 1,783. Six Classic contests (chalk SP Foster Griffin / Framber Valdez / David
Peterson) carry no manifest row and no staged salary file, so they mine at the honest `standings_only` tier and contribute no own
results; the other 22 are full coverage. 65 own entries matched across 22 contests, median finish percentile 33.4, no top-3. Fees,
winnings and paid_places null throughout.
...
## A-040 — 2026-08-11 — 19 contests, 3 slates (1840_3g, 1905_12g Classic; 1905_1g_sd Showdown)

Mined 2026-08-22 (ARCHIVE). Full coverage on every contest and a manifest row on every contest, the cleanest date in the tranche.
Field sizes 21 to 47,562 — the 47,562-entry contest is the largest field in the whole archive. 33 own entries matched across all 19
contests, median finish percentile 36.3, one rank-1 (193564659). Fees, winnings and paid_places null throughout.
...
#### Full-field decomposition — contest 193565124 (field_miner 0.5-review; coverage full; deterministic review proxy / observed outcome; not ROI, win rate, or a probability claim; never auto-applied)

- Verification: parse structurally sound; DK %Drafted agrees with the lineup recompute

- Entries 21 (20 complete lineups); winning score 60.03; multi-entry contest: False.
- Duplication: 19 distinct lineups; 10.0% of entries sat in a duplicated lineup; max copies 2; the winning lineup had 1 copy. Copies histogram: {1: 18, 2: 1}.
- Salary usage: 45.0% of entries within $100 of the cap. Salary-left bins: {'<= 0': 7, '101-300': 4, '> 1500': 2, '1-100': 2, '701-1500': 1, '301-700': 4}.
- Max-stack histogram: {3: 4, 4: 9, 5: 7}.
- Captain field share (top): Bryan Woo 50.0% CPT / 71.43% rostered, Ryan Weathers 15.0% CPT / 57.14% rostered, Spencer Jones 5.0% CPT / 19.05% rostered, Julio Rodriguez 5.0% CPT / 38.1% rostered.
- Winning entry captained Bryan Woo (71.43% rostered, 50.0% of captains); top-owned captain: yes.
- **Self vs field**: 1 own entries; best rank 2/21 (95.24th pct), median 95.24th pct; best 59.63 pts against a winning 60.03; 0 own lineup(s) duplicated by the field (max 1 copies); neither fee nor winnings supplied, so no net line for this contest. Observed outcomes, never a graded prediction.
- Chalk (top-5 %Drafted): Bryan Woo 71.43%, Ryan Weathers 57.14%, Brendan Donovan 38.1%, Julio Rodriguez 38.1%, Austin Wells 33.33%.
- Diagnostics: salary join 100.0% of complete entries fully joined; ownership recompute max diff 0.0 pts; parse OK; DK %Drafted agrees.
...
## A-041 — 2026-08-10 — 5 contests, 1 slate (2138_6g Classic)

Mined 2026-08-22 (ARCHIVE). Full coverage, manifest rows present, field sizes 23 to 142. 5 own entries, median finish percentile
38.0. Fees, winnings and paid_places null throughout.
...
## A-042 — 2026-08-09 — 17 contests, 3 slates (1605_3g Classic; 2020_1g_sd Showdown; one unmanifested Classic slate)

Mined 2026-08-22 (ARCHIVE). Field sizes 23 to 237. Six Classic contests (chalk SP Jacob Misiorowski at 74.3%) carry no manifest
row and no staged salary file and mine at `standings_only`. Three contests on this date were dead zero-byte pulls and are recorded
unrecoverable (193451396, 193451397, 193497117). 35 own entries matched across 11 contests, median finish percentile 66.1 — the
strongest date in the tranche — with three top-3 finishes. Fees, winnings and paid_places null throughout.
...
#### Full-field decomposition — contest 193444992 (field_miner 0.5-review; coverage standings_only; deterministic review proxy / observed outcome; not ROI, win rate, or a probability claim; never auto-applied)

- Verification: parse structurally sound; DK %Drafted agrees with the lineup recompute

- Entries 23 (23 complete lineups); winning score 132.45; multi-entry contest: False.
- Duplication: 23 distinct lineups; 0.0% of entries sat in a duplicated lineup; max copies 1; the winning lineup had 1 copy. Copies histogram: {1: 23}.
- Salary-usage and stack tables unavailable at this coverage tier (no salary file); re-run with --auto-salary if this slate's DKSalaries.csv surfaces, in runs/<run_id>/inputs/ via the date's upload manifest or staged under data/slates/<date>/. A delivered DKEntries_*.csv is NOT a salary source.
- SP-pair field share (top): Jacob Misiorowski/Kumar Rocker 21.7%, Jacob Misiorowski/Matthew Boyd 13.0%, Davis Martin/Jacob Misiorowski 13.0%, Connor Prielipp/Joey Cantillo 8.7%.
- Chalk (top-5 %Drafted): Jacob Misiorowski 73.91%, Kumar Rocker 39.13%, Nick Loftin 34.78%, Alec Burleson 26.09%, Jackson Chourio 26.09%.
- Diagnostics: salary join n/a (standings_only); ownership recompute max diff 0.0 pts; parse OK; DK %Drafted agrees.
...
## A-043 — 2026-08-08 — 13 contests, 3 slates (1505_3g, 1910_9g Classic; 1910_1g_sd Showdown)

Mined 2026-08-22 (ARCHIVE). Full coverage and manifest rows on every contest; field sizes 18 to 171. Five contests on this date
were dead zero-byte pulls and are recorded unrecoverable (193405185, 193405226, 193452362, 193458420, 193461246); DK's export had
aged out by the 2026-08-20 pull, twelve days after settlement. 33 own entries matched across all 13 contests, median finish
percentile 59.4, three top-3 finishes. Fees, winnings and paid_places null throughout.
