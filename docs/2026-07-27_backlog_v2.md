...
previously ran 75 with 62 of them skipped. **R96 did NOT land and is BLOCKED on
Ben.** Its step 1 investigation completed and tripped the entry's own stop
condition: six paths in three structural classes, plus a sixth unrecorded file
in `outputs/2026-08-06/` produced by a path that is not in the tree. Two
decisions gate steps 2-4 and both are Ben's — the disposition of
`build_showdown_theses.py` (close it or delete it in favor of `run_showdown`,
per R31(e)) and the rename policy for `preserve_prior_slate`. Read R96's table
before picking either. **R110 is new**, filed below in Workstream 6 next to R31:
...
3. **R40 — verify one-seat satellite profile routing** (S). Two live
...
4. **R96 — a delivery can exist outside the manifest** (P1, M; find the path
...
pieces. R17 and R106 are contest-truth edges of the same boundary. R108 is
...
### R96. A delivery can exist outside the manifest, so every manifest-driven tool is blind to it (P1, M — step 1 LANDED, steps 2-4 BLOCKED on a scope decision) | new 2026-08-08, from ARCHIVE fragment `2026-08-08_ARCHIVE_1910-4g-unmanifested-delivery.md`; absorbs the unnumbered R36 finding
...
- **STEP 1 LANDED 2026-08-10** (DEV, engine claim `engine_2026-08-11`; the
  session's record is in `CHANGELOG.md` under 2026-08-10). Steps 2-4 are NOT
  done and did not start, because the enumeration tripped this entry's own stop
  condition: it found SIX paths in three structural classes, not the two or
  three a single fix closes. Writing into `outputs/<date>/` there are exactly
  three write sites and one rename site — `execution_pipeline.py:3479`
  (`mirror_to_outputs`), `late_swap.py:816-817`, `showdown.write_showdown_entries`
  (:688, two callers), and `build_slate.preserve_prior_slate` (:420) — and they
  reach the unrecorded state six ways:

  | # | Path | Mechanism | Lands as |
  |---|---|---|---|
  | P1 | `mirror_to_outputs` → `_record_upload_manifest` returns False | the recorder swallows every exception (`except Exception: return False`); the build prints `MANIFEST NOT RECORDED`, sets `manifest_recorded=False`, and still certifies | normal, uploadable name |
  | P2 | `late_swap.py` promotion refused | R29(2)'s window, exactly as predicted: file written at :817, `return 3` at :836, `record_delivery` at :842 never runs | normal, uploadable name |
  | P3 | `late_swap.py` `record_delivery` raises | caught at :853, printed, `return 0` — a SUCCESS exit with no row | normal, uploadable name |
  | P4 | `build_slate.py` Showdown `record_delivery` raises | caught at :1980, printed to stderr, the build continues into the brief | normal, uploadable name |
  | P5 | `build_showdown_theses.py --out` | no `record_delivery` anywhere in the file; the destination is operator-chosen, so pointing `--out` at `outputs/` is unrecorded BY CONSTRUCTION, every run | operator-chosen name |
  | P6 | `preserve_prior_slate` rename | renames `outputs/<date>/DKEntries<suffix>.csv` aside for a different draftgroup, which ORPHANS an existing manifest row rather than failing to write one | tag-suffixed, uploadable name |

  Three classes, because the fix differs per class: P1/P3/P4 are "the write
  succeeded and the record raised", which one write-then-name helper closes
  identically; P2 is "control left the function between the write and the
  record", which that helper does NOT close because nothing calls its second
  half; P5 and P6 are "the record is never attempted" and "the record is
  orphaned after the fact", which are not write-path bugs at all.

- **Two facts from the investigation that change the scope, both measured, not
  inferred.** First, `tools/awaiting_standings.py` contains zero references to
  the manifest — it harvests Contest IDs off `outputs/*/DKEntries*.csv` and never
  cross-checks a row — so step 3 is greenfield and, worse, the scanner currently
  ENROLLS contests from unrecorded files without saying they are unrecorded.
  Second, `outputs/2026-08-06/` holds a SIXTH unrecorded file nobody had counted:
  `DKEntries_showdown_2140_1g_sd_cptcap6.csv`, 23792 bytes, sha256 `68ee815fcb57…`,
  content-distinct from the recorded `DKEntries_showdown_2140_1g_sd.csv`
  (`8650744968c6…`). No writer in the tree produces a `_cptcap6` name, so it
  arrived by operator copy or by P5. That is a real filled delivery file, in the
  exact directory this entry was filed from, produced by a path that is not in
  the code at all.

- **The decision that blocks steps 2-4, and it is Ben's.** P5 is
  `build_showdown_theses.py`, which R31(e) already records as fail-open, in no
  test, eval, or doc, and whose remedy that entry offers as "exit-3-on-failure
  gating plus a smoke test **or delete it in favor of `run_showdown`**". Closing
  P5 means first deciding whether it is a supported delivery path; deleting a
  tracked script is not a DEV call. And P6 is a policy question, not a bug fix: a
  rename that orphans a row can either update the row or refuse the rename, and
  those behave differently for a BUILD mid-slate. Closing P1-P4 and leaving P5-P6
  is precisely the half-close this entry warned about, because step 3's reverse
  check would then fire on P5/P6 output while the write paths stayed open, and a
  reported condition on a path believed closed reads as a false positive. **Do not
  start steps 2-4 without the P5 disposition and the P6 policy.**
...
### R110. `claim.py`'s own remediation advice is not a runnable command (P2, XS) | new 2026-08-10, found at this session's claim release and reproduced

- **What:** `_claim_name` (`tools/claim.py`) appends today's UTC date to whatever it is given, unless the name already starts with `slate_`. So `release` takes the BARE resource — `python tools/claim.py release engine` — while `check` and `sweep` print the DATED directory name, and `_incomplete_note` (`tools/claim.py:141-144`) interpolates that dated name straight into the command it instructs the operator to run. Reproduced: `python tools/claim.py release ledger_2026-08-09_r37_pin_2026-08-09` prints `ERROR  no claim at claims/ledger_2026-08-09_r37_pin_2026-08-09_2026-08-11` and exits 3. The same asymmetry on the other end explains the doubled directory names in `claims/`: `engine_2026-08-10_2026-08-10` is what `take` produces when handed a name that already carries its date.
- **Why it matters despite being XS:** that note is the fix R102 shipped so a hand-written `RELEASED` marker gets completed properly, and it is the single instruction a session in exactly that state will copy. It fails, so the session is left with a claim reading HELD and no working way to clear it — which is the situation that produces a hand-written marker in the first place. Seven claims sit HELD-with-a-marker today, counted off `claim.py check`: `engine_2026-08-04_full_audit`, `engine_2026-08-10_2026-08-10`, `ledger_2026-08-09_r37_pin_2026-08-09`, `ledger_2026-08-09_r37_pin_b_2026-08-09`, `slate_2026-07-29_sealad_sd`, `slate_2026-08-01_stltor_sd`, `slate_2026-08-03_7g`. That is the shape of this loop running seven times. All seven are stale and therefore Ben's to arbitrate, so this entry does not touch them. Same family as R31: a coordination tool whose false or unusable signal teaches the operator to work around the mechanism.
- **Fix:** make the round trip closed rather than picking a side. `_claim_name` should be idempotent — if the resource already ends in `_YYYY-MM-DD`, do not append another — so both the bare and the printed name resolve to the same directory, and every command the tool prints is copy-pasteable by construction. Done when: `release <dated-name>` and `release <bare-name>` both release the same claim, `take` cannot mint a doubled name, and a test asserts the exact string `_incomplete_note` emits resolves to an existing claim.
- **Sequencing:** rides R31. It is the same file, the same argument-handling surface, and R31(a)'s landed half is what emits the broken advice; fixing one without the other means touching `claim.py` twice. Do not fold in R31(d)'s engine-mutex decision, which is Ben's and unrelated.
