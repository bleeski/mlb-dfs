...
1. **R37 — primary-stack shape control** (decision, then M). The one
...
2. **R30(a) then (b)(c) — miner gaps** (S each). `--paid-places` unlocks R40's
...
4. **R40 — verify one-seat satellite profile routing** (S). Check before any
   profile weight moves; depends on nothing, informs R37 tuning.
5. **R31 — claim.py false HELD/STALE signals** (XS). Coordination-layer
   false-PASS family; bites every evening session.
6. **R38, R39 — net_to_date TOTAL line, CPT preservation** (XS/S).
   Opportunistic alongside any ARCHIVE-adjacent work.
7. **R17, R9b, R11, R12** opportunistically. **R13** stays parked on 3.14
   ...
   above drains. **R42** rides the same opportunistic tier (it bit the
   2026-08-01 session start). **R41** (Showdown certification) waits on its
   own sequencing decision against R37; do not start both. **R44**
   (`extract_inbox_zips.py` refills the inbox on contests already archived)
   joins the same tier; it costs an operator a manual diff every time it
   bites and nothing worse.
...
### R42. `env_probe --install` fails ungracefully with no disk headroom (P2, S) | new 2026-08-01, hit live at this session's start (GF spec F-22, narrow form)

- **What:** this session's start could not run the audit at all: the sandbox overlay was full, pip died with `[Errno 28] No space left on device`, and `env_probe --install` reported only "locked install failed (pip exit 1)". No headroom check before installing, no named shortfall, no reclaim path. The `.wheels`/`.pylibs` persistence design is right; the failure is that the installer discovers ENOSPC by dying mid-install.
- **Fix:** before installing, check free bytes on the overlay and the install target and name the shortfall ("need ~180 MB, have 12 MB"); on shortfall, perform the safe reclaims (pip cache purge, stale `.tmp/` contents) and retry once; consider pointing pip's TMPDIR at the persistent mount so the overlay stops paying the build cost. Done when: an ENOSPC session start produces one actionable line instead of a dead install, and the reclaim path recovers the measured common case.
- **Scope guard:** this is the narrow, evidenced core of GF-spec F-22. The rest of F-22 — pyproject, CI matrix, application images — stays rejected: the audit is the CI and the sandbox is not a production platform.
