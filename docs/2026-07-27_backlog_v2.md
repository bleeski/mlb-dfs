...
Pull rules, unchanged in substance and relocated from the old opportunistic
tier: when the queue drains or a live build trips an item, slot it in from the
top of the matching workstream below. Standing holds and riders, preserved:
R36's other accepted findings (F8, F7, F12, F1m, F3m, F6m, F14, F2) slot in
when a build trips one; **R85** is the natural rider on the queue's Showdown
batch if the session has room, since it is preflight-native and S; **R41**
still waits on its sequencing decision against R37, and note the batch plus
R54 shrink the gap R41 must close; **R87** waits on a deliberate golden-regen
decision and must not ride another change; **R81**'s design-pass trigger is
SATISFIED (two of R58/R59/R70 landed) and it is armed; **R13** stays parked on
ledger 3.14; **R31** carries R86 when it lands; **R9b, R11, R12** slot in
opportunistically. Priorities and efforts sit on each entry; the workstream
intros carry the ordering logic.
...
Queue position 1 is this stream's first four entries as one session (R104,
R45, R105, with R54 riding). R41 is the stream's destination and stays
decision-first; everything above it removes a piece of the gap it must
close.
...
The pool is the strategy surface the contract defends hardest. R60 is the
open P1. R81 is the armed design pass that retires a whole wrong-slate
family; R82's unknown-code report and R90's fixtures are its supporting
pieces. R17 and R106 are contest-truth edges of the same boundary.
...
What reaches outputs/ and what the record claims about it. R96 is the open
P1 and queue position 6. R36 is the standing adjudication container: its
...
from colliding. R62 is the P1 and queue position 4. R91 is the systematic
form of half this stream; R79 is its worked example list. R9 and R12 are the
context and process ideas; they slot in opportunistically.
...
- **Fix:** (a) is adopt-or-delete, deliberately: adopting means reading the
  ...
  browser-save step alone. Ben's call which. (b) add `_stage_repo*.tar.gz`
  to `.gitignore` in the same commit, or make the sync protocol's bridge
  step clean up after itself. Done when: `git status` at session start
  reports only genuinely foreign dirt.
