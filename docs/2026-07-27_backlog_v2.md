...
7. **The QA-hardening batch: R129 + R128 + R127 + R126 + R130 + R131**
   (2026-08-16, from BUILD's 2026-08-15 `2138_2g` fragment; Ben directed it in
   as the #1 item) — six entries off one slate that delivered clean, filed in
   the fragment's own suggested order with P6 moved ahead of P4. **R129** (P1,
   S, WS5) and **R128** (P2, XS, WS5) come first because both are cases where
   the RECORD blocked a portfolio the build had already produced: a measurably
   better variant became undeliverable because the manifest had marked it
   superseded inside the same session, and a correct file read as broken at T-5
   because 9 duplicate groups were 9 correct mirrors across two identical
   satellites. **R127** (P1, S, WS4) is the truthful-labels pair and the only
   one carrying a selection defect: an arm absent from the K-rate reference
   keeps the neutral 1.420 multiplier and outranked a better strikeout arm in 9
   of 19 lineups. **R126** (P1, S, WS2) is the objective itself, moved up one
   slot because it is the only item on this board that measures what Ben said
   he is optimizing for. **R130** (P2, S, WS4) is reporting only; its proposed
   remedy is declined on the entry. **R131** (P2, XS, WS6) is the smalls, one
   of which is Ben's rather than DEV's. The batch shares two surfaces with work
   already here — R129 with R98(3)'s restore remainder, R127 with the Tier 4
   FanGraphs decision — and both are cross-referenced rather than duplicated.
8. **R133** (P1, S + one decision, WS3; 2026-08-16, from BUILD's 2026-08-12
   ...
   what it did.
9. **R117, elevated 2026-08-14 evening** (P1, S, WS3) — the paste drops the
...
11. **The swap-rails batch: R66 + R68** (P2, S each, WS4/WS5) — silent
...
12. **R36 F3m + F6m** (P1, S each, WS5; elevated off the trip-only rule by
...
13. **The claim.py batch: R31(b)(c), R86 rides** (XS-S, WS6) — one file.
...
14. **R80(a) + R88** (XS + S, WS6) — the scrub fork that can persist a key
...
15. **R118** (P1, M, WS7/WS2) — retain the per-player FPTS map the miner
...
16. **R48 + R83** (S each, WS2) — the per-contest leverage table and the
...
17. **R10** (P1, M, WS2) — the funded modeling item, unblocked
...
18. **R115** (P1, M, WS4) — SP-pair coverage: the bank freezes at
...
19. **R132** (P1, M with a cheap S variant, WS4; 2026-08-16, from the 08-14
    ...
    `--deliver-always` to mirror.
20. **R124** (P1, M with an S first half that can land alone, WS5) — the
...
21. **R98(3) + (4)-tail** (P1, M, WS4) — derive the bank budget from entry
    ...
    before: the restore-an-earlier-run case (R129 owns the fix) and the
    past-limit-reference brief line.
22. **R81** (M, WS3; the design pass is S and ARMED) — the slate-fingerprint
    ...
    output.
23. **R36 Finding 10's floor + R84** (P1 M + S, WS5/WS4) — top-K compatible
...
**R11's stated gate has cleared (noted 2026-08-16, DEV).** Its own rewrite says
"Priority unchanged pending R114/R116/R117" and all three closed on 2026-08-15,
so the reason it was held no longer holds. It is also listed in Tier 6 as
operator-time class, which now contradicts the rewrite — two tiers, one item.
Not moved here, because moving it is a priority call and this was a merge pass;
the next session to touch it should resolve the double listing rather than
inherit it. Worth weighing when it does: the 08-15 QA fragment is R11's Tier 1
design running by hand for the second time in two days, and it found six items,
which is evidence for the deterministic tier rather than for the browser tier.
...
### R129. Manifest supersession is a one-way door inside a session, and there is no honest way back (P1, S) | new 2026-08-16, from BUILD's 2026-08-15 2138_2g fragment, second filing of the same defect

- **What:** BUILD built five variants exploring the apex/washout tradeoff on 2138_2g. Two of them (`v2`, `v3`) were better on BOTH axes than the first build. By the time one was picked the manifest had marked it superseded, and `preflight_upload.py:874` correctly hard-failed with `manifest marks this file superseded by outputs/2026-08-15/DKEntries_2138_2g.csv; do not upload it`. Verified 2026-08-16: the only escapes are `--no-manifest` (`preflight_upload.py:1508`), which waives the cross-check on a file that DOES have a record and so misrepresents it, or reproducing the build, which failed because the bank had drifted underneath (R130). There is no promote or re-promote tool in `tools/`. A measurably better portfolio became undeliverable for bookkeeping reasons.
- **Why:** the tool is right and the record is right; what is missing is a legal transition between two true states. This is the sharpest instance of Ben's 2026-08-15 framing that process should not stand in the way of a portfolio, and it is worse than it looks because it PENALIZES exploring alternatives — the more variants a session builds, the more certainly its best one is superseded. **This is the second independent filing of the same defect.** The 2026-08-14 floored-bank fragment reported it from the other direction (restoring an earlier certified run to the canonical delivered path makes preflight fail with `manifest marks this file superseded by <the same path>`) and that half was left unmerged until now; it is R98(3)'s restore remainder and this entry owns the fix for both. Two reports, two slates, two directions, one missing operation.
- **Fix:** an explicit re-promote path, `tools/promote_run.py --run-id <id>`, that copies the run's immutable `final/DKEntries.csv` to the delivery path and APPENDS a truthful new manifest row (`status: current`, `re_promoted_from: <run_id>`, the prior current row marked superseded by it). Honest record, no waiver, and the immutability contract is untouched because the run's own `final/` is the source. Three things to get right. The refusal message should name the `run_id` to restore from, since that is the operator's actual next move and today the message names only the path that beat it. The restore case wants a run-scoped filename so the canonical path is not the only place a certified file can live. And the new row has to survive R36 F6m's corrupt-manifest handling, which os.replaces a manifest that reads as empty and takes every prior supersession record with it — so this lands beside F6m or after it, never before.

### R128. Duplicate-lineup reporting has no contest context, so a correct portfolio reads as broken at T-5 (P2, XS) | new 2026-08-16, from BUILD's 2026-08-15 2138_2g fragment

- **What:** the preflight printed `duplicate lineup groups: 9` on the delivered 2138_2g file and BUILD nearly rejected a good portfolio over it. The truth, from an ad-hoc script BUILD had to write: contest 193774256 had 7 entries and 7 distinct lineups, contest 193774257 the same, and five single-entry contests held one each — total duplication WITHIN a contest, zero. The 9 groups were the same 7 lineups mirrored across two identical Pocket Cup satellites. Verified 2026-08-16: `preflight_upload.py:1426` computes `duplicate_lineup_groups` as a flat count over the whole file with no contest partition, and line 1655 prints it unqualified.
- **Why:** duplication across separate contests with separate prize pools is free and often correct construction; duplication inside one contest is waste. The tool conflates them and reports the union under a name that reads as a finding. It fires at T-5, on the preflight, which is the one surface the project treats as the pre-upload rule and the one place an operator has no time to go build a script. Cheap to fix and it prevents a correct file from looking broken at the worst possible moment.
- **Fix:** report `duplicates_within_contest` and `duplicates_across_contests` separately in `preflight_upload.py` and in the brief. Only the first is a finding; the second is information and should read like it. Entry IDs already carry their contest, so the partition needs no new input. Note for whoever takes it that the across-contest number is not noise worth suppressing — it is the number that says whether a satellite bank is being reused deliberately — so this is a split, not a filter.
