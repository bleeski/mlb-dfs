...
the pinned same-game pair). One new P3 tail, **R61-tail**, is filed in section 1.
...
3. **R101 — the swap discards the bank it just built** (P1, S; R47(a)'s
   remainder, filed 2026-08-09). `bank_cache.drop_stale_jobs` runs at the top of
   every `extend_bank` and discards every candidate whose conditions signature
   differs; `late_swap.py` computes `excludes` per entry, so each targeted slice
   throws away the previous slices' candidates. Measured on
   `outputs/2026-08-03/_swap1..9.log`: the cache reached 1,007 candidates while
   the joint solve was handed 8. This is a JOINT-SOLVE INPUT defect, so the
   portfolio it certifies is chosen from a bank two orders of magnitude smaller
   than the one the operator paid for, and nothing in the output says so. Ranked
   here because it is small and because it silently caps the quality of every
   scoped swap. **Decision needed before implementing:** either bucket stored
   candidates by conditions signature instead of discarding (the cache keeps
   several live sets and `as_candidates` serves the union that is valid for the
   requesting entry), or have the swap derive ONE exclude set for the whole
   solve. The second is smaller and changes swap semantics; the first is the
   honest fix and changes the cache's storage contract. Do not simply widen the
   budget — the candidates are discarded after they are built. Documented as
   operator procedure meanwhile in
...
4. **R62 — the audit gate becomes self-contained and skip-transparent**
   ...
   2026-08-04.
5. **R40 — verify one-seat satellite profile routing** (S). Two live
...
6. **R96 — a delivery can exist outside the manifest** (P1, M; find the path
...
list above drains): **R98's remainder** — (3) the budget derived from the job
list rather than from leftover clock, and (4)'s certified-brief half — repriced
M and dropped here 2026-08-08 evening because part (1) landed and the floor now
announces itself, which was the urgent property; **R99** (brief reports a
cross-check failure on an agreeing clock; XS, and it travels naturally with
R98(4) since both are brief-truth),
...
(opportunistic). Full reasoning in CHANGELOG.md.

### R102. A pinned same-game pitcher pair yields zero candidates at any budget (P2, S) | R47(b)'s remainder, filed 2026-08-09
