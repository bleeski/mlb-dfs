...
against a budget that had silently bottomed out on a hardcoded floor. R98 is
queued at position 8 for its two small parts; R99 is a reporting-truth XS in the
opportunistic tier. Both belong to the R51/R92 family — an honest report that
still produces a wrong decision — which is now the most frequently recurring
shape on this board.

Ordered, with the reason:

1. **R37 — primary-stack shape control** (decision, then M). Decision input
   REGRADED 2026-08-04 and moved AGAIN 2026-08-08 (read both bullets in R37
   before confirming any target). The floor-first reading is now three
   tranches deep and strengthening (<=3-primary negative a third consecutive
   time, -9.8pp, while our own share of it climbed 7.0% -> 21.5% -> 26.3%);
   the 08-04 "5-2-1 decayed" read did not survive, which restores the
   narrow-breadth floor-5 leg. Ben-gated; can land any time the decision is
   made. Nothing else on this list is blocked by it.
2. **R10 — satellite ownership + duplication prior** (M, unblocked
   ...
   grading. 3.18's regulars finding (a median 91.3% of Classic satellite
   fields are repeat opponents) is the strongest argument yet that this cell
   is modelable — and it is what R94's silent registry skip was starving.
3. **R61 (+R47) — late swap solves the subset, validates the whole file**
...
4. **R62 — the audit gate becomes self-contained and skip-transparent**
   ...
   2026-08-04.
5. **R63 — the approve=False contradiction** (P1, decision then S).
...
6. **R40 — verify one-seat satellite profile routing** (S). Two live
   positive cases: which profile built the two rank-1 satellite winners
   (A-032, A-034)? The 2026-08-08 cash-line anatomy complicates this item's
   premise and adds a method requirement — grade candidate profiles against
   BOTH the mean-delta and the percentile statistic, never either alone.
7. **R96 — a delivery can exist outside the manifest** (P1, M; find the path
    first). New 2026-08-08, and it is the fifth instance of a state R36
    already recorded and left unfixed, now across two contest families. It
    fails closed at the money boundary, so it is not P0; what it costs is
    supersession tracking, the sha256-at-upload check, and — measured on the
    1910_4g five — permanently degraded archive coverage. Ranked here rather
    than higher only because step (1) is an investigation.
8. **R98(1)(2) — the silent bank-budget floor, and the infeasibility hint
    that teaches the wrong lever** (P1, S). New 2026-08-08 from the 1910_9g
    build. Ranked last of the numbered queue but ahead of the opportunistic
    tier because parts (1) and (2) are small, independent of each other, and
    this is the fourth instance of the same misdiagnosis in eight days — the
    three earlier ones each generalised the wrong variable and were filed as
    "small multi-contest slate." Part (3), the real budgeting fix, is M and
    waits its turn; part (1) alone would have caught all four. Every Cowork
    build is exposed, because the 45s call ceiling is what drives
    `--max-seconds` low enough to hit the floor.

Opportunistic tier, unchanged rule (slot one in when a build trips it or the
list above drains): **R99** (brief reports a cross-check failure on an agreeing
clock; XS, and it travels naturally with R98(4) since both are brief-truth),
**R31** (+ new (c), WRITE_SETS omits CHANGELOG.md),
**R38** (bookkeeping honesty; R50 landed 2026-08-05, R39 landed 2026-08-08), **R17** (+ new precedence note), **R48, R44, R45, R42(b)**, and
...
- **Note 2026-08-05 (DEV): the regrade's feasibility gate is ANSWERED, and it clears. Reproducible via `tools/stack_shape_probe.py`.** The gate was "how much of the thin-slate <=3-primary share is feasibility (team cap x max_shared on 2-4 game slates) versus solver choice." Measured on five staged Classic salary files (3, 3, 4, 6, 8 games), forcing `stack_constraints={'team': T, 'min_size': N, 'max_size': 5}` for every team with >=5 rostered hitters:

  | slate | games | unconstrained primary | 5-primary feasible | floor-4 cost | floor-5 cost |
  |---|---|---|---|---|---|
  | 07-29 | 3 | 5 (5-2-1) | 6/6 teams | 0.00% | 0.00% |
  | 07-30 | 3 | 3 (3-2-2-1) | 6/6 | 0.20% | 0.43% |
  | 07-25 | 4 | 3 (3-2-1-1-1) | 8/8 | 0.17% | 0.76% |
  | 07-30 6g | 6 | 3 (3-2-1-1-1) | 12/12 | 0.76% | 1.55% |
  | 07-29 8g | 8 | 2 (2-2-1-1-1-1) | 16/16 | 0.33% | 0.87% |

  Four readings.
  (1) **It is CHOICE, not feasibility.** A 5-primary lineup is feasible on 100% of stackable teams at every width probed, thin slates included. Nothing in the constraint set is producing the <=3-primary share.
  (2) **The objective drifts AWAY from stacking as the slate widens** (primary 5 -> 3 -> 3 -> 3 -> 2). More teams means more cherry-picking, which is what a correlation-free mean-max solve should do. The thin-slate <=3 share is not a width artifact either; on the two 3-game files the unconstrained solve landed on 5 once and 3 once, so it is coin-flip incidental.
  (3) **A floor is cheap.** Floor-4 costs 0.00-0.76% of the unconstrained best; floor-5 costs 0.00-1.55%, worst case on the 6-game file. Set against the archive's ~8pp top-decile spread between shape families and the ~+/-1pp flatness at the paid line (3.16, unregraded), the floor buys top-end exposure at close to zero cost in projected points.
  (4) **The secondary may not need its own control, which shrinks the item.** Forcing floor-5 produces the shape the archive prefers for that archetype as a by-product of slate width: on the 6- and 8-game files the feasible set splits evenly 5-1-1-1 / 5-2-1 (6/6 and 8/8), and wide slates are where mini-MAX lives (5-1-1-1 strongest, +3.6pp); on 3-4 game files it is 5-2-1 four to six times out of six to eight, and thin slates are where the WTA satellites live (5-2-1 over-wins). 5-3, which the archive says is at-share everywhere, appears ONLY on the thin files (2 of 6, then 1 of 8) and never on 6 or 8 games, so a secondary cap earns its keep on thin slates or nowhere.
  Caveats that travel with these numbers: single-lineup feasibility only, so a PORTFOLIO floor still has to satisfy `max_primary_stack_exposure_pct` and `max_shared_players` across N entries; and with no Savant CSVs the Floor/Ceiling multipliers are uniform, so the cost column measures the constraint's salary-and-position price, not a variance-aware one. Pass the enrichment CSVs to the probe for the latter.

- **Recommendation on the table 2026-08-05 (DEV), awaiting Ben's decision.** Floor-first, keyed to `PAYOUT_BREADTH_BY_SHAPE` (which `_resolve_payout_breadth` already resolves per contest), because the archive's shape spread lives at the top end (~8pp) and is flat at the paid line (~+/-1pp):
  - **Universal baseline: `primary_stack_min_size = 4`, every posture.** Kills the one family with a firm negative (<=3-primary, -2.8pp [-3.8,-1.8], our share tripled to 21.5%) for <=0.76%. Breadth-independent, because the floor is nearly free at the cash end and positive at the top end.
  - **Breadth <= 0.02 (WTA, one-seat satellites, solo shots, mme_gpp/mini-MAX, single_entry_gpp): floor 5.** This is where the whole payout sits at rank 1 and where the top-decile spread is worth 1.55%.
  - **Breadth 0.10-0.22 (ticket_line/cut-line satellites, large_field_gpp): floor 4 plus a partial `min_five_stack_share_pct`,** not a hard 5. Part of the payout is at a cut line where shape does not separate, so buy the top-end exposure on a share of entries rather than all of them.
  - **Breadth >= 0.45 (cash, flat_cash): floor 4, no five-stack quota.** The archive has ZERO cash-game contests and no paid-places coverage since 2026-07-28, so "do nothing extra here" is the honest position, not a measured one.
  - **Cap 4-2-x rather than banning it.** It is -2.2pp and PROVISIONAL, it is 35.5% of our mix, and a floor of 4 does not touch it (4-2-2 and 4-2-1-1 both satisfy floor-4). Needs a secondary-size cap, sized after the floor lands so the two effects stay separable.
  - **Do not target 5-3, and note the `mme` posture's `stack_plan` currently reads `five_three_with_diversification`** while the archive has 5-3 at-share in every slice measured and the mini-MAX slice preferring the lone-5. That string and the data disagree.
  - **No contrarian push** (3.17 confirmed and strengthened), and leave the salary-left gap alone for now (winners leave more, we leave least; recorded, not acted on).
  Sequencing if accepted: land the floor at 4 everywhere first with relaxation counting, measure one tranche, then raise to 5 on the narrow-breadth postures. The 5-2-1-specific push stays the smaller second half and is the weakest-supported part, since its lift decayed to +0.2pp [-2.1,+2.5] as field share grew to 29.7%.

- **Decision input, THIRD tranche, 2026-08-08** (ARCHIVE fragment `2026-08-08_ARCHIVE_r37-decision-input-third-tranche.md`, consumed; ledger 3.18 and `ledger/2026-08-08_tranche31_analysis.md`). The 31-contest tranche moves the input again, and in the direction that makes the decision EASIER rather than harder. Three parts:
  - **The floor half strengthens, which is the half the recommendation above rests on.** `<=3-primary` is negative a THIRD consecutive tranche: -9.8pp [-18.0, -1.6] top-decile lift on n=8 conditioned contests. And our own mix kept drifting into it — own `<=3-primary` share 7.0% -> 21.5% -> **26.3%** across the three tranches, with 68% 4-primary and 5% 5-primary. Three tranches now point the same way, and the thing they point at is the thing we are doing more of.
  - **The 08-04 "5-2-1 decayed under crowding" read did NOT survive, and the correction is about method, not just sign.** Tranche lift came back at +13.6pp [5.6, 21.6] on a 21.6% field share, where the 08-04 tranche was +0.2pp on 29.7%. Across three tranches the lift has moved OPPOSITE to the shape's field share every time (+3.1 on 25.1%, +0.2 on 29.7%, +13.6 on 21.6%); combined archive +4.5pp [1.7, 7.3] on 99 contests, and 5-2-1 won 8 of the 12 shape-known Classic tranche contests. 3.16's win-line concentration is regraded **open -> provisional**. This does NOT reorder floor-first. What it says is that the second half is not dead either, and that **any mix push must watch the shape's CURRENT field share rather than assume a static lift** — a three-point record-only pattern, not a law.
  - **The thin-slate sub-question stands and is not answered by feasibility.** This tranche's own `<=3-primary` entries sat on 4-7 game slates, so slate width is not the explanation. That is consistent with the 2026-08-05 probe above, which already found the `<=3-primary` share to be solver CHOICE rather than a constraint artifact at every width probed.
  - **Net effect on the pending decision:** nothing here changes the recommended shape (universal floor 4, floor 5 at breadth <=0.02, partial five-stack share in the middle band, cap 4-2-x separately later). It strengthens the universal-floor leg, restores the narrow-breadth floor-5 leg that the 08-04 regrade had weakened, and adds one build requirement — if a five-stack quota lands, it should read the shape's current field share rather than a pinned lift constant. Deterministic review proxies and observed outcomes; nothing auto-applies.
...
**Note 2026-08-05 (DEV):** "route on `paid_places == 1` once R30(a) lands" is now UNBLOCKED on the code side. R30(a)'s tool half shipped the same day: a mined record carries `paid_places`, and `classify_tier` returns tier A with reason "winner-take-all (paid_places == 1)" instead of UNRESOLVED, pinned by test. What this item still needs is the archaeology it names — which profile the two rank-1 builds (A-032, A-034) actually scored on — and that reads runs/ and briefs, not the miner. The routing change itself can now be written against a real field rather than a dict nothing populates.
...
**Note 2026-08-08 (ARCHIVE fragment `2026-08-08_ARCHIVE_r40-cash-line-anatomy-input.md`, consumed): the first direct measurement of paid lines COMPLICATES this item's premise, and carries a statistic caution that changes how the archaeology should be graded.** This item is premised on "the floor blend has been pulling toward exactly the construction the archive says does not win seats." The first cash-line anatomy (100 contests with observed `paid_places`, vintage <=2026-07-28; ledger 3.18 and `ledger/2026-08-08_tranche31_analysis.md` sec. 3) does not support that premise cleanly:

- **In satellites the winner and the last seat look alike:** within-field chalk PERCENTILE ~31 (sub-median), looser salary, 4-primary median. The band just OUTSIDE the line is MORE 5-stacked and chalkier than the band that got paid. So on this measurement the min-cash shape and the seat-winning shape are not the opposites the premise assumes, and a floor blend pulling toward 4-primary sub-median chalk is pulling toward what actually got paid.
- **The caution, and it is not a reversal:** this does not contradict 3.17's "+4.9 cumulative-ownership over field MEAN". Mean-delta and percentile are different statistics and both can hold at once in a left-skewed chalk distribution. The consequence for this item is procedural: **grade candidate profiles against BOTH statistics, never either alone**, or the archaeology will read as a reversal of whichever one it omits.
- Read this before deciding the routing. Observed distributions; no build action derives from the fragment, and it does not by itself resolve which profile scored the two rank-1 builds — that archaeology is still the open work.
...
# Section 1 additions — 2026-08-08 ARCHIVE tranche (R93–R96)
...
### R96. A delivery can exist outside the manifest, so every manifest-driven tool is blind to it (P1, M — find the path first) | new 2026-08-08, from ARCHIVE fragment `2026-08-08_ARCHIVE_1910-4g-unmanifested-delivery.md`; absorbs the unnumbered R36 finding

- **What:** `outputs/2026-08-06/DKEntries_1910_4g.csv` exists with filled rows that were demonstrably entered (the standings prove it), and yet: no delivery record in `outputs/2026-08-06/upload_manifest.json`, no `runs/` directory in the 1910 window, and no staged salary in `data/slates/2026-08-06/` (the DKEntries template saved there belongs to the early slate and carries no salary block). Whatever path produced that file bypassed the manifest write entirely. Contests 193297994, 193297995, 193303619, 193344230, 193344231.
- **This is now a RECURRING state, which is why it gets a number.** R36's adjudication already recorded three files under `outputs/2026-07-29/` with no manifest row, and named R29(2) as a fourth path into the state (a promotion refused after the mirror returns but before `record_delivery`); it was explicitly "filed here, not fixed". The 1910_4g case is the Classic sibling of the 2026-08-05/06 BUILD fragments on Showdown manifest status, and the fifth instance overall. A finding that recurs across three months and two contest families is a defect in the delivery path, not an incident.
- **Why P1 and not P0:** default preflight hard-fails an unrecorded file, so the money boundary itself fails CLOSED and nothing invalid can certify on this path. What is actually lost is everything downstream of the manifest: the delivery sits outside supersession tracking, outside the sha256-at-upload check the multi-session contract requires Ben to perform, and outside every manifest-driven tool. Concretely, on the ARCHIVE side: own-entry harvest failed silently for those five contests (it needed an explicit `--my-entry-ids`), and because no salary source exists for that slate they can only ever be `standings_only` — the evidence is permanently degraded, not recoverable by a re-mine.
- **Fix, in order.** (1) FIND THE PATH: enumerate every writer of `outputs/<date>/DKEntries*.csv` and identify which ones can complete without `record_delivery`, R29(2)'s refused-promotion window included. This is the investigation half and it is why the item is M rather than S. (2) Close it: a delivery file either gets a manifest row or gets written as `DO_NOT_UPLOAD_*` (R36's suggestion, still the right one), so an unrecorded file is self-labelling instead of merely failing preflight later. (3) Add the reverse check to the archival side: a `DKEntries*.csv` in `outputs/<date>/` with no manifest row is a reportable condition, since ARCHIVE is where the consequence lands and it currently discovers this by hand. (4) Stage the salary file whenever a delivery is recorded, which is what makes the contest mineable at all and what R49's manifest-first resolution depends on. Done when: no path reaches a filled DKEntries file in `outputs/` without either a manifest row or a `DO_NOT_UPLOAD_` name, pinned by a test per path found in (1).

# Section 1 additions — 2026-08-08 BUILD tranche (R98–R99)
...
### R98. The bank budget bottoms out on a silent floor, and the infeasibility guidance then teaches control relaxation as the fix (P1, S then M) | new 2026-08-08, from BUILD fragment `2026-08-08_BUILD_thin-bank-drove-control-relaxation.md`

- **What, three linked parts.** (a) `build_slate.py:1453` derives the bank
  budget as `bank_time_budget_s=max(deadline - time.monotonic() - 6.0, 5.0)`,
  where `deadline = started + args.max_seconds` (`:2151`). Under the Cowork
  45s call ceiling the skill instructs `--max-seconds` in the low teens; by the
  time control reaches `:1453` most of that window is already spent on imports
  and pool construction, the subtraction goes negative, and the budget lands on
  the hardcoded `5.0` floor. Observed on 1910_9g with `--max-seconds 11`:
  `time_budget_s: 5.0`, `jobs_attempted: 47` of `jobs_total: 2592` (1.8%),
  `job_list_exhausted: false`, `total_candidates: 38` for 9 entries. Nothing in
  the brief or on stderr says the budget hit a floor rather than a computed
  value. (b) Against a 38-candidate bank the default portfolio controls are
  arithmetically impossible, so the build failed twice — first the
  `max_shared_players 6 vs inherent overlap floor 7` pre-check, then
  `contest_allocator.py:1886`'s "no single control is arithmetically binding
  against this bank, so the interaction of the active controls is". Both
  messages are true. Neither mentions the bank, although the allocator holds
  the candidate set `E` and the bank report carries `job_list_exhausted`.
  (c) `build_slate.py:1497`'s hint names exactly one remedy — rerun with
  `--controls-override` set to at least the named floors — and characterises it
  as "not a strategy or player-pool change". That framing is correct for
  `max_shared_players` reaching an engine-named floor and wrong for the
  exposure caps, which have no named floor and are a portfolio strategy
  decision. The operator followed the hint, raised the three exposure caps from
  0.35/0.43 to 0.56 and `max_sp_pair_repetition` from 1 to 2, and certified.
- **Why P1.** This is the R51/R92 family seen from the strategy side: every
  individual report was honest and the composite steered wrong. The delivered
  portfolio's concentration is the shape of an under-explored search presented
  as a considered set of caps, and the run record cannot distinguish the two —
  `controls_override_applied` records what was relaxed, never that the bank
  was 1.8% explored when it was relaxed. It degrades lineup quality silently in
  exactly CLAUDE.md's sense, and it does so by the same logic the pool rule
  already forbids: an infrastructure limit (the sandbox call ceiling) reached
  through to change strategy, invisibly, in the certified output. Note this is
  the fourth recorded instance of the same two-round pattern (2026-08-01,
  08-03, 08-04, 08-08); the three earlier ones all ran short bank budgets too
  and were each filed as "small multi-contest slate," so the operator-side note
  had generalised the wrong variable three times running before this pass
  corrected it. Not P0: nothing invalid certified, preflight and all three
  gates were sound, and the file uploaded clean.
- **Fix, in order.** (1) Make the floor audible — when
  `bank_time_budget_s` resolves to the `5.0` floor rather than the computed
  remainder, say so on stderr and record it in `solve.bank` as an explicit
  `budget_floored: true`, because a silent floor is the whole reason (a) is
  invisible. S, and it is the one part worth landing alone. (2) Teach the
  infeasibility path to check bank exhaustion first: when the joint MILP proves
  infeasible AND `job_list_exhausted` is false, both `contest_allocator.py:1886`
  and `build_slate.py:1497` should name bank growth via the exit-10 resume as
  the FIRST remedy and `--controls-override` as the second, and should separate
  raising `max_shared_players` to an engine-named floor from raising an
  exposure cap that has none. (3) Derive the bank budget from entry count and
  job-list size rather than from whatever is left of `--max-seconds`; five
  seconds for 2592 jobs and 9 entries could never have supported the defaults,
  and the resume loop existed to cover exactly this but was never signalled as
  the answer. M, and it is where the item stops being cosmetic. (4) Record
  `jobs_attempted/jobs_total` and `job_list_exhausted` alongside
  `controls_override_applied` in the brief, so a reader can tell a deliberate
  cap from a starved one. Done when: a build that relaxes an exposure cap
  against an unexhausted job list says so in the brief, and the T-5 reading can
  distinguish the two cases without opening the bank block.
- **Adjacent, do not merge.** R87's closing note ("Classic defers diversity to
  ...
  in-solver overlap work; R98 is the diagnostic and budgeting half and does not
  need R87's golden regen. R73(f) budgets `solver_probe`, a different entry
  point. R92 is the same "report that cannot inform" family on the bank's
  warning lines.
