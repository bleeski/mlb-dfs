...
from colliding. R62's honesty halves landed 2026-08-10 and the stream no
longer carries a P1: what is left of R62 is an XS vendoring, and R109 is the
...
### R62. Vendor the two gitignored salary files the paste suite reads (P2, XS) | audit 2026-08-04; the audit-honesty and skip-guard halves LANDED 2026-08-10

- **Status:** this entry now holds ONLY the deferred fixture vendoring. The
  two halves that made the gate honest landed 2026-08-10 and their record is
  in `CHANGELOG.md` under that date: the audit now runs each suite in its own
  subprocess against a per-suite pin (`EXPECTED_SUITE_COUNTS`), parses
  `skipped=`, prints what is abnormal on the PASS line, and its pin advice is
  conditional — only a `grew` verdict says "stale pin", while `shortfall`,
  `skipped_in_place` and `absent` name the suite, name the precondition, and
  say do not lower the pin. The five unguarded paste classes carry skip
  guards, and `TestDataDependenciesAreVendoredOrGuardedTests` walks tests/ so
  the sixth cannot be written unguarded. That meta-test is what makes the
  vendoring below safe to land later: whichever paths get vendored, the rest
  stay guarded.
- **What is left:** two files, both gitignored under `data/slates/*/`, both
  read by `test_paste_lineups`:
  `data/slates/2026-07-29/DKSalaries.csv` (28 KB) and
  `data/slates/2026-07-30/DKSalaries_1910_6g.csv` (56 KB). Vendor frozen
  copies under `tests/fixtures/slates/` (the enrichment-fixture precedent) and
  repoint the two constants. Done when: a tracked-files-only checkout runs the
  paste suite with zero skips.
- **Why it is now P2 and XS, and why the sequencing note should be re-read:**
  the 2026-08-04 filing sized this at ~2.4MB across
  `data/slates/{2026-07-25,-29,-30}` plus `data/archive/2026-06-03`, and
  deferred it behind the GH PAT because committing 2.4MB to history is
  permanent. Both halves of that premise were checked on 2026-08-10 and both
  are wrong. `data/archive/2026-06-03/` is TRACKED — all 7 files, 476 KB, in
  history since the golden replay was written — so it was never part of the
  vendoring problem, and its disappearance on 2026-08-10 was a partial COPY of
  the tree through the tarball bridge, not a gitignore consequence. And no
  test references `data/slates/2026-07-25/` at all any more. What actually
  remains is 84 KB in two files, which is smaller than several fixtures
  already tracked. **The GH PAT dependency therefore no longer holds on size,
  and Ben should re-decide the sequencing rather than inherit it.** The one
  argument that survives is the original one: until the container can clone,
  a container run starts from a tarball of the working tree anyway, so the
  vendoring buys less than it looks like it does.
- **The defect this closed, for the record:** five classes in
  `test_paste_lineups` read those two files with no guard, so a
  tracked-files-only checkout ERRORED on them while 29 more paste tests
  silently skipped behind `skipUnless(SALARY.exists())`, and CLAUDE.md's
  mandated PASS line was unsatisfiable off this machine. Reconfirmed the day
  it landed: the R101 dev cycle ran against a matched-dependency copy off this
  machine, got `Ran 783` where the pin said 792, the nine missing were
  `test_golden_replay` in full and silent, and the audit's advice read "stale
  pin". Two unittest behaviours made that invisible, both measured while
  fixing it: a class-level `skipUnless` keeps its tests in `Ran` AND adds them
  to `skipped`, while a `setUpClass` raising SkipTest removes the whole class
  from `Ran` and adds exactly ONE to `skipped` — so nine lost tests reported
  as "skipped=1", and only a per-suite pin can tell those two apart.
