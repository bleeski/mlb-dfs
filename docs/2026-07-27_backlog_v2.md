...
7. **The QA-hardening batch: R128 + R127 + R126 + R130 + R131** (2026-08-16,
   ...
   closed with it. **R128** (P2, XS, WS5) is now the batch's head and the other
   case where the RECORD blocked a portfolio the build had already produced: a
   correct file read as broken at T-5 because 9 duplicate groups were 9 correct
   mirrors across two identical satellites. It stays cheap and it stays on the
   preflight surface R129 just touched, so take it next while that file is
   fresh. **R127** (P1, S, WS4) is the truthful-labels pair and the only
...
12. **R36 F3m + F6m-remainder** (P1, S + M, WS5; elevated off the trip-only
