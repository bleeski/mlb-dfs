...
4. **R60 — a posted partial side can leave posted starters out of the pool**
   (P1, S; Workstream 3, reproduced 2026-08-04). This inherits R96's slot.
   Intake truth is the one class of defect that certifies cleanly and is
   invisible in the output, which is the same reason R96 outranked its
   neighbours; unlike R96 it needs no decision first.
...
- **What, two halves, one of them not code.** (a) `tools/sync_check.py` reads
  ...
  contained in `main`, 33 commits behind. A plain `git clone` therefore checks
