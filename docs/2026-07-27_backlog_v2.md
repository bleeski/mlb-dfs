...
*The Showdown intake-and-bookkeeping batch (R104 + R45 + R105 + R54) LANDED
2026-08-10 and its four entries migrated to CHANGELOG.md. Two things it leaves
behind. **R85 was the named rider and was not pulled** — the batch used the
session, and R85 is unchanged and still preflight-native, no network, S, so it
is the cheapest thing on the board to slot in. **R108 is new**, filed below in
Workstream 3: the DK `Starting` token vocabulary is still parsed in two
modules, now with named constants pinned in sync by test rather than two tuple
literals, and the consolidation R104's scope note asked for is deferred rather
than done. R41's gap is measurably smaller: honest relaxation counts and honest
manifest statuses both exist now, so what remains is the sequencing and
relaxation-policy decision, not the bookkeeping.*
