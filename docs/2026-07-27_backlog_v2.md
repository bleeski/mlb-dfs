...
1. **R60 — CLOSED 2026-08-12**, migrated to CHANGELOG.md. Both halves landed
   with the partial-side rule written into the build contract. **R69 is now
   the head of this tier**, and it inherits R60's argument directly: (a) is
   R60's class arriving by schema drift.
2. **R69** (P2, S, WS3) — the intake fail-open trio. (a) alone returns the F1
   shelved-stars defect as a data condition, which is R60's class arriving by
   schema drift; (b) un-ages the platoon reference under a `block` policy;
   (c) exclusions that never block. Take this next: it is the same file and
   the same surface R60 just left, and R60's seeding is the precedent its
   (a) fix follows.
3. **R70** (P2, S, WS3) — stage_slate stages the wrong draftgroup silently on
   the ROUTINE two-classic-slates layout, and `--checkpoint` prints a clock
   that is always None. The 2026-08-09 wrong-entries burn says the class is
   live; the clock is the T-schedule's one budgeting instrument.
...
7. **The claim.py batch: R31(b)(c) + R110, R86 rides** (XS-S, WS6) — one
   file. The broken-release loop has run seven times, and (c) under-reports
   foreign dirt on the one file the contract most wants serialized. R31(d)
   stays Ben's and is NOT in this batch.
...
- **Part (b), found the same session, same root:** `take` does not clear a stale `RELEASED` marker when it re-takes a claim directory that was released earlier. Release-then-retake within one session is a legitimate sequence (the R96 session released the engine claim at 02:28Z, then re-took it at 13:18Z when Ben's decisions unblocked steps 2-4), and afterwards `check` reported the live claim as `HELD ... RELEASED marker present but owner.json still holds it` — the R102 diagnostic firing on a claim that is correctly held by the session reading it. `take` should remove the marker when it writes a fresh `owner.json`, since `owner.json` is authoritative and a marker from a previous take is stale by construction. Same one-line class as (a).
- **Sequencing:** rides R31. It is the same file, the same argument-handling surface, and R31(a)'s landed half is what emits the broken advice; fixing one without the other means touching `claim.py` twice. Do not fold in R31(d)'s engine-mutex decision, which is Ben's — though note that R31(d) acquired a real cost on 2026-08-11 (see its 2026-08-11 note) and is worth re-reading before deciding this one's priority.
