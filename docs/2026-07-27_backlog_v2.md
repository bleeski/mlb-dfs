...
1. **The Showdown intake-and-bookkeeping batch: R104 + R45 + R105, with R54
   riding** (P1, S each; one DEV session). Three defects, three-to-five live
   burns between 2026-08-03 and 2026-08-09, every one worked around by hand at
   T-minus: a PO opener is rosterable in Classic and a declared starter in
   Showdown against Ben's stated policy (R104); the sanctioned paste tool
   refuses every Showdown salary file, so R32's paste-primacy contract is void
   on a family Ben enters routinely and the workaround is a hand-written feed
   at T-18 (R45); and every Showdown delivery fails `verify_export` red on a
   vocabulary disagreement, which trains the operator to ignore the one FAIL
   that will someday be real (R105). R54 rides because its own entry says
   "schedule it with, or ahead of, any Showdown-heavy week" and this is one:
   four counted-relaxation lies on the same surfaces this batch touches. The
   batch is also the cheapest half of R41's remaining gap — certified Showdown
   needs honest relaxation counts and honest manifest statuses first.
2. **R37(2) — the five-stack half** (M, staged; stage 1 landed 2026-08-09).
   Ben decided the shape on 2026-08-09 and stage 1 shipped: the universal
   `primary_stack_min_size = 4` floor with counted relaxations. **The next
   move is a MEASUREMENT, not a build.** Floor 5 at breadth <= 0.02 and the
   five-stack quota are both gated on one tranche archived with the floor-4
   portfolio in it, by the same decision, so the floor's effect stays
   separable from what follows it. Until that tranche exists this item is
   waiting on ARCHIVE and on slates being played, not on a DEV slot. The
   4-2-x secondary cap is sized after that.
3. **R10 — satellite ownership + duplication prior** (M, unblocked
...
4. **R62 — the audit gate becomes self-contained and skip-transparent**
   ...
   2026-08-04. Reconfirmed 2026-08-10 by the R101 commit, which developed
   against a matched-dependency copy of the tree off this machine and got
   `Ran 783` where the pin says 792: the nine missing are `test_golden_replay`
   in full, silently, and the audit's own warning read "stale pin".
...
**R106** (`--postures` accepts a JSON object or `--postures-file`, so a
ticket count reaches the resolver without a per-slate driver script; slot it
in with the next satellite-heavy build, and note it feeds R40's evidence),
**R103** (the pinned same-game pitcher pair, renumbered from R102),
**R31** (+ new (c), WRITE_SETS omits CHANGELOG.md; + new (d), the engine
mutex is nominal — decision-first, because making it real starts blocking
sessions that today proceed),
...
### R45. `tools/lineups_from_paste.py` can't resolve a Showdown salary file (P1, S; was P2) | new 2026-08-03, found by BUILD mid-slate (SD@ARI Showdown, fragment consumed into this entry); elevated 2026-08-10
...
- **Elevated to P1 on 2026-08-10, on the recurrence record, not on a new mechanism.** Two more live burns since filing, both fragments consumed into this entry: 2026-08-05 STL@NYY (16 blockers, every hitter "matches 2 salary rows" where both rows are the same person, both SPs reported NOT IN DK POOL, feed hand-written at ~T-18) and 2026-08-09 HOU@SD (18 blockers, identical shape, feed hand-written off the UTIL rows — safe that night only because DK's own `Starting` column declared both full orders and both SPs, so nothing was inferred). Three burns in six days makes the original "Why P2" reasoning insufficient: pool construction is indeed unaffected, but R32 says the operator paste is the PRIMARY source, and on Showdown the sanctioned tool cannot produce a feed at all, so every Showdown build runs on a hand-built artifact with no provenance line at the most time-pressed minute. A primary intake that fails an entire contest family is a contract violation, not a degraded enrichment. The 08-09 fragment adds one fix detail the original entry lacked: resolve probables off the `Starting` column rather than Roster Position when the file is Showdown. Add the paste test against a Showdown salary fixture (CPT/UTIL double rows, suffixes, both probables); `test_paste_lineups` today covers Classic only. The greenfield spec's F-20 names the same two defects; its `UnderlyingPlayer`/`RosterVariant` type system is rejected as the vehicle — identity dedupe plus a fixture test is the whole job.
...
- **Note 2026-08-10 (from the consumed sync fragment):** the fixture set is ~2.4MB (`data/slates/{2026-07-25,-29,-30}` plus `data/archive/2026-06-03`), and committing it to history is permanent, so R102 deliberately did not. Sequence the vendoring after the GH PAT lands (Sync-tail below), because until the container can clone, a container run starts from a tarball of the working tree anyway and the vendoring buys less than it looks like it does. The R101 commit's own dev cycle reconfirmed the defect the same day: a matched-dependency copy off this machine ran 783 where the pin said 792, the nine missing were `test_golden_replay` in full, silently, and the audit's advice was "stale pin."
...
# Section 1 additions — 2026-08-10 greenfield-revision adjudication and fragment merge (R104–R106)
...
### R104. DK `Starting` token policy: PO is never startable, PLR is a decision point (P1, S-M) | new 2026-08-10, from BUILD fragment 2026-08-05 (slate 1905_7g); spec F-21 names the same defect

- **Ben's instruction, 2026-08-05, verbatim in substance:** "PLR means
  projected long reliever. When you see that designation, web-search to
  confirm, then decide whether to include or exclude that pitcher from the
  candidate set. We should never start someone with a PO designation."
- **What, three parts, all verified in tree 2026-08-10.** (a) Classic intake
  states the wrong meaning for PLR: `live_data_adapters.py:78-79` says PLR
  "is DK's generic listed-player tag and carries no role claim," which is
  false — it is a role claim (projected long reliever), and
  `showdown.py:131-133` documents the same token correctly in the same repo.
  Because Classic intake attaches no meaning to PLR, a PLR arm who is not the
  feed probable never enters the pool at all: live case DET Ty Madden
  (`Starting=PLR`, $5,800, DK 43755567) outside the pool on 2026-08-05 while
  DK's own ID allocation put him inside the declared-starter block. (b) A PO
  arm is rosterable in Classic: PO maps to `viable_bulk_or_alt_sp`
  (`live_data_adapters.py:1073-1074`), which sits in `ALLOWED_PITCHER_ROLES`,
  `OPTIONAL_SP_AUDIT_STATUSES`, and `ALLOWED_PITCHER_ROLES_FOR_GATE` — F17
  removed the opener from REQUIRED SP-pair coverage but left him rosterable,
  so the optimizer may put a one-or-two-inning arm in a P slot priced on a
  starter's workload and the build certifies. (c) Showdown is the sharper
  violation: `showdown.py:136` returns True from `_is_declared` for PO and
  `:150` stamps `Is_Declared_Starter = True`, so on a `declared_starters`
  basis a PO opener is a declared starter outright.
- **Why P1:** the failure is asymmetric. Excluding a PLR arm costs an option
  (and on 2026-08-05 it cost nothing — the portfolio took zero DET/SEA
  exposure unprompted). Rostering a PO arm costs a P slot on a certified
  build and nothing downstream flags it.
- **Fix, two different shapes because the tokens are different facts.**
  **PO — deterministic, no judgment:** bar PO-tagged arms from pitcher slots
  via a new non-rosterable role (`declared_opener`) rather than reusing
  `viable_bulk_or_alt_sp`, so the arm stays visible in the pool report;
  remove PO from `_is_declared` and `Is_Declared_Starter` in showdown.py;
  keep the existing warning text. Decide whether `viable_bulk_or_alt_sp`
  survives (PO is currently its only producer). **PLR — the engine surfaces,
  the operator decides,** because the confirm step is a web search, which is
  non-deterministic and must not live inside a replayable build: intake
  recognizes PLR and emits a named soft blocker per arm; `build_slate.py`
  grows a repeatable `--declare-pitcher <id>[=<role>]` flag plumbed to the
  `declared_pitchers` mapping `build_slate_pool` already accepts
  (`live_data_adapters.py:605`, honored at 1083-1097 — the engine side
  exists, only the CLI surface is missing); the brief records the declaration
  verbatim so the decision is a recorded input, not a hidden fetch.
- **Scope note, carried from the fragment:** the DK `Starting` vocabulary is
  parsed in two places with two token sets and two meanings; it probably
  belongs in one module the way team codes moved to `mlb_engine.team_codes`
  under R59/R82. Weigh that consolidation in this commit or file it as the
  follow-up. The spec's F-21 typed-role apparatus (`role_confidence`,
  `expected_batters_faced`, workload models) is rejected as the vehicle —
  that is Finding 15's projection stack, gated on R13; the policy fix needs
  none of it.

### R105. The Showdown manifest status `review_ready` is outside the manifest vocabulary, so every Showdown delivery fails verify_export red (P1, S) | new 2026-08-10, from BUILD fragments 2026-08-05 (STL@NYY) and 2026-08-06 (2140_1g_sd SD@ARI); spec F-16 names the same defect

- **What:** `upload_manifest.py:36` closes the status set at `("candidate",
  "upload_ready", "blocked", "acknowledged", "superseded")` and the comments
  at `:33-35` and `:140-141` say exactly who writes what. The Showdown export
  path instead stamps the manifest status from `preflight_upload.py:1078`'s
  VERDICT string (`review_ready`), so `tools/verify_export.py` fails the
  delivery: "manifest records status 'review_ready', which is not one of
  [...]; an unknown status reads as current everywhere that only filters out
  'superseded'." Reproduced on both fragments' builds (reported exit 2 on
  08-05 and exit 3 on 08-06 — record the discrepancy and pin the real code
  when fixing); every per-entry check passed on both files.
- **Why P1:** two writers of one field disagree on its allowed values, which
  is the condition `contest_shapes.py` exists to prevent for shapes. The
  named consequence is supersession blindness for Showdown deliveries, and
  the behavioral one is worse: a red FAIL on every clean Showdown export
  trains the operator to ignore the one FAIL that will someday be real. It is
  also a standing tax on R41 — certified Showdown needs honest manifest
  bookkeeping before gates mean anything.
- **Fix:** the 2026-08-06 fragment's second option is right — stop the
  Showdown manifest record from borrowing the preflight verdict and write
  `candidate`, as `build_slate.py`'s own export-path comment already says it
  should. The verdict and the status stay separate facts. Pin the manifest
  status of a Showdown delivery to the accepted vocabulary by test so the two
  writers cannot drift again. The spec's F-16 apparatus (four separate enums,
  generated language bindings, migrations from one schema) is rejected: one
  writer, one vocabulary, one test is the proportionate form. Adjacent, do
  not merge: R34-tail (the three `status: upload_ready` Showdown records from
  07-29 sitting wrong is Ben's history-rewrite call), and R96 (unmanifested
  deliveries are the other way a record escapes the vocabulary entirely).
