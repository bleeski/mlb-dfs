...
1. **R60** (P1, S, WS3) — a posted starter unrosterable while a bench bat is
   in the pool, reproduced. Intake truth is the one defect class that
   certifies cleanly and is invisible in the output, at one session of lift.
...
4. **The false-signal batch, one session: R112 + R103 + R99 + R92 + R71**
   (P2, XS-S each, WS4) — refusals and brief fields honest line-by-line that
   steer wrong: bank scarcity blamed on controls, `+0 targeted candidates`
   ...
   in solver and validator at once. Four builds since 08-01 burned on this
