...
rather than queued, one ordering-rule amendment, and two fragments RETAINED. The
list below runs 1 through 17, counted rather than inherited** (it was 15). No
gate was run: this session changed no code. `CLAUDE.md`,
`skills/generate-lineups/SKILL.md` and `docs/cowork_sync_protocol.md` are the
only non-board files touched and all three edits are prose; the two prose-pinning
tests that read `CLAUDE.md` assert the PRESENCE of strings this edit did not
remove.

***What the inbox held, since the count is the point.*** Fourteen files: twelve
untracked and TWO already committed and therefore invisible to `git status`,
which is the trap `docs/backlog_inbox/` keeps setting (22-vs-38 on 2026-08-23,
12-vs-14 here). One of the two committed ones is the 2026-08-09 satellite-archetype
decision, RETAINED by design and correctly labelled as such; the other is a
2026-09-06 DEV measurement filed hours before this session started. **List the
directory, never the diff.**

***The five new numbers.*** **R317** (W3, P1 for (b)): the platoon reference's
only automated refresh is 403'd by a urllib CLIENT FINGERPRINT where `curl`
returns 200 on the same URL in the same minute -- so `--fetch` is one transport
swap from working, not a network problem -- and a PARTIAL refresh stamps
`collected_date` today, silencing the 7-day staleness check for all thirty teams.
**R319** (W3, P2): `lineups_from_paste` mints a FALSE team-disagreement warning
when two DK rows share a normalized name (`:803` re-derives the team from a
name-keyed dict after `_resolve_one` already matched the right row), and reports
a rostered starter as `NOT IN DK POOL` on a first-name alias. **R320** (W4, P2):
`factors_inert` cannot see a HALF-inert composite, and the Showdown brief emits no
platoon counter at all, so two surfaces report a factor RESOLVED and are read as
APPLIED. **R321** (W4, P2): the bank stage is 91-98% of build wall clock, it is
57 to 383 independent solves executed one at a time, and no parallel primitive
exists anywhere in the repo. **R318** does not appear below because it SHIPPED
with this commit: the clock rule's two unreached doors, in CLAUDE.md's T-schedule
and SKILL.md.

***Seven riders, and two of them change what an existing entry means.*** R305
gains a THIRD sighting and it is the one the first two do not warn you about --
Classic-file-against-Classic-feed on a multi-draftgroup night, where nothing but
the game set separates the two files. R310 gains a second field instance, an
implementable warn predicate that needs no new data (APPG above the pool's 90th
percentile among hitters WHILE salary sits in the bottom quartile), and a measured
negative result on the obvious wrong fix. R87 is REPRICED with the measurement it
was filed without in August. R10 gains its substrate (29,921 player-rows, 379
contests, 26 dates, already on disk) and loses a stale premise. R123, R254 and
R125 take the rest.

***One entry falsified by its own named falsifier.*** **R125(a)** proposed
auto-declaring the `PLR` on a `PO`+`PLR` bullpen day and named its own falsifier:
"any bullpen-day where the PLR read is wrong." 2026-09-04 NYY is that day (PO Max
Fried $9,000 was the actual starter, PLR Will Warren $7,000 was not), and 28
archived Classic team-days say **PLR dearer on 13, PO dearer on 7, tied on 8** --
defensible on 13 of 28, wrong on 7, undefined on 8. Rewritten to "declare the
DEARER arm, surface on a tie", with the grading against realized `fpts` filed in
the smalls because the table measures DK's PRICING and not who took the ball.

***Four fragment claims corrected on merge, recorded because the board's value is
that its numbers survive re-reading.*** The PO/PLR fragment says "`PO` is
classified nowhere": FALSE, `DK_STARTING_OPENER_TOKENS` is at
`live_data_adapters.py:97` and R104 barred openers from pitcher slots
deliberately, so what is wrong is the REFUSAL TEXT and not the classification.
The ownership-substrate fragment says the prior's only grade is one archived grid:
stale, R306 step 1 shipped 2026-09-03 and grades on 297 contests. The first clock
fragment says both recorded instances overstated elapsed time so the overstating
direction is the dangerous one: its own sibling, filed the same night, is a third
instance in the other direction, so the shipped wording names no safe direction.
And the 2210_2g fragment's environment note ("`device_bash` still has no network")
is narrowed by R316 and by the 09-06 measurement: the device VM answered 200 from
five hosts.

***The ordering rule gains a fourth term, and R118 moves 7 -> 5. This is the one
call in this merge worth reversing if Ben disagrees.*** A DEV fragment argues that
R118 cannot win a head slot under the 2026-09-03 criteria as a matter of the
RULE rather than of scheduling: it builds no lineup and changes no delivered byte,
so criterion (1) ranks it below everything that touches a build and criterion (2)
ranks it below every XS and S, while criterion (3) protects it only from things
that depend on it and the 09-03 note states that nothing in slots 1-9 does. The
09-03 demotion note proves the mechanism in its own words -- *"six weeks of not
building it is evidence about how a head slot gets spent"* -- which enters
non-construction as evidence against construction. **Adjudicated rather than
handed back, per Ben's own criterion (4): the rule gains a fourth term --** *an
item that is the sole gate on a named tier is ranked by what it unblocks, not by
its own direct prize contribution* **-- and R118 moves to slot 5.** Not higher:
slots 1-4 are XS-S and every one of them cost a measured lock-window call on 09-03
or 09-04, and a lost slate is still a certain zero. Not lower: R302's start
condition is literally "R118 built and ten deliveries graded", all of Tier 2 runs
through it, and R37(2)'s two live bands are running against live slates today with
no counterfactual to compare them to. What the project currently cannot do,
verified absent this session: no delivered portfolio has ever been replayed
against an archived field, no construction policy has ever been A/B'd, and the
dual objective in CLAUDE.md has never been measured in either half. Reversing this
is a one-line edit to the slot number and a strike of the fourth term.

***Owed by Ben, and it is decaying.*** `CONTESTS_AWAITING_STANDINGS.md` read 121
contests across 9 slate dates on 2026-09-03 and **205 across 11 dates on
2026-09-05**. DK's export ages out some days after a contest settles and pulling
is a manual DraftKings download by the money-and-entry wall, so the fit set R10
needs is decaying oldest-first at roughly forty new contests a day against zero
pulls. No DEV session can supply this one. R206's entries-vs-dollars decision and
R13 are the other two minutes-of-decision items unchanged from the 09-03 note.

***Two fragments RETAINED, deliberately, and both say so in their own headers.***
The 2026-08-09 satellite-archetype decision stays until ARCHIVE writes its nine
rows into `data/reference/dk_contest_archetypes.csv`; it is the sole carrier of
that table and of its four cautions. `2026-09-04_BUILD_fangraphs-platoon-refresh-procedure.md`
is newly retained on the same precedent: it is the sole carrier of the step-0
staleness checker, the grid extraction script and the per-team HTML synthesis, a
session needs them TODAY, and it is consumed in the commit that ships R317(c),
at which point the procedure is one command. Its caveat 1 is already dead and
its superseded sibling was swept. The other twelve fragments are consumed and
deleted.*

*2026-09-04, DEV, claim `engine` (`engine_2026-09-04b`): **R294 SHIPPED
COMPLETE, all three parts, and migrated to CHANGELOG.md; slot 1 closes, so the
list below runs 1 through 15, counted rather than inherited** (it was 16). Gate
`PASS v2.26.0 28 modules 1796 tests` -> `PASS v2.26.0 28 modules 1810 tests`,
`tests.test_core` 1140 -> 1154, all five suites `clean`. **18 mutations written,
18 killed, 0 survivors** -- the first round produced three genuine survivors and
all three were the SAME fault in the test rather than in the code, so they are
recorded here as well as in the changelog: the fixture gave the protected entry
exactly one compatible candidate, which means the forced-coverage branch was
what passed the test and the per-entry reserve was never exercised. The fixture
now gives it two and asserts `forced_coverage_kept == 0`. A test that passes by
the wrong mechanism is the R300(a) shape.

***What the closing found that the filing did not.*** Three things, and the
first is why (a) was worth its slot. The prefilter starvation did not only relax
the stack floor: on the same repro it ALSO relaxed the engine-defaulted
`max_candidate_reuse` to no cap and delivered **one distinct lineup across both
entries**, which is maximum concentration on the portfolio-level washout axis
the dual objective names. A starved MILP degrades whichever control the ladders
reach for first, not the one the filing happened to name. Second, (b)'s "seven
solves" was close and not right: measured 4 on a 3-entry fixture and 5 on a
9-entry one, and the ceiling read off the rung functions is EIGHT, because the
reuse ladder takes one further step past the end of its own list. Third, (c)'s
subject was misidentified. Status 3 is UNREACHABLE through this call site (every
variable is bounded in [0,1]) and status 4 was not producible in a probe, so
neither is a field sighting -- but **status 0 with an `x` that fails the
incumbent verification** is reachable, was labelled "proven infeasible", and is
the member that matters. The label is fixed for all three.

***One rank change and two entries touched.*** **R36's Finding 10 (prefilter
starvation) is the root of (a) and is now FIXED at the mechanism**, so its owed
re-measure on the next live pinned-entry swap changes from "does this happen" to
"confirm it no longer does"; F10's line inside the R36 entry in Workstream 5 is
annotated rather than left describing a live defect. **R298(a) is unaffected in
substance and its read is now smaller**: it reads the three ladder relaxation
blocks that (a) used to mis-attribute, and those blocks are unchanged -- what
changed is that a relaxation appearing there is now genuinely the bank's or the
control's, so R298(a) becomes MORE worth doing, not less. Its entry says so.*
...
ordering rule.** Ten new numbers (R304-R313), six riders on existing entries,
three findings rejected with the reason, and one severity raised. The list below
runs 1 through 18, counted rather than inherited; it was twelve, and eight of the
ten new numbers earned a slot because they are cheap and they touch what wins.

***Ben's ordering instruction, 2026-09-03, and how it was applied.*** (1)
Prioritize what helps win a large prize, leverage above all. (2) Deprioritize
large technical lifts. (3) Respect technical dependencies. (4) For
impactful-but-hard and easy-but-marginal, adjudicate rather than hand the choice
back. That REPLACES the 09-02 rule as the tiebreaker without contradicting it: a
lost slate is still a certain zero, so the "legal file every slate" items keep
their rank on criterion (1) rather than on a separate axis. What changed is the
treatment of leverage. The project has, for the first time, a MEASURED leverage
finding sitting in an unconsumed fragment, and the cheapest half of it needs no
new code at all. That is the top of the list now.

***The three adjudications, stated so they can be reversed.*** **PROMOTED to
slots 1-2: R306, the captain ownership market.** Eight archived Showdown
contests say captain shares sum to 100% and person shares to 600%, that only
8-13 distinct captains appear per contest against roughly 20 rostered players,
that six of eight top captains are ARMS, and that Spearman between person-level
and captain-level ownership runs 0.33 to 0.85 with a 60%-rostered player landing
at 6% captain. Every Showdown leverage decision the project makes today aims at
the person-level number, because that is the only one the tool emits. Step 1 is
cheaper than anything else on this board: `ownership_pred.py grade` already
exists, `actuals_from_standings` already reads realized `%Drafted`, 53 Showdown
standings files already parse, and `grep -rn "Ownership prior grade" ledger/`
returns ZERO. One grade has ever been run and its result lives in a module
docstring. The constraint was never sample count; it is that nothing's loop calls
`grade`. **DEMOTED: R118 to slot 10 from slot 2.** It is still the instrument
everything strategic is gated on and it is still M, but nothing in slots 1-9
depends on it, criterion (2) is explicit about lift, and six weeks of not
building it is evidence about how a head slot gets spent. It keeps its whole
Tier 2 tail and the R302 start condition still names it. **DEMOTED and unchanged
in substance: R302 to slot 16.** The eleventh edition's Section 4 is a second,
independent statement of the same strangler design, arriving from a reviewer who
had not read the board; that is corroboration, not new information, and criterion
(2) is exactly the instruction to leave an L alone. Its §4.10 p95 table is
adopted as ACCEPTANCE CRITERIA on R302 rather than as work.

***The eleventh edition, adjudicated.*** Twenty-two of its thirty-five findings
are entries already on this board and were checked as such rather than re-filed:
D01/D02/D03/D17 = R297(a)(b)(c)(e); D04/D05/D06 = R294; D07-D10 = R295;
D14/D15/D16 = R298; D18 = R290(a); D20 and D28 = R303 and R36 Finding 6;
D21-D26 = R299; D27 = R216 + R217 + R218 + R180; D30 = R225 + R226; D32 = R41.
That ratio is the useful fact about the document: it audited `92cd423`, it
verified five earlier blockers CLOSED (R291, R292, R296, R290(c), R293, in its
own table and matching this board's closures), and its genuinely new material is
five findings, four of which the same day's BUILD sessions had already filed as
fragments from the field. An outside review that mostly reproduces the board is
a check on the board, and it is worth exactly that much. **REJECTED, with
reasons.** *D34 (the audited runtime lacks scipy)* is a measurement of the
auditor's own Windows host, not a repository defect: CLAUDE.md's R271(c) already
records that Ben's Windows Python has no scipy and that `.pylibs/` is a Linux
build, so `--run-tests` cannot pass there and the gate is the session's job. Its
one usable sliver, refusing to print PASS across mixed runtime identities, rides
R217. *D29 (ZIP resource limits)* was adjudicated and severity-reduced on this
board once already, under the outside spec's own D26; R44 owns that tool and no
re-file is warranted. *§4.10's p95 service levels* are targets for a system that
does not exist; recorded on R302, not filed as work. **MODIFIED.** *D11's claim
that both brief paths now write `declared_pitchers` is FALSE for Showdown*, and
the correction is the point of the finding: an AST-bounded count over
`run_showdown` (`build_slate.py:3203-3920`) returns ZERO occurrences of either
`declared_pitchers` or `declare_pitcher`, while the Classic paths do write it at
`:2692` and `:2744` (since R104, `7730190`). Both ends of the escape hatch are
out at once on the one geometry where the flag is broken, which is R304(d) and is
worse than what was filed. *D13 gets no number*: it is R285's subject, and ed11's
own remedy ("no regular expression over `errors[]` belongs in the final fix") is
R285's reader half stated better; merged as a rider carrying the 09-03 field
sighting. *D19's severity is RAISED* from R303's hygiene tail to P1 and promoted
to R313, because the mechanism is a legal-pool REDUCTION and CLAUDE.md's hard
guardrails name that as the forbidden move, in the same direction R289 was filed.

***What the fragments cost, since that is the argument for the top of the
list.*** Three of the eleven cost calls inside a lock window on 2026-09-03
alone: a supervisor that stopped on the remedy CLAUDE.md lists FIRST as
unattended (R285's rider), a referee that cross-checked a Classic six-game file
against a one-game Showdown feed and printed 73 false roster warnings (R305),
and a declaration flag inapplicable on Showdown whose only remaining move was to
reduce the legal pool, leaving 11 of 17 entries with no pitcher (R304). None of
the three is larger than S. That is the shape criterion (4) asks about, and it
...
(`test_core` 1095 -> 1127, `grew`; the other four unchanged and all `ok`; the
module count moved on its own with the new engine module); golden replay 9
tests unmoved; no delivered byte changes without `--deliver-by`.**

**The list is RENUMBERED to THIRTEEN slots, counted rather than inherited: the
numbered list runs 1 through 13.** R290(c) held slot 1 and closed, so the old
slot 2 (R293) is now slot 1 and everything below moved up one, per the
2026-08-27 and 2026-09-02 precedents. Slot numbers in every note below this one
read one higher than the current list.

**The ladder is TWO rungs and that is a reconciliation, not a shortcut.**
R290(c) said "walks a fixed documented relaxation ladder"; CLAUDE.md's T-15
rung, written hours later from the same post-mortem and carrying the measured
cost, says open every binding control AT ONCE because five careful steps cost
more than one crude step under a clock. CLAUDE.md is binding, so the ladder is
fixed and documented and it has one crude rung plus the blank-row rung.

**Two defects this commit found in its own work, and the second is worse than
anything in the filing.** The first cut opened overlap to the ROSTER SIZE — two
lineups sharing every slot are the same lineup, two identical entries in one
contest are a DK rejection, so the rung would have converted a BADLY-SHAPED
refusal it may fix into an ILLEGAL one it may not. And **commit 1's
classification was silently wrong for four DK-rule gates**: `_GATE_ERROR_RE`
matches only PRE-export gate errors, `execute_portfolio` emits post-export
failures in a different shape, so `failed_gates` came back empty and
`roster_legality_passed` took the no-gate-named default of BADLY-SHAPED. The
governor would have re-solved past a DK illegality. Both found by writing the
acceptance test rather than by reading, which is the argument for putting the
executed test in the same commit as the mechanism.

**A third finding from a SURVIVING mutation:** the refusal payload was writing
`refusal_class` twice with the second write winning, so the first was a
live-looking value no reader saw. One writer now.

**26 mutations, 26 killed, each against the single guard written for it.** And
one string pin retired opportunistically: R288's source-text assertion is now
an AST question plus an executed test, which is one member of R300(c).

**The named remainder, holding no slot:** rung 2 (accept blank reserved rows) is
Showdown-only, because `execute_portfolio` passes
`require_all_reserved_filled=True` at both validator calls. It is filed on the
R290 stub with the engine change it needs, and a test asserts the cited
constraint still exists so the boundary cannot go stale quietly.*
...
(`test_core` 1040 -> 1047, `test_showdown` 166 -> 173, both `grew`); golden
replay 9 tests unmoved; no delivered byte changes for a salary file without an
`Excluded` column, and the plain-file `projection_digest` is the same
`9b9577bc53c73866` before and after, so no live bank bucket is orphaned.**

**Slot 1 is REFILLED by promotion, and the list below is RENUMBERED to fifteen
slots rather than annotated** — the 2026-08-27 precedent, and the reason is that
nothing about closing R291 changes any other item's rank: R292 was slot 2 on the
same three criteria (prize impact, then lift, then dependency) and is now slot 1.
Every slot number in the notes BELOW this one is therefore one higher than the
current list; read the list, not the older notes' numbers.

Two riders this landing earns. **R300(a) shipped with it** and R300 keeps its
slot for (b), (c) and (d) — (b) is the wall-clock preflight branch that printed
the 09-01 false PASS, and it sits with slot 1 now because R292 touches the same
referee. **R164's `Excluded` third shipped with it**; what remains there is (a),
(c) and the opponent-direction half of (b), which is the same shape (a job the
solve can never answer) reached from a different fact and belongs beside the
`eligible` frame this landing added.

**And the enumeration paid for the ninth consecutive time.** R289 named two stamp
sites; the class had five. A sixth defect surfaced only because a new test failed
for the right reason: the Showdown both-teams MILP rows are built from the LEGAL
pool, so an exclusion covering one side made that rule vacuously true and the
solve returned a one-team lineup DK rejects — the exact vacuity
`melt_showdown_salary_csv` refuses a single-team FILE for, reached through a door
the fix itself opened. Two of the sentences R289 wrote into CLAUDE.md were also
false and are corrected in place, which is the part worth carrying forward: a
closing entry that asserts a property is a claim, and R233's rule about
enumerating the class in the entry is worth nothing if the entry's own claims are
not measured.*
...
***The session ended on a dead sandbox and that is why R165 and R163 did not
ship.** Immediately after `cd3e145` the Linux workspace stopped answering
(`Failed to create bridge sockets after 5 attempts`, ten probes over ~10 minutes,
confirmed as a transport failure and not a boot race: no stdout at all, not even
an `echo`). No tests, no gate, no `git`. The R165 fix was fully written — one
shared normalizer pair with `_prepare_single_lineup_df` routed through it, and
all five live members of the class fixed — and then **backed out of the working
tree**, because it changes a return dtype on two helpers and unverified edits to
the file CLAUDE.md calls the lineup source of truth do not sit on disk waiting
for someone else to discover them. `mlb_engine/optimize/optimizer_v3.py` is back
to what `cd3e145` committed.*

***What was kept instead is the analysis, in the entries, because that is the
part that took the session.** R165's entry now carries a re-verified premise, the
five-member enumeration with each unnamed member's mechanism, the two members
kept with reasons, the model site, the revised fix, and the one fixture fact that
would otherwise cost the next session an hour (these helpers read DK's ROSTER
Position, not DK's `Position`, whose value for an arm is `SP`, and
`_parse_positions('SP')` contains no `'P'` — so a repro built on the wrong column
reports a zero-arm pool and the defect reads as having no reach). R163's entry
carries its corrected citation and the two-site reading that makes its fix
implementable, including which site is KEPT and why, and the budget consequence
to measure rather than assert. **`docs/backlog.md` is the one dirty tracked file
and it is documentation only** — it cannot change engine behaviour, which is the
property that made it safe to leave when the engine edits were not. It needs a
commit; the next DEV session or Ben takes it.*
...
real `fetched_at` stamp, which also closes the mtime-only-aging hole R277(d)
filed. The (b) measurement: **max population-only |Δmultiplier| 0.0123, 0 of 191
arms over the 0.05 bar, 95 up / 96 down** — the stop condition does not fire and
this was a coverage fix. Two earlier cuts of that measurement said 0.13 and
tripped the bar; both were junk, because comparing the old file to the new one
compares two dates as well as two populations. **That file's date has now
confounded FOUR measurements**, and the generalisable lesson is in the CHANGELOG:
a two-file comparison cannot separate population from time, so construct a
one-file comparison that holds time fixed. (c): the [50,100) TBF band went 0 → 32
arms whose median K/9 is 7.56 against the ranked population's 8.22 — LOWER, not
the high-K reliever contamination the entry predicted. Floor constants stay, now
on evidence rather than on sequencing.*
...
2. **R315(c) — the tracked manifest that points at an untracked file** (P1,
   ...
   2 rather than in the smalls because it is silent, it is already committed
   wrong, and the remedy is one `git add`. (That sentence read "slot 3" until
   2026-09-06; R315(c) has held slot 2 since it was filed and the number was
   stale on arrival.) Then, unchanged:
   **R310 + R312 + R313 — three falsehoods with XS remedies** (P1, XS each).
   ...
   (`optimizer_v3.py:3749`) is an EV claim on a deterministic proxy, which is
   CLAUDE.md's own non-negotiable, and the rename is one line plus its pins.
   R313: a forbidden core whose members have left the pool becomes
...
3. **R308 — an external field estimate in the build loop** (P1, S-M; one Ben
   ...
   diagnostic we run can see, while `qa_portfolio` section 4 read the same file
   as chalk-POSITIVE on its own prior. Precondition only Ben can settle: the
   working transport is his own signed-in Chrome plus a live RotoWire
   subscription; container `curl`, `web_fetch` and the built-in browser pane
   all fail. Build `tools/rst_from_paste.py` on `odds_from_paste.py`'s
...
4. **R317 + R319 + R320 -- the pool the artifact does not describe** (P1/P2,
   ...
   mattered most. R317(b): a partial platoon refresh stamps `collected_date` today
   and the 7-day check reads the top-level stamp, so six teams refreshed on 1810_3g
   silenced the warning for twenty-four that were a month old -- a build seeds a
   projected nine from month-old data with nothing printed, and on that slate the
   30-day file held three players no longer in PIT's order and missed three now in
   it (apex proxy 1056.83 -> 1085.38 after the refresh). R317(a) is why nobody
   refreshes it: `--fetch` is 403'd by a urllib CLIENT FINGERPRINT where `curl`
   returns 200 on the identical URL in the same minute, so the documented one
   command is a thirty-page browser procedure. R319: the paste tool -- the ONE
   input that turns on the F4 platoon factor on a DK-covered slate -- mints a false
   team-disagreement warning on a shared normalized name (two sightings, one night,
   `--resolve` cannot even express the fix) and reports a rostered starter as
   `NOT IN DK POOL` on a first-name alias, which routed the slate's best stack
   through the TBD path. R320: `factors_inert` cannot see a HALF-inert composite,
   so `none inert: every computed factor moved at least one row` printed true over
   `f4_platoon_applied: 0`; the Showdown brief emits no platoon counter at all and
   reports handedness RESOLVED where a reader takes it for APPLIED. Every one is
...
5. **R118 — the replay tool** (P1, M; Tier 2 head). **PROMOTED 2026-09-06 from
   ...
   XS-S and each cost a measured lock-window call on 09-03 or 09-04, and it
   moves above the three P1 batches at 6-8 because those are quality items with
   no measured cost this week while this one has six weeks of them. It gates
   everything below. Then, unchanged in order: R48 + R83 → R10 (still gated on R225; its
...
8. **R285 (+ its 09-03 rider) + R311 + R207 + R204's naming half** — refusals
   ...
   binary-search it. Measured: 9 minutes of a 66-minute window on 1940_6g spent
   bisecting five builds to find that `max_player_exposure_pct` had to be
   exactly 1.0; one call on 1915_6g spent on a `BANK-LIMITED:` refusal whose
   ...
   expression over `errors[]` in the final fix. **NEW here 2026-09-06:
   R125(a)'s refusal half.** A side whose only marked arm is a `PO` refuses with
   `NYY: no probable or declared starter`, which is false -- it is a side whose
   only marked arm is an OPENER, a different fact with a named remedy
   (`--declare-pitcher`). It cost attempt 1 of the 2026-09-04 late build. Same
...
9. **R307 — the captain leverage sleeve** (P1, M). **ITS GATE IS DISCHARGED as
   ...
   lift and it is the only M between slots 6 and 11 (R118 at 5 and R87 at 12
   are the others on the list); a session that wants it earlier
...
12. **R87 (REPRICED 2026-09-06) + R321 -- the build clock** (P2, M then S-M).
    ...
    clock** across four direct-path runs, one 18-entry six-game build issues ~98
    `milp` calls, the base bank pays 13-20x per candidate what `extend_bank` pays
    from the same pool, and **zero solver timeouts have ever been recorded on any
    date** -- so the wall clock is many individually fast, proven-optimal solves,
    proving optimality to a gap of 8e-5 on an INPUT to the allocator. R321 is the
    fifth member of R87's own class and was unfiled: the augmentation pass and
    `extend_bank`'s job loop issue 57 to 383 fully independent solves one at a time
    and no parallel primitive exists anywhere in the repo (grep returns 0). R87
    first, because parallelism multiplies whatever the per-solve cost then is.
    The criterion (1) argument is the T-schedule's: an 83.6s build inside a 130s
    inner budget leaves room for one attempt, and every lost-window entry on this
...
14. **R301 — the instruction corpus** (P2, M). Three of the four money-boundary
    ...
    is **the ledger pin**, and R301's own count is amended 15 -> 11 there. **Two more
    corrections shipped 2026-09-06 with the board merge, same fix-rather-than-
    queue treatment**: `docs/cowork_sync_protocol.md` gained the third transfer
    direction (chat-uploaded DK files reach the mount by `stagedPath`, no
    `SendUserFile`), and SKILL.md's `--leverage` example gained the `archetype`
    key it needs to run at all plus the slate-thinness caveat on its -7.72%
    figure. **And a DEBIT against this item's own goal:** R318 added ~14 lines
    to CLAUDE.md's T-schedule and ~9 to SKILL.md, both paid deliberately for a
    rule three slates have now broken. The ≤ 6 KB target below is that much
    harder and the additions are named here so the shrink pass knows what it
    is looking at.
...
16. **Smalls, batched opportunistically:** **R315(b)** (the standings_pulls
    ...
    R243, R269 (with R245), R271's two-line remainder. **Added 2026-09-06:**
    **R125(a)'s grading** (the 28-row PO/PLR salary table graded against
    realized `fpts` in `data/archive/`, about an hour, and it is what turns a
...
verbatim, is the version of this file at `92cd423`; `git show
92cd423:docs/backlog.md` carries it. Every reason that still applies is restated
in the slot it moved to above; git is the record, not a second copy here.)*
...
|---|---|---|---|---|---|
| no tilt (baseline) | 0.25 | 6 | 33.6% | 2 of 16 | 0 |
| cap alone, no tilt | 0.13 | 10 | rose | 2 of 16 | 1 captain |
| mild tilt 1.25/0.90 | 0.19 | 8 | 30.9% | 5 of 16 | 0 |
...
**Finding 2's sample-size guard now has a measured negative result attached
(2026-09-04, 2210_1g_sd; the full table is on R310).** Tightening
`max_player_exposure_pct` against an APPG prior standing on two games moved the
offending player 58.3% -> 50.0% -> 41.7% across caps 0.50 / 0.42 / 0.34, **relaxed
at every setting**, and bought the flatness by leaving $13-15k unspent in the tail.
`structural_floor_pct` was 0.30. An exposure cap redistributes a ranking and cannot
repair one, so finding 2's guard has to sit at the PRIOR and not at the cap.
...
**(c) closed in two commits.** Commit 1 classified ELEVEN refusal exits, not
the eight this entry filed nor the nine its ed10 rider claimed -- exit 10
refuses too, and the item enumerated `return 3` while the class is refusal
EXITS. The table lives in code as `REFUSAL_SITES` with an N+1 test rather than
in prose, because the count moved three times in three days. Commit 2 built
`--deliver-by` over the BADLY-SHAPED set. Gate 1703 -> 1723 -> 1755, golden
replay unmoved, no delivered byte changes without the flag.

**Two premises in this entry were false and one of them hid its own subject.**
The table labelled two Showdown sites `bank_short`; measured, the second is
`bank_short_of_reserved_rows`, the BLANK RESERVED ROW site -- exactly the wall
(c) exists to move, filed as a duplicate of its neighbour. And `tools/build_
slate.py` does not exist; the file is
`skills/generate-lineups/scripts/build_slate.py`.

**The finding.** `portfolio_caps_passed` is exactly the exposure and overlap
errors, every one of them Ben's own number and every one a file DK accepts,
while `roster_legality_passed` is DEFINED as everything that is not one of
those -- and one exit stood over both, so the refusal that lost 1940_9g could
not tell a DK rejection from a concentration preference. Same shape one
function over: `verify_classic` emitted one string over a partially filled row
(DK rejects) and an all-blank reserved row (DK enters nothing).

**And commit 1 shipped a defect commit 2's acceptance test found.**
`_GATE_ERROR_RE` matches only PRE-export gate errors, so `failed_gates` came
back empty for all six post-export gates and four DK-rule gates took the
no-gate-named default of BADLY-SHAPED. The governor would have re-solved past a
DK illegality. Both halves fixed in commit 2; the reasoning is in the CHANGELOG
entries.

**SURVIVING REMAINDER, no slot: rung 2 on Classic.** `accept_blank_rows` is
Showdown-only. `execute_portfolio` passes `require_all_reserved_filled=True` at
BOTH of its `validate_dk_entries` calls, so a bank short of the reserved rows
fails `roster_legality_passed` inside the engine before any caller can label the
result -- which means Ben's "blank rows block certification" and R290(c)'s
"blank rows stop blocking delivery" cannot both be implemented on Classic
without an engine change on the certification path. Not smuggled in behind a
flag. `test_rung_two_is_unavailable_on_classic_and_the_refusal_says_so` asserts
the cited constraint still exists, so this boundary cannot go stale silently.
- **Fix shape:** a `require_all_reserved_filled` that the CALLER sets, defaulting
  True, with the blank-row error moved out of `roster_legality_errors` into its
  own reported fact so the gate keeps failing while the file becomes
  deliverable. That is a change to a gate the money-boundary items depend on,
  so it wants its own commit and its own evidence.
- **Also open, and measured rather than assumed:** there is no executed Showdown
  refusal test. The MIN@CHC fixture delivers under every cap tried
  (`max_shared_players: 0`, `max_cpt_exposure_pct: 0.05`, at 30/60/200 entries)
  because the Showdown ladder relaxes its own three controls before it
  truncates, so what that test needs is a fixture whose pool cannot fill its
  reserved rows.

**(a) and (b) below are UNCHANGED and hold no slot.** They live here only, are
easy to misread as part of the closed work, and share no surface with (c).

**Rider 2026-09-02 (ed10, GF10-S6), RETAINED because it is (a)'s and it was
nearly lost with (c)'s migration.** (a) is TWO omissions, not one: the three
leverage controls are absent from the bucket key AND `Projected_Ownership_Pct`,
the column those constraints read, is absent from
`bank_cache._PROJECTION_COLUMNS` (:542-544), so `apply_leverage_ownership`'s
overwrite (`execution_pipeline.py:4479`) moves neither the bucket nor the
digest; the allocator has no per-candidate ownership filter, so the "bank is a
superset" argument (`bank_cache.py:39-43`) does not hold for this control.
Append `lev:` and the sorted ownership column only when leverage is non-empty.
**Rider 2026-09-02 (ed10, GF10-D3), (b)'s half:** (b)'s proxy delta must be
computed on the UNWEIGHTED `Base` or it inherits the thesis weighting; R295(b)
carries the fix, so re-verify (b)'s premise against it before building.
...
**The training substrate is verified present and the blocker was never data
collection (rider 2026-09-06, from BUILD fragment
`2026-09-04_BUILD_ownership-fit-set-already-archived.md`).** Every mined contest
file in `data/archive/*/mined_*.json` carries a `player_table` of
`{player, player_norm, roster_position, pct_drafted, fpts}` -- DK's own
post-contest %Drafted, observed and not projected. That is **29,921 player-rows
across 379 contests and 26 slate dates** (2026-06-03 to 2026-08-13; the archive
directory is at 27 dates as of 2026-09-06). The filing session's search for an
outside pre-lock source came back empty and correct -- LineStar and
FantasyTeamAdvice are login-gated, Savant and FanGraphs publish no ownership at
all -- and that is the wrong place to have been looking, because the FIT set is
already on disk.

***Two claims in that fragment are stale at this head and the correction matters
for scoping.*** It says the prior's only grade is a mean signed error of -10.44
from a single archived grid (06-03) and that one date was used. **R306 step 1
shipped 2026-09-03 and grades the prior on 297 archived contests across four
archetypes, per contest and never pooled**, by a tool rather than a runbook step.
So this item's remaining work is the FIT, not the grade, and the grade it will be
measured against now exists: median per-contest Spearman 0.430 to 0.629 by
archetype with a systematic level under-prediction of -2.30 to -4.16 points. The
bar is unchanged -- beat flat-12 in the satellite cell, graded into the ledger,
before any production column flips -- and two acceptance lines are worth carrying:
`ownership_pred.py` emits a version string that is not `v0.1-prior` and carries
its own out-of-sample error, and `qa_portfolio`'s "the two scales do not meet
until the prior is fit" caveat can be deleted.

***The set decays while unpulled, and pulling is BEN'S action by the
money-and-entry wall.*** `data/standings/CONTESTS_AWAITING_STANDINGS.md` read 121
contests across 9 slate dates when regenerated 2026-09-03 and **205 contests
across 11 slate dates** at its 2026-09-05 regeneration. The 2026-07-25 ledger note
records that DK's export ages out some days after a contest settles, so the
training set is decaying oldest-first at roughly 40 contests a day of new arrivals
against zero pulls. This is the one input on this item that no DEV session can
supply. Surfaced in the queue's decisions-owed line rather than left inside this
entry.
...
here rather than given its own number so the mispricing family keeps one owner.
It supplies the mechanism this entry was missing.** The screen this entry asks
for should put prior ownership beside the build's own projection rank per posted
starter and surface the divergence. The case worth naming, from 2005_1g_sd:
Cal Quantrill (TEX, SP) at 27.4% prior ownership and DK APPG 8.5, against a 3.01
ERA and a 1.49 L7. **A starter's `AvgPointsPerGame` collapses role and
performance into one number, so a good arm used in short outings reads as a bad
arm, the field prices him accordingly, and the number that suppresses his
ownership is the same number the engine ranks him on. The engine inherits the
field's mistake instead of exploiting it.** That is a rule, not an anecdote: flag
any probable SP whose APPG sits below a threshold while his season ERA/WHIP sit
above one. Two companions from the same table: Jake Burger (TEX, 6th) at 21.8%
and APPG 6.9 with real HR power, where the captain slot pays 1.5x for spikes; and
Junior Caminero (TB, 3rd) at 45.9%, the field's default captain. **R310 is the
same defect on the other side of the distribution** (APPG standing on one game,
reading high), so the two share a diagnosis: APPG is a role-and-sample-blind
prior and every consumer of it needs to say so. Nothing above is an edge or a
probability; the ownership column is an uncalibrated structural prior.
...
**Negative result on the field side, 2026-09-04, so the search is not re-run.**
No projected-ownership source for an MLB SHOWDOWN slate was found. Stokastic's
showdown ownership page is a paywalled index and returns no rows to `WebFetch`;
DailyFantasyFuel publishes the projections and ships the ownership column EMPTY.
So there is no external feed to seed `outputs/<date>/ownership_pred_<tag>.json` on
a single-game slate, and this screen currently has a projection side and no field
side. **Anything delivered on Showdown today is projection-driven and is not
leverage-priced; say so in the brief rather than implying otherwise.** R306 step 2
gives the captain slot its own 100% prior, which is the internal answer; R308 is
the external one and its transport precondition is Ben's.
...
**Why this file is not cosmetic.** `data/reference/fangraphs_platoon_lineups.json`
supplies the projected batting order for any team that has NOT posted, through
`live_data_adapters.build_slate_pool`. On a slate with a TBD team it decides
which nine bats are in the pool and what F2 weight each gets. Measured on
1810_3g (2026-09-04): the file was 30 days old, PIT was the one unposted side,
its projected order vs RHP held three players no longer in it (Endy Rodriguez,
Jacob Gonzalez, Ronny Simon) and was missing three now in it (Konnor Griffin,
Oneil Cruz, Rafael Flores Jr.), and only slots 2 and 3 were unchanged.
Rebuilding on the refreshed file at identical controls and the same three gates
moved the apex review proxy 1056.83 -> 1085.38, the worst entry 134.03 ->
147.37, and worst-triple concentration 4/7 -> 3/7. PIT went from one primary
stack to two. Both are deterministic review proxies, not probability claims.

**(a) `--fetch` dies on a urllib fingerprint, not on egress, and the tool's own
refusal says the wrong thing.** `fetch_page` (`tools/fetch_fangraphs_platoon.py:271`)
builds a `urllib.request.Request` with `_UA` and calls `urlopen`. Measured on the
device VM 2026-09-06, same URL, same minute
(`https://www.fangraphs.com/roster-resource/platoon-lineups/pirates`): `curl`
with its default UA returned **200** (155,942 bytes, both `Go-To Starting Lineup
vsR`/`vsL` present), `curl -A "python-urllib/3.10"` returned **200**, and
`urllib` returned **403** under both a Chrome UA and a full browser header set.
A UA naming python gets through under curl and a UA naming Chrome does not get
through under urllib, so it is TLS/HTTP2 client fingerprinting on FanGraphs'
edge -- not the proxy, and not the User-Agent the module blames. `FetchRefused`
tells the operator "the site declining automated access", which is true of the
CLIENT and false of the site. **Fix (S):** give `fetch_page` a curl transport
(`subprocess` to `curl --fail --silent --show-error --max-time N`) where curl
exists, falling back to `urlopen`, with the transport that answered named in the
tool's own JSON. Classify the failure R147's way -- no route / declined /
unknown -- and **do not encode "there is network" either**; R316 is the standing
instruction that egress is a per-session measurement in BOTH directions. Keep
`--from-dir`: it is the only path that works when neither client reaches the
host, and it is what made this measurement possible with no code change at all
(three curl'd pages ran straight through: `parsed 3 team(s), 27 failure(s)`,
the 27 being `no saved page` for teams not downloaded).

**(b) A partial refresh silences the staleness check for all thirty, and this is
the P1 half.** `merge_preserving` correctly stamps each carried-forward team with
`carried_forward_from`, but the top-level `collected_date` still becomes today
and `build_slate.py`'s 7-day check reads the TOP-LEVEL stamp. Six teams were
refreshed on 1810_3g at 17:03 and the warning went quiet for the twenty-four that
were a month old. Harmless there (the one TBD team was among the six) and not
harmless on a slate whose TBD team is one of the twenty-four: the build seeds a
projected nine from month-old data with **nothing printed**. That is the same
shape as R289 and R313 -- a pool-membership fact the artifact does not carry --
arriving through a reference refresh. **Fix (XS):** age the check per team
against `carried_forward_from` where present, or carry a second top-level stamp
naming the OLDEST team in the file. Until it lands, the fragment's step-0
checker (which reports the oldest team, not the file's stamp) is the reading to
trust, and the standing instruction is refresh all thirty or none.

**(c) Two pages carry all thirty teams; the tool walks thirty.** `TEAMS` is a
30-entry table and `--fetch` loads `/roster-resource/platoon-lineups/<slug>` once
per team. `roster-grid?platoon=vsr` and `?platoon=vsl` each return all 30 teams at
nine slots each, verified this session, and the `platoon` parameter genuinely
switches the order (PIT leads Horwitz on `vsr`, Griffin on `vsl`). Cross-check
run: 30 per-team pages against both grid pages, **vs_RHP 30/30 identical, vs_LHP
30/30 identical, 0 mismatched, 0 not-found** -- 540 slots, position and name, in
slot order, from two independently rendered views. **Fix (S):** `--from-grid
<vsr.json> <vsl.json>`. Everything it needs is in the retained fragment's Steps
2-3: the side-by-side cell-block arithmetic (five teams per table, four columns
each, team `k` at `[k*4 .. k*4+3]`), the FG->DK crosswalk (CHW/KCR/SDP/SFG/TBR/WSN
-> CWS/KC/SD/SF/TB/WSH), the `B/T` split the per-team page did not have, and the
1-9 completeness assertion. **Note what sourcing from the grid costs:** the grid
stops being an independent check and the per-team pages become the cross-check,
so spot-check two or three teams after a grid refresh rather than claiming
corroboration you did not run.

**(d) The file's VERIFICATION STATUS paragraph claims neither of the two things
that are now true.** The parser is verified against RENDERED table structure
three times and, since 2026-09-06, against curl'd SERVER HTML for three teams.
Say so with the count; that is a stronger position than the 09-04 fragment's
caveat 2 recorded and it is the distinction that matters when `--fetch` runs from
Ben's Windows machine.

**The procedure fragment is RETAINED in the inbox, not deleted, on the 2026-08-09
precedent.** `docs/backlog_inbox/2026-09-04_BUILD_fangraphs-platoon-refresh-procedure.md`
is the sole carrier of the step-0 staleness checker, the grid extraction script,
the per-team HTML synthesis, and the `reference`-claim sequence. Those exist
nowhere else and a session needs them TODAY, before (a) and (c) land. It is
consumed and deleted in the commit that ships (c), because at that point the
procedure is one command. Its caveat 1 is already dead -- superseded by (a) above
-- and its superseded sibling
(`2026-09-04_BUILD_fangraphs-platoon-browser-path-works.md`) was swept on merge.

**Audit fields.** Moves: pool truth, speed. Acceptance: (a) a fixture where
`urlopen` raises 403 and curl returns a page parses, with `transport: "curl"` in
the JSON, and a genuine site refusal still raises `FetchRefused` with the reason
classified; (b) a file with one team refreshed today and one carried from 30 days
ago reads STALE, and the warning names the oldest team; (c) `--from-grid`
produces byte-identical output to `--from-dir` on the same slate. Owner: DEV for
the code, `reference` claim (ARCHIVE) for any write to
...
**The measurement that makes it a step rather than a curiosity.** On 2140_2g
(ATH@SEA + STL@LAD, 9 entries, delivered `635cc0b52e73`), `qa_portfolio.py`
section 4 read `ownership_pred_2140_2g.json` and scored all six contests
chalk-POSITIVE, +37.4 to +57.4 pp against its own field mean, with low-owned
carry 0 in 5 of 9 entries. RotoWire's DK roster-% for the same portfolio says
something different in the same breath:

| player | our exposure | RotoWire RST% | delta |
|---|---|---|---|
| Ivan Herrera (STL) | 56% | 0.04% | +56 pp |
| Zack Gelof (ATH) | 56% | 0.67% | +55 pp |
| Freddie Freeman (LAD) | 67% | 12.71% | +54 pp |
| Kade Anderson (SEA) | 67% | 19.11% | +48 pp |
| Jose Fermin (STL) | 44% | 0.47% | +44 pp |
| Jordan Walker (STL) | 33% | 0.56% | +33 pp |
| Tarik Skubal (LAD) | 67% | 37.68% | +29 pp |
| **Kyle Tucker (LAD)** | **0%** | **26.57%** | **-27 pp** |
| **Alex Call (LAD)** | **0%** | **20.38%** | **-20 pp** |

Both statements are true about different halves of the file: chalk-positive on
the two arms and the LAD/SEA core, and carrying FOUR sub-1%-owned bats at 33-56%
each. The prior could not see the second half, because its own R135 grade is a
mean signed error of -10.44 points, i.e. it under-concentrates, which is exactly
the shape that flattens a near-zero tail into something ordinary. The `-27 pp`
row is the other half of the value: Kyle Tucker at $3,500, batting 7th for the
slate's highest implied team, `Base` 7.20 and the best points-per-dollar of any
LAD bat in the frame, the field's third-most-owned play, and the delivered file
has him at zero. No diagnostic we run says that out loud, because none of them
knows what the field is doing.

**Truthful label, kept because it will be tempting to drop.** RST% is a
third-party PROJECTION, not a measured ownership and not a probability. It is
one more labeled prior whose single virtue is that it is not ours.

**The precondition, which is a fact only Ben has and is why this is not slot 1.**
Measured 2026-09-03: container `curl`/`urllib` to rotowire.com gets a proxy 403
on CONNECT; `web_fetch` returns an empty body (client-rendered); the built-in
browser pane renders headers and zero data rows because that profile is not
signed in; **Claude in Chrome with Ben signed in works, full 250-row grid**.
Stokastic is a subscriber-gated marketing page and costs clock for nothing. So
this rung needs Ben's own browser and a live RotoWire subscription, which is a
precondition to state in the skill rather than discover at T-25.

**The extraction that worked, so nobody re-derives it.**
`https://www.rotowire.com/daily/mlb/player-roster-percent.php` renders a Webix
grid; read the DOM by column index and **do not enumerate `window` keys**, which
trips a `[BLOCKED: Cookie/query string data]` guard and costs a call.

    const col = i => [...document.querySelectorAll('.webix_column[column="'+i+'"] > div')]
                       .map(d => d.innerText.trim());
    // 0 PLAYER (suffixed "(L)"/"(R)"/"(B)" and injury tags "10D"/"60D"/"DTD")
    // 1 POS  2 TEAM  3 OPP  4 START(batting order)  5 SAL  6 FPTS  7 VAL  8 RST%

Strip the handedness and status suffixes before any crosswalk. **The view is ALL
GAMES, not the draftgroup**: a 2-game night slate returned 250 rows across every
game on the date, so ordering should survive but LEVELS will not, because a
2-game slate concentrates everything. Whether RotoWire exposes a per-draftgroup
RST% is UNRESOLVED and is the first thing to check next time.

**Fix.** (1) `tools/rst_from_paste.py` modelled on `tools/odds_from_paste.py`,
which is the right precedent: header-named table in, crosswalk to DK
`Player_ID` against the salary file, refuse rather than half-write, round-trip
its own output, record source and `fetched_at`, and additionally record the
SCOPE (`all_games` vs `draftgroup`) since that is not recoverable later; refuse
silently-partial coverage the way the odds tool refuses a half-priced slate.
(2) An input both `build_slate.py` and `ownership_pred.py` accept. `--leverage`
today reads only the repo's own `ownership_pred_<tag>.json` schema and REFUSES
anything else by design; the decision is whether an external file becomes a
second accepted schema or `ownership_pred emit` gains a blend input and stays
the one reader. **Recommend the second**: it keeps one file feeding the solver
and keeps the grading loop intact, which is R306's whole subject. (3) The delta
table in `qa_portfolio.py` section 4, printed beside the prior's chalk-sum
columns rather than as a separate tool, because the value was seeing the two
disagree in one place; two columns, our realized exposure and RST%, sorted by
delta, both ends shown. (4) A `SKILL.md` step under "Better data when there is
time" carrying the clock gate (**run when T-lock > 25 min, skip below, never let
it block a build**; the working path cost about three minutes and the dead ends
another three), the Chrome precondition, and the snippet above.

**Open, and DEV's rather than BUILD's.** Whether the delta feeds a CONSTRAINT or
stays a report. Ledger 3.17's winner-minus-field medians are chalk-POSITIVE for
the archetypes in this file (Classic satellites +4.9, solo shots +9.5,
single-entry GPPs +8.6) and chalk-NEGATIVE only for supersatellites (-13.2), so
the direction to push is archetype-dependent and a global cap is wrong. If it
ever becomes a constraint it belongs in R307's sleeve, not in the whole-portfolio
objective. Grading is already asked for:
`ledger/inbox/2026-09-03_BUILD_rotowire-rst-vs-structural-prior.md` asks ARCHIVE
to grade BOTH this snapshot and the structural prior against realized `%Drafted`
when the six contests land, which is the first head-to-head the project has had
and is what says whether the step earns its three minutes.

**Scope note.** RotoWire is not DraftKings and reading a subscription page Ben
pays for in his own signed-in browser is not a scripted DK read. Nothing here
touches the money-and-entry wall, the manual-DK rule, or the truthful labels.

### R310. APPG as the Base prior is broken for a sub-5-game callup, the 0.60 salary shrink does not catch it, and nothing anywhere warns (P1, XS for the warning half, S for the shrink) | new 2026-09-03, from BUILD fragment `2026-09-03_BUILD_appg-base-is-broken-for-sub-5-game-callups.md`; measured on 2210_1g_sd with a counterfactual build

**What.** On 2210_1g_sd (STL@LAD, 19 entries) Leo Bernal (STL, $5,000 UTIL)
carried `AvgPointsPerGame = 12.5` in the DK salary file off approximately ONE MLB
game: he debuted 2026-09-01/02 and went 1-for-5 with 2 RBI. That is the
THIRD-highest APPG in the entire 20-player declared pool, ahead of Freddie
Freeman (8.0) and Jordan Walker (9.4). `showdown.py` uses APPG as the Base
proxy, so the ladder ranked a two-game callup above two of the best bats on the
slate and put him in 9 of 19 entries (47.4%), one slot under the 50% player cap.
**Every control read clean: `counted_relaxations.clean = true`.**

**Counterfactual, measured rather than argued.** Supplying `--projections` with
a single row (Base 5.5, from his Memphis line, .278/.363, .831 OPS, 16 HR in
101 G) moved him to 4 of 19 (21.1%) with zero relaxations either way. The freed
slots went to Pages, Saggese and Urias.

**(a) The shrink is scoped to the wrong noise.** `apply_base_prior`'s 0.60
salary weight exists for APPG small-sample noise, and it did not shrink this:
$5,000 is a real salary and 12.5 points is not the $5,000 salary line, so the
regression pulled toward a number the player has no claim to. What the shrink is
missing is a GAMES-PLAYED denominator. The salary file does not carry one, but
`Status` plus a roster-tenure lookup would, and **a flat "APPG resting on fewer
than N games is untrusted" floor needs no feed at all**, which is why the two
halves are sized separately.

**(b) There is no warning anywhere, and that is the cheap half.** The pool report
names thin teams and the excluded column; nothing names a Base prior standing on
one game. An operator who did not happen to research the callup ships 47%
exposure to it and every artifact says the build is clean. Fix:
`pool_report.small_sample_base` naming any declared starter whose APPG rests on
fewer than N games, with the count. XS, no feed, no strategy change, and it turns
an invisible input into a readable one.

**Not filed as new, recorded because it is the third sighting:**
`api.the-odds-api.com` 403s on CONNECT from the container, so
`construction.win_share_basis` came back `even_split_no_market_input` on a game
the market priced at LAD -325. Hand-writing a raw-events odds JSON from a
published line and passing `--odds` fixed it (allocation moved 9/9 → 11 favorite
/ 4 underdog / 4 neutral). The fallback works; the gap is that nothing prompts
for it, which is R236(b)'s rider.

**SECOND instance, ten days later, plus a working workaround and a measured
negative result (merged from BUILD fragment
`2026-09-04_BUILD_appg-workaround-supplied-base-from-a-slate-source.md`, which
explicitly asks for no new number).** On 2210_1g_sd 2026-09-04 (WSH@LAD), Yohandy
Morales (WSH, 1B, $3,400 UTIL, batting 6th) carried DK `AvgPointsPerGame` 13.5 --
**the highest of any hitter in the 20-man pool**, James Wood next at 10.8 -- off 2
games and 8 PA (37.5% K, contract selected 2026-09-01). The default build put him
in 7 of 12 entries (58.3%) and relaxed the 0.50 player cap by one slot. Exactly
this item, exactly R123 finding 2.

***The warn predicate, and it is cheaper than the one this entry proposed.*** DK
ships no game count, so a PA-keyed guard needs data we do not have. The screen that
would have caught BOTH callups needs nothing but the pool: **APPG above the pool's
90th percentile among hitters WHILE salary sits in the pool's bottom quartile.**
Morales was the highest-APPG hitter in the pool at the cheapest posted salary; Leo
Bernal was third-highest at $5,000. Make it a `caution` line and a brief field,
never a gate -- a genuinely cheap star is a real thing and this is a question, not
a verdict. That is (b) with an implementable predicate attached.

***The fix that does NOT work, measured, so nobody re-runs it.*** Tightening
`max_player_exposure_pct` against the bad prior, same odds, same overlap and
captain caps: 0.50 (default) -> Morales 7 (58.3%), cap relaxed 1 slot, all tail
salaries >= $45,000; 0.42 -> 6 (50.0%), still relaxed 1, tail $40,400/$34,700;
0.34 -> 5 (41.7%), still relaxed 1, tail $38,200/$37,300/$36,200. `structural_floor_pct`
is 0.30 (6/20), so between the floor and the default there is not enough room:
with 12 rosters, a 20-man pool and max overlap 4, the cap buys exposure flatness
by spending $13-15k of unused salary in the tail and it relaxed at every setting.
**An exposure cap redistributes a ranking; it cannot repair one.** The 0.50 default
was right the whole time and the prior was the defect. (Recorded on R123 as well.)

***The per-slate workaround, no code change, for use before this ships.***
`--projections` is Showdown-only and takes a `Player_ID,Base` CSV used AS the
prior. Four steps, in order: find a slate projection source
(`dailyfantasyfuel.com/mlb/showdown-single-game-projections/` answered `WebFetch`
from the container and carried projected FP for all 20 posted players including the
cheap bats); **prove it is the same slate before trusting a single number** -- DFF's
salary column matched the DKSalaries UTIL column exactly on all ten WSH bats, and
without that check a page for the wrong game silently rewrites the entire prior
with nothing in the brief to catch it, because a supplied Base bypasses every
derived check; generate the CSV FROM the salary file by name, never by hand-typed
id; and supply the UTIL id, knowing that a 20-long `unmatched_ids` for the CPT rows
is not a failure (the pool is keyed one row per PLAYER, so UTIL carries the Base
and CPT inherits it). Result at identical controls: Morales 7 -> 5,
`player_exposure.relaxed_slots` 1 -> **0** with no cap touched, Teoscar 0 -> 3
captain slots; cost was `overlap_relaxed_slots` 0 -> 1 and one lineup spending
$37,000 of $50,000. `supplied_base` ratio_min 0.437, median 1.0234, max 1.9714,
18 of 20 differing from APPG. Preflight `review_ready`, 0 failures, 0 warnings.
**On Showdown the APPG-vs-external ratio table belongs BEFORE the first build**
whenever any posted bat is a recent callup or the pool is small enough for one
prior to dominate -- this session ran it only after Ben asked, having already
delivered at 58.3%.

***The handedness half of the same residual is R320***, not this item: every
player APPG overrated on 2210_1g_sd was a left-handed bat facing a lefty and every
one it underrated was a right-handed bat facing a lefty, 2x wide at the edges, and
nothing in the Showdown brief can say whether the platoon term was applied.

### R291. CLOSED 2026-09-02 -- SHIPPED, entry migrated to CHANGELOG.md

The salary file's `Excluded` column now reaches the assembled frame, the bank
job grid, the Showdown melt and its solve, and survives the late-swap refresh;
the pool report's count and the checkpoint's read the same number. **The R289
class had FIVE members, not the two its own enumeration named**, and two of the
sentences R289 wrote into CLAUDE.md were false: the column reached the pool
report and stopped, and the digest re-key was true only of a hand-built frame
(measured: the same `9b9577bc53c73866` for the plain and the restricted salary
file). One site was found by a test failing for the right reason -- the Showdown
both-teams MILP rows are built from the LEGAL pool, so an exclusion covering one
side made that rule vacuously true and returned a one-team lineup DK rejects,
the same vacuity the melt refuses a single-team FILE for. Full hit list, both
grep forms with what each one misses, the before/after repro and the job-grid
counterfactual are in CHANGELOG.md. R300(a) shipped with it; R164's `Excluded`
third shipped with it; R164's opponent-direction half is R293 and is untouched.
...
written down; this is the third sighting of inert F1.** Measured per host and
per transport on 1915_1g_sd: `api.the-odds-api.com` HTTP 000 from the container;
`actionnetwork.com/mlb/odds` empty body under `web_fetch`, **WORKS in the
built-in browser**; `espn.com` empty body under `web_fetch`;
`baseballsavant.mlb.com/leaderboard/*?csv=true` HTTP 000 from the container and
**WORKS under `web_fetch`**. `THE_ODDS_API_KEY` is present in `.env` and was not
the problem: the CONNECT is refused, so the key is never used, which is R147's
classification rule applied to odds. Savant is the useful contrast, dying at
rung 1 and passing at rung 2, so "the proxy blocks it" does not imply
"unreachable", and a session reading the router's blocked-host list as final
skips a transport that works. **The ladder is: container `curl` → `web_fetch` →
built-in browser (`preview_start` + `get_page_text`) → `odds_from_paste.py`.**
**The blocker on documenting rung 3 is this entry's own contract.**
`odds_from_paste.py` requires a book column and its docstring calls averaging
book columns by eye "R205's bug executed by hand and unauditable afterwards".
`get_page_text` flattens the Action Network grid into a column-less number
stream: rotation numbers identify the game and the pairs are away/home in order,
but which book owns which pair is not recoverable, because the header is a
separate flattened run and absent books collapse to `N/A` pairs that still
consume a slot. The filing session read MIL -120 / CHC +104 off it as a hand
consensus, used it only as a sanity check on the 8/8 side split, and did NOT
feed it to `--odds`. **Two fixes, both cheap.** (1) Read the grid
STRUCTURALLY: `read_page` returns the accessibility tree with header cells
intact, so column index → book name is recoverable; a small
`tools/odds_from_actionnetwork.py` consuming a `read_page` dump and emitting
this tool's per-book paste format closes the loop with attribution preserved.
(2) Failing that, document rung 3 as ONE named book: Action Network labels a
DraftKings column, `--book draftkings` is auditable and honest, and it is
strictly better than inert F1 while being worse than R205's de-vigged multi-book
average, which the documentation must say. **Two traps that each cost a call:**
`/mlb/odds/total` 404s because the market views are in-page controls and not
routes, and `find` locates the "Total" tab but the click is refused ("entirely
outside the viewport"), so the filing session got the moneyline and never the
total. Solving the second is what makes rung 3 equivalent to the API, since F1
wants totals more than moneylines. Third sighting of the underlying cost:
2026-08-24 1940_7g `f1_games_priced: 0`, 2026-09-03 1915_6g "odds fetch 403
through the proxy", 2026-09-03 1915_1g_sd here; R310's fragment adds a fourth on
2210_1g_sd, where `even_split_no_market_input` stood on a game the market priced
at LAD -325.
...
`leverage_base_2005_1g_sd.csv` and its provenance survives only in a chat
transcript. The proposed `data/slates/<date>/evidence_<tag>.json` carries
`player_id`, `claim`, `source`, `retrieved_utc`, `factor` and `applies_to`, which
is this entry's freeze discipline for an input that arrives by hand rather than
by fetch. A session taking either should take both; the argument is identical and
the file format is the only thing to agree on.
...
### R320. Two brief surfaces report a factor RESOLVED and are read as APPLIED: `factors_inert` cannot see a HALF-inert composite on Classic, and the Showdown brief emits no platoon counter at all (P2, XS for the Classic half, S for Showdown) | new 2026-09-06, merged from BUILD fragments `2026-09-04_BUILD_2210_2g-feed-identity-and-half-inert-f4.md` §(b) and `2026-09-04_BUILD_appg-workaround-supplied-base-from-a-slate-source.md`; VERIFIED-read at `build_slate.py:2237` and by AST-bounded absence over `run_showdown`

**The Classic half.** F4 is a COMPOSITE -- opposing-SP quality x platoon. On the
2210_2g zero-fetch build, quality moved all 36 rows so F4 was not inert by the
definition `inert_factors` uses (`build_slate.py:2237`, "scored N rows and moved
none"), while the platoon term applied to zero hitters. The two surfaces a
session under a clock actually reads both came back clean:

    factors_inert: []                                                   <- brief
    factors: none inert: every computed factor moved at least one row   <- stderr
    enrichment.counts.f4_non_neutral: 36
    enrichment.counts.f4_platoon_applied: 0                             <- the only tell

"Every computed factor moved at least one row" is TRUE and reassuring and the
build was half-blind. It mattered on that slate: both starters in the higher-total
game were LHP against righty-heavy lineups, and closing it moved apex 876.23 ->
883.84 and evened the washout histogram across both games. **Fix (XS):** a
`factors_partial` sibling that fires when a composite factor's SUB-TERM applied to
zero rows while the factor itself moved rows, printed on the same stderr line;
`f4_platoon_applied == 0 and f4_non_neutral > 0` is the whole predicate for the F4
case. Cheaper alternative if that is too invasive: add `f4_handedness_unavailable`
to SKILL.md's "Reporting back" checklist, which today lists pool warnings and stale
platoon pages and never names this key.

**What is NOT wrong, recorded because the filing session nearly filed it.** The
brief DOES name the sides that lost handedness and it was exactly right:
`brief.pool.dk_batting_order.f4_handedness_unavailable == ["ATH","LAD","SEA","WSH"]`
on the zero-fetch build, empty on the paste-fed rebuild. R143's surface did its
job; the session read `factors_inert` and the stderr line and never opened the
pool report. That is SKILL.md's "Diagnose from the artifact, never from
documentation" skipped, and it is worth keeping in the entry because the fix
below is for the two surfaces that CANNOT show this, not for a missing one.

**The Showdown half is worse and is what makes this S rather than XS.** On
2210_1g_sd, walking the brief for any key matching f4 / platoon / handedness /
inert returns exactly two fields, and **neither is a usage counter**:

    construction.handedness = {hitters_with_side: 18, teams_with_hand: 2, teams_without_hand: []}
    construction.platoon_unresolved_teams = []

No `enrichment` block, no `f4_platoon_applied`, no `f4_non_neutral`, no
`factors_inert`. Those are Classic-path keys written at `build_slate.py:1768-1770`
and `:2237`; `run_showdown` writes none of them. Handedness WAS available to that
build (`lineups_feed.json`, 13.5 min old, both teams, all 18 hitters), and both
surfaces report it RESOLVED while nothing reports it USED -- a reader takes those
for the same thing. The residual against an external slate projection was
cleanly handedness-shaped and 2x wide at the edges: every player APPG overrated
was a left-handed bat facing a lefty (Morales 0.44, Abrams 0.67, Wood 0.69) and
every one it underrated was a right-handed bat facing a lefty (Smith 1.34,
Teoscar 1.52, Kike 1.81, Rojas 1.86, Call 1.97). **Fix:** have the Showdown brief
emit the same `f4_platoon_applied` / `f4_non_neutral` counters the Classic path
computes, OR, if the ladder genuinely applies no platoon term to the APPG-derived
Base, say that in the brief in one line. Either answer is fine and the current
silence is not.

**Two things a taker should not confuse this with.** `--projections` bypasses the
platoon factor by design (`supplied_base.label` says so), so supplying a Base
routes AROUND the question rather than answering it -- the question has to be
settled on the derived path. And this is not R159(d)/R151: those are about the
key's GRAIN and the merge's name normalization, i.e. when the key is WRONG. This
is two other surfaces reading clean while the key is right.

**Audit fields.** Acceptance: a Classic fixture with `f4_non_neutral > 0` and
`f4_platoon_applied == 0` prints `factors_partial` naming F4's platoon term and
carries it in the brief; a Showdown fixture emits a platoon applied-count or the
one-line statement that no platoon term is applied. Owner: DEV. Rollback: report
only; no delivered byte moves.

### R321. The bank stage is 91-98% of build wall clock, it is 57 to 383 INDEPENDENT solves executed one at a time, and no parallel primitive exists anywhere in the repo (P2, S-M; behind or with R87) | new 2026-09-06, from DEV fragment `2026-09-04_DEV_build-clock-is-91pct-bank-and-nothing-is-parallel.md`; four measured runs, and the grep establishing the lever is unfiled returns 0

**What the clock is, measured from `runs/<run_id>/final/diagnostics.json`**
(`bank_diagnostics.diversity_augmentation.elapsed_s` and, since R293,
`bank_diagnostics.anti_correlation.solves_observed`; direct-path builds from
2026-08-25 on, the field is absent from older and sliced runs):

| run | brief | total_s | bank stage | share | base bank | aug |
|---|---|---|---|---|---|---|
| `20260903T224330Z_2fa0b3f8` | 1915_6g_t1 | 83.6 | 76.26 | **91%** | 36 lineups / 69.325s | 57 solves |
| `20260903T224137Z_99106fb9` | 1915_6g_open | 81.4 | 77.63 | 95% | 36 / 69.795s | 57 |
| `20260827T221014Z_412a55ff` | 08-27 t1, 5g/14e | 72.1 | 70.70 | 98% | 28 / 62.093s | 47 |
| `20260829T164416Z_8e34cf22` | 08-29 v2, 12g/31e | 155.9 | 147.86 | 95% | 42 of 62 requested / 145.447s, budget exhausted | 4 |

One 18-entry six-game build issues roughly **98 `scipy.optimize.milp` calls**.
Per-solve cost, measured rather than modelled: base bank **1.93 s/lineup** (6g),
3.46 (12g), 2.22 (5g), 0.32 (2g); augmentation **0.12-0.18 s/solve**;
`extend_bank` sliced jobs 0.090 / 0.106 / 0.153 s/job. **The base bank pays
13-20x for a candidate `extend_bank` produces from the same pool, and the gap is
the accumulating pairwise overlap rows**, not the pool and not the solver. Two
independent instrumentations agree: `optimizer_v3.py:2429-2431`'s own docstring
(159-row pool: n=5 3.2s, n=8 7.6s, n=10 12.2s) and ed10's GF10-S9 repro (10.0s
wall, **96% in `milp`**, rows 546 -> 555, every gap <= 8e-5).

**The fact that reprices R87, and it is the argument R87 was filed without.**
**Zero solver timeouts have ever been recorded, in any brief, on any date.**
Across `outputs/`: `jobs_timed_out: 0`, `time_limited_accepted: 0`,
`jobs_raised: 0`, `jobs_unanswered: 0`, and every Showdown `solver_compute` block
reads all zeros. The 30-second per-solve limit has never bound. The wall clock is
**many individually fast, proven-optimal solves executed one at a time**, and the
engine is spending branch-and-bound time proving optimality to a gap of 8e-5 on a
candidate whose objective is a labeled prior and which is an INPUT to the
allocator rather than the deliverable. Verified in tree: the `milp` call at
`optimizer_v3.py:1163` passes `c`, `integrality`, `bounds`, `constraints` and
`options={'time_limit','disp'}` and nothing else; `grep -rn "mip_rel_gap\|x0="
mlb_engine/ --include=*.py` returns **0**. No warm start, no incumbent carry, no
model retained, no gap relaxation, at any of the four call sites
(`optimizer_v3.py:1163`, `showdown.py:563`, `contest_allocator.py:1417`, `:2898`).

**R233 enumeration before anyone files a sixth number.** The class is "make the
bank stage cheaper" and it has five members, four already owned: (1) `mip_rel_gap`
on bank solves only -> **R87**; (2) build the base constraint matrix once per pool
and append per-solve rows -> **R87** (`_prepare_single_lineup_df` at
`optimizer_v3.py:764` re-runs `normalize_id_frame`, re-parses positions and
re-assembles a fresh CSR per solve; GF10-S8 measured ~17ms of 29ms wall on a
136-row case, so small on small frames and unmeasured on large); (3) aggregate the
per-(SP, hitter) opposing rows into one row per SP -> **R87**; (4) stop paying for
guaranteed-infeasible jobs and duplicate solves -> **R73(a)(b)** and **R164** with
its R293 rider, with **R98(3)**'s ed6 rider owning the related waste that the
DIRECT path never opens the `BankCache` at all; (5) execute the independent solves
concurrently -> **THIS ITEM, and it was unfiled.** The grep, run 2026-09-04:
`grep -rnE "multiprocessing|concurrent\.futures|ThreadPool|ProcessPool|import threading|joblib|Parallel\(|n_jobs" mlb_engine/ tools/ tests/ skills/ | wc -l` -> **0**,
and `grep -rn -i "multiprocessing\|worker process" docs/backlog.md` -> no hits.
Re-run at this head, still 0. The only occurrence anywhere is a PROPOSED design
inside the ed10 critique (`docs/2026-09-02_critique_greenfield_spec_ed10.md:538`)
which the board absorbed into **R302** -- so the one lever nobody has costed is
reachable in the engine as it stands and is currently parked behind the largest
item on the board.

**What.** `bank_cache.extend_bank`'s job loop (`bank_cache.py:862`, one
`build_single_lineup` per iteration at `:874`) and the augmentation pass
(`optimizer_v3.py:4320-4445`) issue 57 to 383 fully independent solves per build
at 0.09-0.18s each. `build_multi_lineup`'s base loop (`optimizer_v3.py:2576`) is
**NOT in scope**: each iteration depends on the accepted priors, which is the
whole source of the ramp.

**Why.** These are the cheapest possible parallel targets -- no shared state, no
ordering dependence within a slice, results already collected into a set. Four
workers on the 2026-09-01 sliced build (383 jobs, 58.5s) is the difference
between a bank that fits one Cowork call and one that does not, and every
lost-window entry on this board (R285, R207, R290(c), R296) is a session that
needed a second attempt inside a 130s inner budget.

**The determinism constraint is the whole design problem and it is
non-negotiable.** CLAUDE.md pins `PYTHONHASHSEED=0` and requires every set
reaching the solver to be sorted (`determinism.stable_union`). A worker pool must
collect results and re-sort by a stable job key before anything downstream reads
them, never append in completion order. Whoever takes this states the reduction
order in the entry and pins it with a test that runs the same slice at 1 and 4
workers and asserts **byte-identical candidate ordering**.

**Sequencing.** Behind or with R87, not ahead of it. Gap relaxation and matrix
reuse make each solve cheaper for free; parallelism multiplies whatever the
per-solve cost then is, so doing it first buys a 4x on work that should not have
been done at all. Both move golden bytes and share R87's golden-regen sequencing.

**Gaps a taker inherits, all filed here rather than rediscovered.** No per-solve
duration is persisted anywhere -- `optimizer_v3._record_solver_status` computes
`elapsed_s` per solve at `:1178` and only the last survives in
`LAST_SOLVER_RESULT`, so the `[0.09 ... 1.33]` profile exists solely because an
ed10 reviewer instrumented it by hand; **a persisted per-solve histogram is the
acceptance evidence for this item and for R87 and it does not exist yet.**
`autobuild.py` records no timings at all. No archived `solver_probe.py` output
exists in the repo, so its projection model has never been checked against a
realized build. Showdown has no `solves_observed` equivalent for
`showdown.py:563`. Two clock costs that are not solver time, for whoever prices
the above: importing `optimizer_v3` off the mounted filesystem costs about **15
seconds by itself** (`SKILL.md:684`) and the default path does two in-build
network fetches, one with a 25-second timeout; and `drop_stale_jobs` purges the
bank on any projection-digest change, which reads `superseded_jobs_dropped: 4702`
on `outputs/2026-09-01/build_brief_open2005.json` -- correct behaviour and a
measured cost of iterating.

**Truthful labels.** Nothing here says a faster build is a better portfolio. The
claim is narrower and it is the one the T-schedule post-mortems keep making: an
83.6s build inside a 130s inner budget leaves room for one attempt.

### R309. QA can only describe what already shipped, and a hand-applied factor's provenance survives only in a chat transcript (P2, S; report-never-a-gate preserved) | new 2026-09-03, from BUILD fragment `2026-09-03_BUILD_captain-leverage-and-qa-as-research-arm.md` part 2; part 2(a) merged into R254 rather than duplicated here

**The structural limit.** `tools/qa_portfolio.py` is well built for what it does
and honest about its gaps: on 2005_1g_sd it printed `CHALK-SUM, LOW-OWNED CARRY
and CAPTAIN TIER: ABSENT`, named the ambiguity (two prediction files in
`outputs/2026-09-03/`), and gave the exact command to fix it. It also reported
`ADVERSARIAL FINDINGS (0)`, which was true, while 4 of 16 captains sat on the two
chalkiest bats on the slate. Compliance was clean and the portfolio was not
interesting. QA runs after delivery and can only DESCRIBE.

**(a) is R254's**, not this entry's: the captain-slot mispricing report, with the
APPG-collapse mechanism as its sharpest case. Merged there as a rider so the
mispricing family keeps one owner.

**(b) An evidence file, so external research is auditable.** The research
transport already works and is measured (`baseballsavant.mlb.com/leaderboard/*?csv=true`
dies at container `curl` and passes at `web_fetch`; FanGraphs' daily SP chart
fetched clean the same way). What it lacks is a record. On 2005_1g_sd a `0.85`
factor was applied to McClanahan off a sourced fact (FanGraphs SP chart,
2026-09-03: no start over 6 IP in his L6, against a TEX club ranked 5th in wOBA
vs LHP). The factor survives in `leverage_base_2005_1g_sd.csv`; its PROVENANCE
exists only in a chat transcript, so the next session sees a magic constant it
cannot audit, reproduce, or expire. Shape:

    data/slates/<date>/evidence_<tag>.json
    [{ "player_id": "44015053", "claim": "no start over 6 IP in L6",
       "source": "https://fantasy.fangraphs.com/starting-pitcher-chart-september-3rd-2026/",
       "retrieved_utc": "2026-09-03T23:48:00Z", "factor": 0.85,
       "applies_to": "pitcher_innings" }]

QA reads it and reports which adjustments moved which lineups and which were
inert. That also makes a bad adjustment falsifiable after the slate, which is the
only way any of this improves. **Lands with R251**, which is the same discipline
applied to the skill and market inputs; a session taking either should take both.

**(c) QA emits candidate swaps, still never a gate.** For each entry, name the
legal alternate captains within a stated projection band and with lower prior
ownership. QA has the salary file, the delivered bytes and the prior already, and
`repair_entry.py` already owns the legality filter and prints a first-match
rejection census (R272/R292). QA proposing and a human disposing keeps CLAUDE.md's
...
### R312. `Portfolio_EV_Proxy` is an EV claim on a deterministic proxy, on the rule CLAUDE.md calls non-negotiable, and the rename is one line (P1, XS) | new 2026-09-03, from the greenfield eleventh edition (D31); VERIFIED in tree

**What.** `mlb_engine/optimize/optimizer_v3.py:3749`:
`metric_name = 'WTA_First_Place_Proxy' if mode == 'wta' else 'Portfolio_EV_Proxy'`,
with the name repeated in the module changelog at `:66`. The score combines
projection, correlation, salary uniqueness and a heuristic field-pressure term.
No calibrated payout expectation, field distribution, duplicate/tie settlement or
out-of-sample relationship is computed anywhere in the path.

**Why it is P1 despite being XS.** CLAUDE.md's truthful-labels section is
explicit and Ben wrote it: "Never call anything ROI, profitability, win rate,
cash rate, or a probability claim." "EV proxy" asserts that the quantity proxies
expected value, which is a probability-weighted payout claim. R1b already made
this exact correction one shape over, renaming the satellite family's metric to
`Ticket_Line_Advance_Proxy` because labelling a ticket contest `Portfolio_EV_Proxy`
named something the model does not compute; the general case was left standing.
The autonomy section says explicitly that autonomy does not license a probability
claim, so this is the one entry on the board that the project's own contract
already decides.

**Fix.** `Portfolio_Contest_Fit_Proxy` for the non-WTA branch (ed11's suggestion,
and it is the accurate name), `WTA_First_Place_Fit_Proxy` for the WTA branch if
that rename is taken in the same pass; update the module changelog line, the
tests that pin the string, and any brief/QA reader. The target EV estimator, if
it is ever built, is a separate versioned model behind calibration gates and must
never silently reuse this name.

### R313. A forbidden core whose members have left the pool becomes a STRICTER constraint and excludes the lone survivor, which is a legal-pool reduction (P1, XS) | promoted 2026-09-03 out of R303's solver list on a severity argument R303's filing missed; VERIFIED-repro there, re-read here as D19

**What.** `optimizer_v3.py:1129-1144` (R303 filed it at `:1084-1093`; re-read at
HEAD before fixing). `_known_pids(core)` drops members absent from the active
pool, then the row's upper bound is set to `len(pids)-1`. A forbidden
three-player core with only ONE survivor becomes `x_survivor ≤ 0`: the survivor
is excluded from every lineup, even though the original forbidden combination is
already impossible and the row should not be emitted at all.

**Why the severity moves.** R303 filed it as hygiene among thirty other solver
smalls. The mechanism is a REDUCTION OF THE LEGAL PLAYER POOL, which CLAUDE.md's
hard guardrails name as the forbidden move ("Never trim the player pool ... it
may never reduce the legal player set, because that is a strategy change and it
is invisible in the certified output"), and it is invisible in the certified
output in exactly the way that sentence describes. R289 was filed on the mirror
image of this, an explicit instructed restriction silently dropped; this is an
un-instructed restriction silently added. Same class, opposite direction, and the
board had one at P0-adjacent and the other in a hygiene list.

**Fix.** Emit the row only when every member of the original core is still
present in the pool:
    ...
    if len(present) == len(original) and present:
        add_selected_sum_constraint(present, -inf, len(present) - 1)

Acceptance: a three-member forbidden core with two members excluded leaves the
third selectable, and the pool report's legal-player count is unchanged by the
core's presence. The sibling `forbidden_player_combos` path takes the same
treatment in the same commit, and per R233 the entry carries the enumeration of
every site that shrinks a multi-player row to the surviving subset.
...
### R294. CLOSED 2026-09-04 -- SHIPPED, entry migrated to CHANGELOG.md

All three parts (a)(b)(c) landed in one commit. Three premises came back
different from the filing and the corrections are in the changelog entry:
(a) also relaxed `max_candidate_reuse` to no cap and delivered one distinct
lineup across two entries, so the starvation cost portfolio diversity as well
as the stack floor; (b)'s worst case is EIGHT solves and not seven, measured
at 4 and 5 on two fixtures; (c)'s reachable member is status 0 with an
unverifiable incumbent, while statuses 3 and 4 are respectively unreachable
...
**Rider 2026-09-03, the greenfield ELEVENTH edition's Section 4.** It is a
second, independent statement of this same design, from a reviewer working
against `92cd423` who had not read this board: the `dfs/` package map, one owner
per fact, content-addressed inputs, an append-only run state machine with
leases, one versioned JSON result envelope on stdout with human text on stderr,
disjoint `SearchBudget` / `StrategyPolicy` types, a separately implemented
referee contract, and a seven-phase strangler migration (A repair boundaries, B
build the replay instrument FIRST, C extract contracts/intake/referee, D replace
candidate and portfolio internals, E bring Showdown under the gates, F activate
calibrated EV, G delete the old paths and the prose). Two things to take from it
and one not to. **Take:** its Phase B ordering is this entry's start condition
arriving independently, which is worth more than the design itself; and its
§4.10 p95 table is adopted here as ACCEPTANCE CRITERIA rather than as work
(stable-input detect < 250 ms; cached intake rebuild < 150 ms; warm single-lineup
solve < 100 ms Classic and < 30 ms Showdown; warm 150x150 assignment < 500 ms;
warm late-swap package < 1 s; cold 100-candidate bank < 5 s; referee plus atomic
delivery < 250 ms; all measured as distributions over checked-in 1/3/6/9/12-game
fixtures, never one best run). **Do not take:** its "roughly 4,000-6,000 lines
excluding tests" sizing as a plan. That is the estimate every greenfield edition
has made, and criterion (2) of Ben's 2026-09-03 ordering instruction is the
standing answer. Start condition unchanged: R118 built and ten deliveries graded.
...
entry in Workstream 5; **its MECHANISM was fixed by R294(a) on 2026-09-04**
...
- **What, three parts, all "act then report, never act silently":** (a)
  ~~a bullpen-day policy — when a side carries a `PO` and a `PLR` and no `SP`,
  treat the `PLR` as the declared bulk arm automatically~~ **— FALSIFIED
  2026-09-04 by the falsifier this entry named for itself; REWRITTEN below to
  "declare the DEARER arm". The rest of (a) stands: record the
...
**(a) IS FALSIFIED AND REWRITTEN, 2026-09-04, from BUILD fragment
`2026-09-04_BUILD_po-plr-declare-the-dearer-arm.md`.** This entry named its own
falsifier -- "any bullpen-day where the PLR read is wrong and an explicit channel
would have caught it" -- and 2026-09-04 is that day. NYY on the late slate carried
`PO Max Fried $9,000` and `PLR Will Warren $7,000` with no `SP`. **The actual
starter was Fried, the PO** (baseball-reference's Friday previews and Pinstripe
Alley's series preview both name him). (a) as written would have declared Warren
and left the real starter outside the pool; Fried finished in 7 of the 17
delivered entries.

***The archive says it is not a one-off.*** Twenty-eight Classic team-days
carrying a `PO`+`PLR` pair (Showdown excluded: captain pricing is not Classic
pricing and would distort the comparison): **PLR priced higher on 13, PO priced
higher on 7, tied on 8.** So the blanket rule is defensible on 13 of 28, wrong on
7 and undefined on 8. The seven PO-dearer days are 2026-07-21 WSH (Palmquist
$6,000 / Mikolas $5,000), 07-25 SD (Vasquez $6,600 / Sears $6,400), 08-05 DET
(Anderson $8,800 / Madden $5,800), 08-16 PIT (Bachar $6,400 / Barco $6,200), 08-23
PIT (Bachar $6,600 / Mlodzinski $6,100), 09-03 PIT (Bachar $6,000 / Dotel $4,000),
09-04 NYY (Fried $9,000 / Warren $7,000). The eight ties are 07-22 TEX, 07-24 MIA,
08-12 KC, 08-14 TB, 08-18 KC, 08-26 TOR, 08-26 WSH, 09-01 ATH. **Two count errors
to correct while the table is in front of the next reader:** the fragment's own
prose says "30 days" in one place against its 28-row table, and a project-memory
note recorded "PLR dearer on only 15 of 30". The table is the record: 13 / 7 / 8,
28 rows.

***The rewrite.*** **Declare the higher-salaried arm of the pair, not the token.**
DK prices the arm it expects to carry the innings, so salary is the discriminator
the token is not. Where the two are TIED (8 of 28) the pair is genuinely ambiguous
and the existing behaviour is correct: surface it and let the operator confirm.
Acceptance: fixture side with `PO`+`PLR`+no `SP` and unequal salaries declares the
dearer arm with the brief recording the inference and BOTH prices; fixture with
equal salaries does NOT auto-declare; an explicit `--declare-pitcher` still wins in
both cases. **Ungraded, and the grading is about an hour:** this measures DK's
PRICING and not who took the ball on 27 of the 28 days. `data/archive/*/mined_*.json`
carries `fpts` per player, and a starter separates from an opener on `fpts`
cleanly, so the 28 rows can be graded against realized outcomes and this turned
from a pricing heuristic into a measured rule. Do that before shipping the policy;
it is the difference between 13-of-28 and a number.

***The second half of that fragment is a REFUSAL-LEGIBILITY item, not a token
reclassification, and one of its premises is false.*** The fragment reports that
`DK_STARTING_PROBABLE_TOKENS = frozenset({"SP", "P"})`
(`live_data_adapters.py:448`) excludes `PO`, so a side whose only marked arm is a
`PO` reads as having no probable at all -- which is what refused the 2026-09-04
build on attempt 1 with `NYY: no probable or declared starter`. The exclusion is
correct and verified in tree, and the fragment's claim that "`PO` is classified
nowhere" is **FALSE**: `DK_STARTING_OPENER_TOKENS = frozenset({"PO"})` sits at
`:97` and R104 classified it deliberately, with the comment above it recording
why -- an opener throws one or two innings by design and a Classic roster priced
on a starter's workload is a material error, so `PO` carries `BARRED_OPENER_ROLE`
and is barred from pitcher slots outright. Admitting `PO` as a probable would
REVERSE R104 on the strength of one night where DK's own $9,000 price contradicted
its own token. **What is genuinely wrong is the refusal text.** A side with a
`PO` row is not a side with "no probable"; it is a side whose only marked arm is
an opener, which is a different fact with a named remedy (`--declare-pitcher`).
Fix: name the PO arm, its price, and the flag, in the refusal. That is R285/R207's
class -- a refusal that does not name the move that clears it -- and it rides
...
**REPRICED 2026-09-06 from DEV fragment
`2026-09-04_DEV_build-clock-is-91pct-bank-and-nothing-is-parallel.md`. This entry
has sat since 2026-08-04 for want of a measured payoff and it now has one.** The
bank stage is **91-98% of build wall clock** across four measured direct-path runs
(83.6s total / 76.26s bank; 81.4 / 77.63; 72.1 / 70.70; 155.9 / 147.86), one
18-entry six-game build issues roughly 98 `milp` calls, and the base bank pays
**13-20x per candidate** what `extend_bank` pays from the same pool -- the gap
being the accumulating pairwise overlap rows. And the fact that changes the
argument: **zero solver timeouts have ever been recorded, in any brief, on any
date**, so the 30-second limit has never bound and the wall clock is many
individually fast, proven-optimal solves proving optimality to a gap of 8e-5 on an
INPUT to the allocator. Verified in tree: `grep -rn "mip_rel_gap\|x0=" mlb_engine/
--include=*.py` returns 0 at all four `milp` call sites. Members (1), (2) and (3)
of this entry are the first three of a five-member class enumerated on **R321**,
which is the fifth member (concurrency) and was unfiled until now; R321 sequences
BEHIND this entry, because parallelism multiplies whatever the per-solve cost then
is. **Acceptance evidence both items need and neither has: a persisted per-solve
duration histogram.** `_record_solver_status` computes `elapsed_s` per solve
(`optimizer_v3.py:1178`) and only the last survives in `LAST_SOLVER_RESULT`.
...
### R314. `check_started_games` has no postponed-game exemption, it is now in BOTH referees, and on a postponed-game slate CLAUDE.md's own two-referee clause cannot be satisfied at all (P1, S; the input already exists and is thrown away) | new 2026-09-03, merged from DEV fragment `2026-09-02_DEV_started-games-has-no-postponed-exemption.md`, filed deliberately unfixed by the session that shipped R292

**The condition.** `preflight_upload.check_started_games` (`:818`) reads the
salary file's `Game Info` and nothing else, and R287's docstring gives the
reason: it must run at 19:56 on a night with no feed, and `Game Info` is in the
file the operator is already passing. The cost of that choice is that a
POSTPONED game's scheduled start has passed while its players are still
swappable, and the salary CSV cannot tell the two apart.
`verify_export.derive_locked_teams` says so in its own docstring: "It cannot
tell a postponed game from a game in progress."

**Why it is now worse, and why R292 was right to leave it.** R292(d) put the
same check in `verify_export` on its no-parent branch, so the false positive is
in BOTH tools rather than one. That is the CONSISTENT state and it is the reason
not to patch one of them: two implementations of one rule that disagree is this
repo's named no-op failure class. The rule has one owner and the exemption
belongs to the owner.

**What it makes unreachable.** A file legitimately holding players from a
postponed game hard-fails at the money boundary, and the only way past is
`--force`, which exits 4. CLAUDE.md's R272 clause requires **both referees to
exit 0** before a hand-corrected file ships. So on a postponed-game slate that
clause is unsatisfiable, and the operator's only remaining moves are the two the
contract forbids. This is R27's shape at the money boundary: a protection that,
in one real condition, becomes the process preventing the lineup.

**The input already exists and is discarded.**
...
the feed reports postponed, cancelled or suspended, which is an affirmative "has
not locked" and not merely an absence. `resolve_locked_teams` (`:240`) consumes
it, subtracts it, and does not return it.

**Fix.** `check_started_games(entries, salary, as_of, rep, exempt_teams=())`, an
optional set defaulting empty so `preflight_upload`'s own call (which has no
feed at that point) behaves exactly as today; `resolve_locked_teams` returns
`not_locked` as a fourth value (two callers, `verify_export.py:536` and
`test_upload_integrity.py:2580`'s `_resolve` helper, which unpacks three);
`verify_export` passes it. **The exemption is NAMED in the report and never
silently applied**, on R237's rule that an absence and an observation are not the
same fact. Preflight can reach the same evidence when it resolves a feed
(`:2144`), but that happens AFTER `check_started_games` at `:2105` and R287 put
the check early on purpose ("a started game makes every other verdict about that
entry moot"), so moving it is a separate decision and R305's typed feed
resolution should land first.

**NOT VERIFIED, and it is the fact worth having before spending the change:**
whether DK actually keeps a postponed game's slots editable in a bulk CSV
upload. The engine treats postponed as not-locked throughout
(`excluded_game_ids`) and the swap path has behaved that way since R29, but
...
- **Finding 10 (P1, M).** The candidate prefilter keeps one representative per primary stack and per SP pair up to `max(6*E, 40)`, and there is NO retry on the full bank: the single `milp()` call is the only one, and both production callers return on first failure. Worse than the critique states: on `scipy_status == 2` the error reads `"entry-level joint MILP proven infeasible"` and the binding-constraint diagnosis is computed from the FILTERED sets, so "N distinct lineups x cap < entries" can be a fact about the reduced problem only. That is CLAUDE.md's reserved "proven infeasible: <constraint>" label attached to a proof about a problem nobody asked to solve. The time-limit branch already names the prefilter; the infeasible branch must too. Fix: one bounded expansion pass on the full bank before the word "infeasible" is used, and never diagnose from post-filter sets. **Reconfirmed 2026-08-04 (audit) with a live reproduction of the starvation itself:** coverage is guaranteed ONLY for single-option entries (`contest_allocator.py:680`, `if len(options) == 1`) — a 60-candidate bank where an entry's only 2 compatible candidates score below the keep line drops both and reports "proven infeasible … Active:" with an empty controls list, on a problem that is feasible unfiltered. That 2+-options-but-all-filtered shape is exactly the late-swap pinned-entry case (targeted candidates built around locked players score below the general bank). The fix gains a floor: keep the top-K (K>=2) compatible candidates per entry, and name any entry whose compatible set the prefilter emptied. R84's per-entry compatible-in/kept counts are the visibility half. **Premise note, 2026-08-09 (R47), updated 2026-08-10:** the late-swap half of the reasoning above is measured against a bank the solve never saw. The swap's targeted candidates were mostly being DISCARDED before the prefilter ran (`drop_stale_jobs`), so "targeted candidates score below the general bank and get filtered" describes a starvation that had a second, larger cause upstream. **R101 landed 2026-08-10 and the upstream cause is gone; the re-measure is still owed.** It was not done in that commit and could not have been: the 2026-08-03 inputs no longer rebuild that run's projections frame (`data/reference/` has moved since), so the shape has to be measured on the next live swap that produces a pinned entry with 2+ compatible candidates, against the union bank the solve now actually receives. Until that measurement exists, treat this item's late-swap justification as UNSIZED rather than as reproduced. The single-entry coverage floor it proposes is unaffected either way.
  **STATUS 2026-09-04, R294(a) SHIPPED: the mechanism this finding describes is
  FIXED and the fix is the floor this finding asked for.** `_prefilter_candidates`
  now iterates SELECTABLE candidates only, reserves `PREFILTER_PER_ENTRY_RESERVE
  = 2` compatible candidates per entry before any coverage or score fill, and
  reports `entries_emptied_by_prefilter` / `entries_emptied_ids` on every path
  with a WARN on the refusal payload. The three ladders no longer step on a
  non-proof (`proven_infeasible = scipy_status == 2`) and
  `binding_constraints` is empty unless the solve actually proved infeasible, so
  the "diagnosis computed from the FILTERED sets" half is narrowed too: the
  diagnosis still reads post-filter sets, but the filter can no longer empty an
  entry that had an option, which is the condition that made the reading false.
  The 60-candidate 2+-options-all-filtered shape is now a test
  (`AllocatorTruthTests`, `tests/test_core.py`), reproduced at `67bfbf3` and
  asserted fixed. **What is NOT discharged and stays open in slot 14's R36
  Finding 10 line:** the "one bounded expansion pass on the full bank before the
  word infeasible is used" remedy was not built -- with the starvation gone it is
  a different and smaller item -- and the owed LATE-SWAP re-measure is still
  owed. Its question changes from "does the pinned-entry starvation happen" to
  "confirm it no longer does", on the next live swap producing a pinned entry
...
**Rider 2026-09-03, second: `optimizer_v3.coerce_excluded_column` joins the
Tier 5 list, with the decision named rather than left open.** Merged from DEV
fragment `2026-09-02_DEV_coerce-excluded-column-has-no-caller.md`. Measured at
HEAD: `grep -rn "coerce_excluded_column" mlb_engine tools skills tests
--include=*.py` returns ONE hit, the definition at `optimizer_v3.py:700-707`; an
AST walk finds nothing the grep did not. It is a third exported entry point into
the one reading of the `Excluded` column, beside `read_excluded_cell` (scalar)
and `excluded_flags` (frame), and R291 had to choose between readers at four
sites, which is exactly where a same-named third option with no caller gets
picked wrong once. It also carries a live-looking bug shape a caller would hit:
`out['Excluded'] = flags if report['column_present'] else False` writes a scalar
on a column-less frame, so the "real booleans" promise is met two different
ways. **Decision: DELETE it** (R273's dead-site treatment) rather than give it
the caller `_drop_excluded_rows` and the two bank-stamp sites at
`execution_pipeline.py:3446-3449` and `:4823-4826` each open-code a piece of.
Deleting is cheaper and more honest, and the R291 class has two P0-adjacent
filings against it already, so the argument for fewer readers is the whole
subject. Reverse this only if someone remembers the intended caller.
**Rider 2026-09-03: the `stack_core_blocklist` / `forbidden_player_combos`
member is PROMOTED OUT of this entry to R313 at P1**, on a severity argument
this filing missed: the mechanism reduces the LEGAL PLAYER POOL, invisibly in
the certified output, which CLAUDE.md's hard guardrails name as the forbidden
move. It stays listed in the solver paragraph below for continuity of the
enumeration; R313 is the owner and this line is the pointer. The greenfield
eleventh edition re-read it as D19 and reached the same severity independently.
...
**Rider 2026-09-03, the one usable sliver of the greenfield eleventh edition's
D34, whose main claim is REJECTED** (it measured the auditor's own Windows host,
and CLAUDE.md's R271(c) already records that Ben's Windows Python has no scipy,
that `.pylibs/` is a Linux build, and that the gate is therefore the session's
job). What survives is a rule this entry's (d) half already wants: the gate
record should carry the runtime identity (`sys.version`, resolved
`sys.executable`, `platform.platform()`, and the sha256 of `requirements.lock`)
per assembled unit, and **`--gate-report` must REFUSE to print the PASS line
across mixed identities** rather than assembling units from two interpreters into
one claim. That is the same completeness argument the fingerprint already makes
about the tree, extended to the thing running it.
...
**Rider 2026-09-03, sharpening F-48's settlement rule with the greenfield
eleventh edition's §4.7, which states it exactly.** For contest `c` and an entry
`j` scored against the archived field, with `a = 1 + count(i != j : S_i > S_j)`
the first rank the tie group occupies and `d = count(i : S_i = S_j)` the group
size: `Payout_j = (1/d) * Σ[h = a .. a+d-1] prize_c[h]`, and `Net_j = Payout_j -
entry_fee_c`. Duplicates stay separate entries and consume separate ranks. This
is DK's own tie rule and it is arithmetic over an OBSERVED field, so it adds
nothing to the four sim-gate preconditions and is not a simulation. It matters
because the naive alternatives (award `prize[a]` to every tied entry, or average
the whole tied block) are both wrong in the direction that flatters a
concentrated portfolio, which is exactly the portfolio shape this tool exists to
grade.
