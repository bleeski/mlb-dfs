...
- No hitter against a rostered opposing pitcher unless the user explicitly overrides.
...
`project_audit.py --run-tests` must reproduce every test count claimed in the release manifest.
...
Tests for the parked results/calibration/simulation apparatus live in `parked/test_parked.py` and are not part of the audited suite.
...
> Parked as of v2.20.0. The results tracker and post-slate evaluator live in `parked/`; at one accumulated slate they are below their signal thresholds. Raw standings are still archived by hand for when the apparatus is un-parked.
...
> Parked as of v2.20.0. `calibration_engine.py` lives in `parked/` and is statistically inert at current slate volume. No weights, factors, or scores are auto-applied.
...
> Parked as of v2.20.0. `slate_sim.py` lives in `parked/`. Below the comparable-slate threshold it falls back to a static tail and adds no signal, so it is out of the audited engine.
