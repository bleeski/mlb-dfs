...
2. Take the claim: `python tools/claim.py take engine --role DEV --scope "<Session NN or R-numbers>"`. If `engine` for today is held, a scoped `engine_<slug>` blocks nobody (the mutex is nominal), so say so and commit early.
...
Run `/land`. It is the checklist: full gate, CHANGELOG entry in the same commit, roadmap row Complete, NEXT advanced and ledger row, register migration, explicit-path add and commit, subject at most 100 characters, release the claim, do not push.
