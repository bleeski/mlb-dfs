...
6. Check each premise against the tree before building on it: grep for what the entry says is missing, and trace who writes what it reads (`.claude/rules/board.md`). A premise a handful of greps can settle is yours to check directly. For a block with several independent entries, the `dfs-premise` agent can take them in parallel, one per entry, all in one message; its report carries every command and its output, so read that evidence instead of re-running it, and re-check by hand only a verdict its evidence does not support.
...
- `TZ=... date`, `nohup` and `setsid` are bash idioms: they work in a cloud container and not in PowerShell. On Ben's Windows machine use `python` and `Get-Date` instead. Prefer a `python` entry point either way, so the same command works on both.
