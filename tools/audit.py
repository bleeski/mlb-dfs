...
EXPECTED_SUITE_COUNTS = {
    ...
    # Session 06 (R388(b)), 2026-09-23: 1491 -> 1517, the fifteen ControlProvenanceTests (the vocabulary in one place; posture, band-added engine, floor, typed-relaxable, never-relax and per-game weather provenance off the real resolver; confidence_derived unless never-relax; a governed move keeps its prior provenance; F-3's row inside an ungoverned gate; the resolver unchanged without the new arguments; every accepted never-relax held at every relaxer; run_slate publishes and refuses; autobuild forwards once, stops on a held floor, floors an unheld one, filters its effective controls), six DeadlineGovernorTests (held not opened; move record; merge's second lock; F-3 default; comma and repeat parsing; unholdable names refused), two DeadlineGovernorWiringTests (a never-relax cap survives the re-solve; a typed cap alone still opens) and three DeadlineGovernorCliTests (exit 4 before staging; a misspelling names the accepted set; the set reaches the build with F-3).
    # Session 06 review fixes (R388(b)), 2026-09-23: 1517 -> 1524, seven more ControlProvenanceTests (confidence never loosens a cap held below its floor; the games a scalar sets carry its provenance; run_slate labels a moved key from its move; late swap restates never-relax and refuses the cap it strips; a Showdown refusal names the right reason; autobuild's resume keeps the set; autobuild names the child's exit-4 status).
    # R413 (roadmap Session 100), 2026-09-23: 1524 -> 1528, the four PlanStatusTests that pin a three-digit Session ID (row parsed and its status linted; --print lists it in numeric order; NEXT may name it; a ledger row naming it resolves).
    # Session 07 (R207, R396(b), R345), 2026-09-23: 1528 -> 1551, the ten TypedRefusalTests (the probe names each control that alone restores and its step; one that does not says so; the budget bounds it; no budget runs nothing and errors[] stays byte-identical; interaction refusal only; the 1240_6g step; remedy_typed agrees with every failing sentence; typed_refusal_remedy orders and marks held; run_slate threads budget and probe; run_classic carries the remedy and skips the window), one RefusalClassificationTests (every stamp carries refusal_validity), and twelve SupervisorLostWindowTests (the contract codes are build_slate's; a child exit 5 is off-contract; not retried; exhaustion stop; spent resume stop; delivery writes none; --declare-pitcher forwarded and recorded; no declaration no block; resume restores unless restated; the typed floor; the interaction stop's remedy; the strategy stop's remedy).
    # Session 07 review fixes (R207, R345), 2026-09-23: 1551 -> 1558, six more TypedRefusalTests (no single control only when every drop was proven; every step enforces its count and is the smallest, totals 2-200; a remedy value enforces its cap at nine entries; the probe waits for a grown bank and honours not_after; the per-game caps are one row with no step; the probe limits skip the window, T-15 and a relaxable tightening) and one SupervisorLostWindowTests (a flag is never taken as a declaration's value).
    # Session 08 (R393(b), R297(c)), 2026-09-23: 1558 -> 1578, eighteen LastUsableArtifactTests (a crash after create_run ends blocked and crashed; a crash past the first refusal site too; a crashed promotion keeps the certified record and hands the export back; a crash after the inputs moved hands nothing back; stale bytes are not essential-valid; the handler survives its own failed write; the validate_only door; a certified build carries the artifact; the label is the artifact's own; essential validity is gate_classes' V set; coverage off the bytes; the file prints before the narrative; a crash with nothing usable is a crash; a crash after certification exits 7; a raising brief still delivers through _main_recording_refusals; nothing usable still propagates; build_asserted exits through the same door; a Showdown file whose record failed exits 7) and two SupervisorLostWindowTests (a child 7 is a delivery never re-run; a child 7 without a hash-bound brief is a stop).
    # Session 08 review fixes (R393(b), R297(c)), 2026-09-23: 1578 -> 1589, eleven more LastUsableArtifactTests (a crash keeps its stack; raise_on_engine_crash prints it and is late_swap's one definition; late_swap checks for a crash before any passed read; a failed report tail publishes no mirror; the mirror's own error reaches the result; an unpromoted mirror name is a later failure and never presented; a refused deferred promotion carries no artifact; the file is presented when its record cannot be built; the exception brief takes the file's date; the handler survives an unprintable exception and a promoted run; a deadline Showdown file carries the deadline label).
    # Session 102 (R415), 2026-09-24: 1589 -> 1606, eleven BankCapGrowthTests (the cap scales with the call budget and Cowork keeps twelve per entry; an ambient ceiling never grows Cowork and the floor, ceiling and sub-130 bounds hold; the operator cap is honored with its source; the flag selects the sliced door and reaches the brief; stop_reason reads candidate_cap, time_budget and job_list_exhausted; the bank reason resolves over every slice; a same-cap re-run adds nothing and a raised cap grows; exit 10 is not a loop at the cap; every grow remedy, the allocator's included, names the lever; a share-capped slice does not stop a bank that still grows, through the real extend_bank; an answered list re-run at its cap reads exhausted) and six AutobuildBankCapTests (a capped refusal doubles the cap and records raise_bank_cap; at the ceiling it stops raising; a clock-bound refusal grows by re-run from the refusal brief's bank_exploration; a BANK-LIMITED refusal on the direct door takes the sliced bank once; a failing slate check goes before bank growth; the operator cap is forwarded, recorded and restored on --resume).
    # Session 103 (R416), 2026-09-24: 1606 -> 1615, seven DirectDoorResolveBankTests (the R407 re-solve reuses the bank and gets no second budget; the calls together stay inside the window; the governed re-solve solves on the first bank; a re-solve with nothing to reuse builds on what is left; the refusal brief and record carry each solve; a raise after delivery keeps the solves; the direct floor notice names the direct remedy) and two RunSlateAutoBankReuseTests (the bank is handed back and reused with no second build, its record in candidate_bank and the same file; a reused bank beside an override is refused).
    # Session 09 (R388(d), R124(a), R268), 2026-09-24: 1615 -> 1652, twenty-one ReviewGradeExportTests (a P-only refusal is byte-identical and delivers UNCERTIFIED; preflight review_ready naming the gates; a MIXED gate with a V fact stays DO_NOT_UPLOAD; a certified build unchanged and records its controls; recorded controls keep numbers and drop the id maps; never mirrored over a certified row, same bytes included; certified supersedes UNCERTIFIED; the manifest backstop; S and P gates placed and V gates block; the wrong-draftgroup errors are V; a cap breach ships unless never-relax, the game scalar included; an unclaimed error is V; an S cap breach through execute_portfolio and its never-relax twin; an off-allowlist method is P and a V failure beside it blocks; selection_certified is not V; a raising verdict leaves the refusal as it was; bytes that do not bind are withheld; a refused swap run carries none; the refusal brief names and presents the file; a later V refusal still names it; a file failing the re-read is not presented) and sixteen LateSwapReviewParentTests (a review-grade parent with no flag and no pointer; past an unrelated promotion; a superseded promoted parent; a certified unpromoted parent; no match still raises; a V-failing candidate refused by name; a tampered review-grade parent fails integrity; the swap inherits what the parent shipped; a pre-R268 parent records none; the swap label never beats the parent's; a deadline label read from the run; an UNCERTIFIED parent carries its gates; a governed parent that still failed is UNCERTIFIED, not deadline; a downgrade label read from the tracked record alone; the tool refuses before the bank; a dry run only warns).
    # Session 10 (R389(a)), 2026-09-24: 1652 -> 1686, the thirty-four BaselineCoreTests (the helper is the ValueError branch's frame byte for byte; run_classic's degraded frame goes through it and its enriched attempt does not; every reserved row covered and no contest holds a lineup twice; two slot orders of one player set are one lineup; a held lineup is never reassigned in its own contest; a partial row is named and never filled; a requirements mapping gives the CSV result; both or neither input is a caller error; candidate #1 is the probe; a raising probe is recorded and the grid supplies #1; a pool with no pitchers is short and never raises; a window ending after the probe returns a short count; an expired deadline solves nothing; the distinct fill covers what the grid cannot; every solve sees the whole frame; the anti-correlation allowance reaches every solve; every BankCache method that reads its path is overridden; the core writes nothing, audit-hooked in a subprocess; the same candidates under two hash seeds; the window defaults to the host call budget; an off-salary ID is dropped and its row stays uncovered; the allocator payload keeps the probe first in BankCache's shape; the labels are truthful; and, from the diff review, a contest named twice is a caller error; a held signature matches whatever its order; a Showdown entries file is refused; a frame the salary file does not describe is a caller error; covered rows with a missed target say so; a malformed roster is never a candidate; a rejected fill roster joins the overlap rows; a timed-out fill solve names its own limit; a raising grid keeps what it built; the payload refuses to drift off candidate_index).
    # Session 11 (R389(b)), 2026-09-24: 1686 -> 1717, the thirty-one ClassicBaselineFirstTests (a crash in research or at the enhanced solve's entry delivers the baseline with exit 7; the enhanced file supersedes nothing Ben holds and preflight passes on both files' bytes; the baseline is essential-valid on its exact bytes; it runs after the leverage check and before research; its run_slate call is approved, unenriched, labelled, in its lineage and opened never-relax-aware; the engine's call is pinned approve=True with the allowance; the window is a share of the build's deadline; short, refused and raising baselines are named and the build continues; a failed re-read, essential check or label is never presented; the governor still walks; a refusal keeps exit 3 and re-presents the baseline last; a rerun reuses the row; the row reads relaxed with its note; supersession stays inside a lineage; identical bytes in two lineages are two live rows; a live baseline never withholds an UNCERTIFIED file; a refinement retires the other lineages and chains its parent; an unknown lineage is refused; run_slate refuses a bad lineage before any work; preflight names the retiring file; a late swap records in its parent's lineage; a re-promoted baseline keeps its label and lineage; outcome review files beside the enhanced; an S/P-only baseline is never published; a filled entries file is rebuilt whole; an earlier live certified row is named; the output never claims upload-ready; and, from the diff review, an enhanced file that fails its re-read leaves the baseline current and names no row of its own; a baseline run is never re-promoted under the enhanced name).
    # R421, 2026-09-25: 1717 -> 1732, the fifteen ClassicTailSeatTests (the coverage table; the market-only bottom third, dropped by name; top-heavy placement at most half a contest, none for a cash posture; tail seats off the contest before the weights; a pinned seat takes only its team's stack, caps binding, a stack under the floor no seat, a missing team counted; both switches change nothing; depth jobs only when short, counting distinct projection-world rosters and the direct door's own bank; one helper for the bank and the stamp; run_slate stamps before any door; the delivery record carries the market and a re-promotion keeps it; the miner keeps the stack team at the engine's threshold; the brief line).
    "tests.test_core": 1732,
