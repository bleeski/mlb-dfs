...
EXPECTED_SUITE_COUNTS = {
    ...
    # Session 102 (R415), 2026-09-24: 1589 -> 1603, nine BankCapGrowthTests (the cap scales with the call budget and Cowork keeps twelve per entry; an ambient ceiling never grows Cowork and the floor, ceiling and sub-130 bounds hold; the operator cap is honored with its source; the flag selects the sliced door and reaches the brief; stop_reason reads candidate_cap, time_budget and job_list_exhausted; the bank reason resolves over every slice; a same-cap re-run adds nothing and a raised cap grows; exit 10 is not a loop at the cap; every grow remedy, the allocator's included, names the lever) and five AutobuildBankCapTests (a capped refusal doubles the cap and records raise_bank_cap; at the ceiling it stops raising; a clock-bound refusal grows by re-run from the refusal brief's bank_exploration; a BANK-LIMITED refusal on the direct door takes the sliced bank once; the operator cap is forwarded, recorded and restored on --resume).
    "tests.test_core": 1603,
