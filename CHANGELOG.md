...
**Why.** Ben's instruction, 2026-09-02: merge the edition into the backlog, then re-prioritize on how much an item changes what wins (large prizes first), how much lift it costs, and what it depends on, adjudicating the large-impact/large-lift and low/low cases rather than asking. The rule the list now states and applies, in order: a legal file entered on every slate; the file holding the construction the operator asked for; construction the archive says wins this field (R118); the referee catching a bad upload; the record reading true at T-5; hygiene. Every impact is a deterministic mechanism, never a rate.

**The thirteen numbers, and what closed items they reopen.** R291 (`Excluded` dropped at `_assemble_projection_frame`, re-stamped, overwritten on swap, unread by the Showdown melt and the bank grid — R289's class, two of five sites), R292 (repair tool writes header + repaired rows + the whole original table; admits an observed-`started` player; three `--as-of` semantics; `verify_export` never runs the started check — R272's operational half), R293 (`_teams_of_pair` skips the own team not the opponent; anti-correlation control absent from the auto-bank and the augmentation whitelist while the brief says applied — R288's members six and seven; R164(b)'s premise reversed), R294 (prefilter ignores compatibility so the stack floor relaxes against a bank that could satisfy it; ladders re-solve after a failing slate check; status 3/4 called proven), R295 (R250 hold vs thesis lock breaches the player cap 7/12 under 6; R247(a)'s Showdown proxy is thesis-weighted; rung 5 duplicate solve; melt key), R296 (eight lost-window doors; R290(c)'s count is nine plus two, not eight; R213's fourth door; R169(b) three of twenty; R285's third reader), R297 (validator certifies any ten ids on an empty pool; `workflow_gates` overrides any derived gate; crash strands `building`; preflight pitcher test dead on Showdown; `passed` frozen before the R176 read-back), R298 (three T-5 readers that lie; R176(d)'s brief half), R299 (six intake seams incl. the UTC slate date and WAS/OAK), R300 (the two tests that would have caught R291 and the 09-01 PASS; string pins on money-adjacent claims), R301 (fifteen quoted contradictions; the corpus restructure), R302 (the strangler engine, filed as a decision with a start condition), R303 (the hygiene tail, eleven members demoted to Tier 5 in place).

**The adjudications, recorded because they are the part that gets lost.** R302 (large impact, L) to slot 14 behind a hard start condition — R118 built and ten deliveries graded — not struck and not the head: the fixes above it define the behaviour a strangler must reproduce, and R118 is what would grade it; the 2026-08-01 "parallel package" rejection rested on an EV-core premise R302 does not share, and the Do-not-build paragraph narrows that rejection on the §3.6 measurement rather than by argument. R290(c) (P0 by value, M-L) keeps its rank but lands after its S-sized neighbours in slot 3. R301 (M) at 13, ahead of R302 and behind every code batch. R118 PROMOTED to 6, above every prospective referee item, because six weeks unbuilt is the measured cost and three slots wait on it. The money-boundary batch DEMOTED from 2 to 9 on its own notes' concession that every member but R279 is prospective; R297's fail-opens join it. Audit hardening (old 6-8) to 12: no delivered byte. R268 + R204 (old 1) to 7: measured cost stands, but R292 fixes the tool it runs through. Smalls to 15 with eleven demoted to Tier 5. The previous list is the file at `32cdd32` (`git show 32cdd32:docs/backlog.md`); git is the record, not a second copy in the section.

**R233 enumeration for this entry (docs only, so the class is documents).** Files carrying the queue order: `docs/backlog.md` (the only list; rewritten), `NEXT_SESSION_PROMPT.md` (its "What to tackle, in order" pointed at the old slots; addendum added, list left as history), `docs/2026-09-02_critique_greenfield_spec_ed10.md` §4.8 (its sequencing was written before the re-prioritization and says so in the addendum below — the board wins). Grep: `R290\(a\)` as the first thing to tackle appears in NEXT_SESSION_PROMPT.md:82 and nowhere else; superseded by the addendum.

**Environment, disclosed.** `mcp__workspace__bash` was denied for the whole board half of the session. Consequences honoured rather than hidden: the engine claim was re-taken by writing `claims/engine_2026-09-02/owner.json` with the same payload `claim.py take` writes (`taken_utc` approximate, stated in `scope`); the consumed fragment could not be moved to `_to_delete/` and carries a CONSUMED banner instead; no commit was made — the commit and push are handed to Ben in the session close-out, with the lock sweep in the same PowerShell block. `NEXT_SESSION_PROMPT.md` says the same.
